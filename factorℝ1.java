...
public class factorℝ1 implements Function<Integer, Real> {
   ...
   public Real evaluate(Integer in, int order, int bits, Real result) {
      ...
      do {
         ∏ℝ1.mul(factorℝ2.evaluate(k, bits, valueℝ2), bits);
      } while(k.increment().compareTo(endIndexℤ1) <= 0);

      var10000.mul(∏ℝ1, bits, ℝ1);
      Real var10001 = result.div(in.factorial(bits, ℤ2).set(ℝ2), bits, ℝ3);
      factorℝ3.n = in;
      ∏ℝ2.multiplicativeIdentity();
      k.set(c1);
      endIndexℤ2.set(q);

      do {
         ∏ℝ2.mul(factorℝ3.evaluate(k, bits, valueℝ3), bits);
      } while(k.increment().compareTo(endIndexℤ2) <= 0);

      return var10001.mul(∏ℝ2, bits, result);
   }
