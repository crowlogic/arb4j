                                     ...
                                     TestCase
{
  ...
  /**
   * z = 2i: Σ zᵏ diverges, but its Padé resummation is 1/(1−2i) = 0.2 + 0.4i,
   * exact at 3 terms.
   */
  public void testDivergentComplexGeometricExactlyResummed()
  {
    try ( ComplexEpsilonTable t = new ComplexEpsilonTable(8,
                                                          bits);
          Complex Z = new Complex(); Complex pow = new Complex(); Complex s = new Complex(); Complex est = new Complex(); Complex lim = new Complex();
          Complex denom = new Complex(); Complex d = new Complex(); Real mag = new Real(); Real tol = new Real())
    {
      Z.set(0.0, 2.0);
      pow.one();
      s.zero();
      ...
      d.sub(lim, bits, d);
      d.abs(bits, mag);
      assertTrue("complex resummation of geometric(2i)=" + est + " must equal 1/(1-2i)=0.2+0.4i, |Δ|=" + mag, mag.compareTo(tol) < 0);
  ...
  {
    try ( ComplexEpsilonTable t = new ComplexEpsilonTable(40,
                                                          bits);
          Complex out = new Complex(); Complex lim = new Complex(); Complex denom = new Complex(); Complex d = new Complex(); Real mag = new Real();
          Real tol = new Real())
    {
      denom.set(1.0, -2.0);
