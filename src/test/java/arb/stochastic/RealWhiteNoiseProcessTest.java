                                       ...
                                       TestCase
{
  public void testWhiteNoiseSampleStream()
  {
    ...
    try ( RealWhiteNoiseProcess whiteNoise = new RealWhiteNoiseProcess())
    {
      ...
      int    limit = 10000;
      double mean  = whiteNoise.stream(128, limit).mapToDouble(Real::doubleValue).average().getAsDouble();
      whiteNoise.initializeWithSeed(777);
      double mean2 = whiteNoise.sample(128, Real.newVector(limit)).stream().mapToDouble(Real::doubleValue).average().getAsDouble();
      assertEquals(mean, mean2);
      whiteNoise.initializeWithSeed(777);
      double variance = whiteNoise.stream(128, limit).mapToDouble(x -> x.pow(2, bits).doubleValue()).average().getAsDouble();
