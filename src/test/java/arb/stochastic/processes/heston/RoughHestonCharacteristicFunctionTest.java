                                                   ...
                                                   TestCase
{
  ...
  /**
   * Identity-of-μ invariant: after construction, {@code cf.μ} is the same
   * {@link Real} instance as {@code cf.riccati.α} and as
   * {@code cf.context.getVariable("μ")}. Mutating any one propagates.
   */
  public void testMuIsCanonicalSingleInstance()
  {
    try ( var φ = new RoughHestonCharacteristicFunction())
    {
      Real ctxMu = φ.context.getVariable("μ");
      assertSame("φ.μ should be the same instance as the Context's μ", φ.μ, ctxMu);
      assertSame("φ.μ should be the same instance as riccati.α", φ.μ, φ.riccati.α);
    }
  }
  ...
  public void testContextBasedConstructorSharesInstances()
  {
                            ...
                            prec).setName("T"))
    {
      ...
      {
        assertSame("CF should adopt the caller's μ instance", μ, φ.μ);
        assertSame("CF should adopt the caller's λ instance", λ, φ.λ);
        assertSame("CF should adopt the caller's T instance", T, φ.T);
