                                                   ...
                                                   TestCase
{
  ...
  {
    try ( Real λ = new Real("0.3", prec);
          Real θ = new Real("0.02", prec);
          Real ν = new Real("0.3", prec);
  ...
  public void testModulusAtMostOneForRealArgument()
  {
          ...
          Real mod = new Real())
    {
      for (int ui = -5; ui <= 5; ui++)
      {
        ...
        φ.evaluate(u, 1, prec, res);
        res.abs(prec, mod);
        assertTrue("|φ(u)| must be ≤ 1, got " + mod + " at u=" + ui,
                   mod.compareTo(new Real("1.0", prec)) <= 0
                   || mod.sub(new Real("1.0", prec), prec, new Real()).containsZero());
      }
    }
  }
  ...
  public void testConvergenceInN()
  {
    try ( Real λ = new Real("0.3", prec);
          ...
          Real ρ = new Real("-0.7", prec);
          Real μ = new Real("0.6", prec);
          RoughHestonCharacteristicFunction φ8 = new RoughHestonCharacteristicFunction(λ, θ, ν, V0, ρ, μ, 8, prec);
          RoughHestonCharacteristicFunction φ10 = new RoughHestonCharacteristicFunction(λ, θ, ν, V0, ρ, μ, 10, prec);
          Complex u = new Complex();
          Complex r8 = new Complex();
          Complex r10 = new Complex())
    {
      u.re().set("1.0", prec);
      u.im().zero();
      φ8.evaluate(u, 1, prec, r8);
      φ10.evaluate(u, 1, prec, r10);
      assertEquals("Re convergence N=8 vs N=10", r8.re().doubleValue(), r10.re().doubleValue(), 1e-6);
      assertEquals("Im convergence N=8 vs N=10", r8.im().doubleValue(), r10.im().doubleValue(), 1e-6);
    }
  }
  ...
  public void testConstructorRejectsNLessThanOne()
  {
          ...
          Real μ = new Real("0.6", prec))
    {
      try
      {
        new RoughHestonCharacteristicFunction(λ, θ, ν, V0, ρ, μ, 0, prec);
  ...
  public void testConstructorRejectsPrecLessThan32()
  {
          ...
          Real μ = new Real("0.6", prec))
    {
      try
      {
        new RoughHestonCharacteristicFunction(λ, θ, ν, V0, ρ, μ, 3, 16);
