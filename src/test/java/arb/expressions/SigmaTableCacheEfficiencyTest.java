...
/**
 * Detects exponential σ-table recomputation (#1034) via IndexCache hit/miss
 * counts.
 *
 * The σ-table's 3-term recurrence should cache ~O(n*m) values. Without proper
 * caching, it generates millions of cache accesses = exponential recomputation.
 */
public class SigmaTableCacheEfficiencyTest
{
  ...
  public void testCacheHitsDetectExponentialRecomputation()
  {
    ...
    assertTrue("σ-table was not exercised: totalAccesses=" + totalAccesses + " — the test measured nothing and must NOT pass as green", totalAccesses >= 100);
    ...
    if ( totalAccesses == 0 )
    {
      fail("thing didnt count anything");
    }
    if (totalAccesses > 10_000)
    {
      fail(String.format("EXPONENTIAL RECOMPUTATION DETECTED (#1034):\n"
                         + "  Cache Hits:    %,d\n"
                         + "  Cache Misses:  %,d\n"
                         + "  Total Access:  %,d\n"
                         + "  Hit Rate:      %d%%\n"
