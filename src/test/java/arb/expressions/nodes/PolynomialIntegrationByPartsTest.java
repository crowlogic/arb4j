...
/**
 * Unit tests for polynomial integration-by-parts functionality. Tests the
 * tabular method implementation in MultiplicationNode.integrate().
 * 
 * The tabular method computes ∫ p(x)·f(x) dx by repeatedly differentiating the
 * polynomial p(x) (which eventually becomes 0) while integrating f(x).
 * 
 * @author Stephen Crowley ©2024-2025
 * @see arb.documentation.BusinessSourceLicenseVersionOnePointOne © terms
 */
public class PolynomialIntegrationByPartsTest extends
                                              TestCase
{
  /**
   * Test ∫ x e^x dx = e^x(x - 1)
   * 
   * Tabular method: u = x → u' = 1 → u'' = 0 dv = e^x → v = e^x → v₁ = e^x
   * 
   ...
   * 
   * Tabular method: u = x² → u' = 2x → u'' = 2 → u''' = 0 dv = e^x → v = e^x → v₁
   * = e^x → v₂ = e^x
   * 
   ...
   * 
   * Tabular method: u = x → u' = 1 → u'' = 0 dv = sin(x)→ v = -cos(x)→ v₁ =
   * -sin(x)
   * 
   ...
   * 
   * Tabular method: u = x → u' = 1 → u'' = 0 dv = cos(x)→ v = sin(x) → v₁ =
   * -cos(x)
   * 
   ...
   * 
   * Using LIATE rule: u = ln(x), dv = x dx u = ln(x) → du = 1/x dx dv = x → v =
   * x²/2
   * 
   * Result: (x²/2)·ln(x) - ∫ (x²/2)·(1/x) dx = (x²/2)·ln(x) - ∫ x/2 dx =
   * (x²/2)·ln(x) - x²/4
   */
  ...
  /**
   * Test constant × exp: ∫ 3 e^x dx = 3 e^x This is a simple case that doesn't
  ...
  /**
   * Test the main example from issue #635: ∫ (2x² + 3x) e^x dx
   * 
   * By linearity and tabular method: = 2∫ x² e^x dx + 3∫ x e^x dx =
   * 2·e^x(x²-2x+2) + 3·e^x(x-1) = e^x(2x² - 4x + 4 + 3x - 3) = e^x(2x² - x + 1)
   * 
   * At x=1: e(2-1+1) = 2e
   */
   ...
   * 
   * F(x) = e^x(x-1) F(1) - F(0) = e(1-1) - e⁰(0-1) = 0 - (-1) = 1
   */
  public void testDefiniteIntegralXTimesExp()
  {
    // Antiderivative at x=1
    var F = RealFunction.express("x→int(x*exp(x),x)");
    try ( Real one = new Real("1.0",
                              128);
          Real zero = new Real("0.0",
                               128);
          Real F1 = new Real(); Real F0 = new Real())
    {
