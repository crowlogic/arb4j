 ...
 * complex) into H such that ⟨Ax,y⟩=⟨x,yA⟩ for all x,y∈DA. If DA is an
 ...
 * DA=DB is again a SymmetricOperator, while if λ is a real number, then λA is
 * also symmetric. Every symmetric operator A has a uniquely defined closure
 * Aoverbar and an adjoint A^*⊃Aoverbar. In general, A^* is not symmetric and
 * A^∗≠Aoverbar. If A^∗=A, then A is called a self-adjoint operator. This holds,
 * for example, in the case of symmetric operators defined on the whole of H. If
 * A is symmetric and bounded on DA, then A can be extended as a bounded
 * symmetric operator to the whole of H.
 * 
 * @see <a href=
