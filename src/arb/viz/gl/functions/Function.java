...
public class Function
{
  ...
  public void setVar(String newVar)
  {
    ...
    {
      if (ExpressionHandler.isVariable(token) && !ExpressionHandler.isNumber(token) && !token.equals(newVar))
      {
        reduced.add("0");
      }
      else
  ...
  {
    return new Function((new ASTree(postfix.clone())).diff(var).toPostfix(),
                        var);
  }
