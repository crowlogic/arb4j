...
/**
 * Simple example of how to rotate axis label and different label
 * collision-avoidance schemes
 *
 * @author rstein
 */
public class RotatedAxisLabelSample extends
                                    Application
{
  private static final Logger LOGGER    = LoggerFactory.getLogger(RotatedAxisLabelSample.class);
  private static final int    N_SAMPLES = 100;                                                  // default number of
                                                                                                // data points
  ...
  public void start(final Stage primaryStage)
  {
    ...
    final XYChart chart = new XYChart(xAxis0,
                                      yAxis0);
    ...
    for (AxisLabelOverlapPolicy policy : AxisLabelOverlapPolicy.values())
    {
      ...
      if (policy.equals(AxisLabelOverlapPolicy.SKIP_ALT))
      {
        ...
        final DefaultNumericAxis yAxis2 = getSynchedAxis(yAxis0,
                                                         "y-axis (-90°, " + policy + ") + extra label spacing");
