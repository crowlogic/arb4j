...
public class HistogramRendererBarSample extends
                                        Application
{
  ...
  // country,age,men 1980 [%],men 2000 [%],men 2050 [%],women 1980 [%],women 2000
  // [%],women 2050 [%],men 1980,men 2000,men 2050,women 1980,women 2000,women
  // 2050
  private final Map<String, Map<String, Map<Integer, Histogram>>> relDistribution   = new ConcurrentHashMap<>();                                // Map<country
                                                                                                                                                // name,
                                                                                                                                                // Map<men/women,
                                                                                                                                                // Map<year,Histogram>>>
  private final Map<String, Map<String, Map<Integer, Histogram>>> absDistribution   = new ConcurrentHashMap<>();                                // Map<country
                                                                                                                                                // name,
                                                                                                                                                // Map<men/women,
                                                                                                                                                // Map<year,Histogram>>>
  ...
  public void start(final Stage primaryStage)
  {
    ...
    toolBarData.getItems()
               .add(new HBox(new Label("country: "),
                             country));
    ...
    country.getSelectionModel()
           ...
           .addListener((ch, o, n) -> updateHistogram(country.getValue(), year.getValue(), relative.isSelected()));
    year.getSelectionModel()
        .selectedItemProperty()
        .addListener((ch, o, n) -> updateHistogram(country.getValue(), year.getValue(), relative.isSelected()));
    relative.selectedProperty()
            .addListener((ch, o, n) -> updateHistogram(country.getValue(), year.getValue(), relative.isSelected()));

    root.setTop(new VBox(toolBarData,
                         toolBarRenderer));
  ...
  private void updateHistogram(final String country, final int year, final boolean relative)
  {
    ...
    histogramRenderer.getDatasets()
                     .setAll(dist.get(country).get(WOMEN).get(year), dist.get(country).get(MEN).get(year));
    if (relative)
    {
  ...
  {
    try ( BufferedReader csvReader = Files.newBufferedReader(Paths.get(Objects.requireNonNull(this.getClass()
                                                                              ...
                                                                              .toURI())))
    {
      ...
      while ((row = csvReader.readLine()) != null)
      {
        ...
        final Map<String, Map<Integer, Histogram>> relCountryMap            = relDistribution.computeIfAbsent(country,
                                                                                                              c -> new ConcurrentHashMap<>());
        final Map<Integer, Histogram>              relWomenDistribution     = relCountryMap.computeIfAbsent(WOMEN,
                                                                                                            g -> new ConcurrentHashMap<>());
        final Map<Integer, Histogram>              relMenDistribution       = relCountryMap.computeIfAbsent(MEN,
                                                                                                            g -> new ConcurrentHashMap<>());
        final MyHistogram                          relWomenDistribution1980 = (MyHistogram) relWomenDistribution.computeIfAbsent(TAG_1980,
                                                                                                                                 y -> new MyHistogram(TAG_1980
                                                                                                                                               + ": "
                                                                                                                                               + country
                                                                                                                                               + " - "
                                                                                                                                               + WOMEN
                                                                                                                                               + TAG_REL));
        final MyHistogram                          relMenDistribution1980   = (MyHistogram) relMenDistribution.computeIfAbsent(TAG_1980,
                                                                                                                               y -> new MyHistogram(TAG_1980
                                                                                                                                             + ": "
                                                                                                                                             + country
                                                                                                                                             + " - "
                                                                                                                                             + MEN
                                                                                                                                             + TAG_REL));
        final MyHistogram                          relWomenDistribution2000 = (MyHistogram) relWomenDistribution.computeIfAbsent(TAG_2000,
                                                                                                                                 y -> new MyHistogram(TAG_2000
                                                                                                                                               + ": "
                                                                                                                                               + country
                                                                                                                                               + " - "
                                                                                                                                               + WOMEN
                                                                                                                                               + TAG_REL));
        final MyHistogram                          relMenDistribution2000   = (MyHistogram) relMenDistribution.computeIfAbsent(TAG_2000,
                                                                                                                               y -> new MyHistogram(TAG_2000
                                                                                                                                             + ": "
                                                                                                                                             + country
                                                                                                                                             + " - "
                                                                                                                                             + MEN
                                                                                                                                             + TAG_REL));
        final MyHistogram                          relWomenDistribution2050 = (MyHistogram) relWomenDistribution.computeIfAbsent(TAG_2050,
                                                                                                                                 y -> new MyHistogram(TAG_2050
                                                                                                                                               + ": "
                                                                                                                                               + country
                                                                                                                                               + " - "
                                                                                                                                               + WOMEN
                                                                                                                                               + TAG_REL));
        final MyHistogram                          relMenDistribution2050   = (MyHistogram) relMenDistribution.computeIfAbsent(TAG_2050,
                                                                                                                               y -> new MyHistogram(TAG_2050
                                                                                                                                             + ": "
                                                                                                                                             + country
                                                                                                                                             + " - "
        ...
        final Map<String, Map<Integer, Histogram>> absCountryMap            = absDistribution.computeIfAbsent(country,
                                                                                                              c -> new ConcurrentHashMap<>());
        final Map<Integer, Histogram>              absWomenDistribution     = absCountryMap.computeIfAbsent(WOMEN,
                                                                                                            g -> new ConcurrentHashMap<>());
        final Map<Integer, Histogram>              absMenDistribution       = absCountryMap.computeIfAbsent(MEN,
                                                                                                            g -> new ConcurrentHashMap<>());
        final MyHistogram                          absWomenDistribution1980 = (MyHistogram) absWomenDistribution.computeIfAbsent(TAG_1980,
                                                                                                                                 y -> new MyHistogram(TAG_1980
                                                                                                                                               + ": "
                                                                                                                                               + country
                                                                                                                                               + " - "
                                                                                                                                               + WOMEN
                                                                                                                                               + TAG_ABS));
        final MyHistogram                          absMenDistribution1980   = (MyHistogram) absMenDistribution.computeIfAbsent(TAG_1980,
                                                                                                                               y -> new MyHistogram(TAG_1980
                                                                                                                                             + ": "
                                                                                                                                             + country
                                                                                                                                             + " - "
                                                                                                                                             + MEN
                                                                                                                                             + TAG_ABS));
        final MyHistogram                          absWomenDistribution2000 = (MyHistogram) absWomenDistribution.computeIfAbsent(TAG_2000,
                                                                                                                                 y -> new MyHistogram(TAG_2000
                                                                                                                                               + ": "
                                                                                                                                               + country
                                                                                                                                               + " - "
                                                                                                                                               + WOMEN
                                                                                                                                               + TAG_ABS));
        final MyHistogram                          absMenDistribution2000   = (MyHistogram) absMenDistribution.computeIfAbsent(TAG_2000,
                                                                                                                               y -> new MyHistogram(TAG_2000
                                                                                                                                             + ": "
                                                                                                                                             + country
                                                                                                                                             + " - "
                                                                                                                                             + MEN
                                                                                                                                             + TAG_ABS));
        final MyHistogram                          absWomenDistribution2050 = (MyHistogram) absWomenDistribution.computeIfAbsent(TAG_2050,
                                                                                                                                 y -> new MyHistogram(TAG_2050
                                                                                                                                               + ": "
                                                                                                                                               + country
                                                                                                                                               + " - "
                                                                                                                                               + WOMEN
                                                                                                                                               + TAG_ABS));
        final MyHistogram                          absMenDistribution2050   = (MyHistogram) absMenDistribution.computeIfAbsent(TAG_2050,
                                                                                                                               y -> new MyHistogram(TAG_2050
                                                                                                                                             + ": "
                                                                                                                                             + country
                                                                                                                                             + " - "
  ...
  private static Chart getDefaultChart(final String title)
  {
    ...
    DefaultNumericAxis yAxis2 = new DefaultNumericAxis("age group",
                                                       null);
  ...
  public static class MyHistogram extends
                                  de.gsi.dataset.spi.Histogram
  {
    public MyHistogram(final String name)
    {
      super(name,
            ...
            NBINS - 1.0,
            false,
            HistogramOuterBounds.BINS_CENTERED_ON_BOUNDARY);
    ...
    public void fill(final String dataLabel, final double value)
    {
      ...
      LOGGER.atWarn()
            .addArgument(dataLabel)
            .addArgument(value)
            .log("encountered unknown label '{}' -  adding value = {} to under-flow bin");
      addBinContent(0, value); // fill to under-flow bin
    }
  }

  private static class HistogramRendererParameterTooBar extends
                                                        ToolBar
  { // NOPMD NOSONAR -- number of parents
    ...
    private void init()
    {
      ...
      final Spinner<Number> dynBarWidth = new Spinner<>(0,
                                                        100,
                                                        histogramRenderer.getBarWidthPercentage(),
                                                        10);
      ...
      final Spinner<Number> barWidth = new Spinner<>(0,
                                                     100,
                                                     histogramRenderer.getBarWidth());
      ...
      final Spinner<Number> shiftBarOffset = new Spinner<>(0,
                                                           100,
                                                           histogramRenderer.getShiftBarOffset());
