 ...
 * @author rstein
 */
public class HistoryDataSetRendererSample extends
                                          Application
{
  ...
  private void generateData(final XYChart chart)
  {
    ...
    dataSetNoError.lock().writeLockGuard(() -> dataSet.lock().writeLockGuard(() ->
    {
      for (int n = 0; n < HistoryDataSetRendererSample.N_SAMPLES; n++)
      {
        ...
        final double y1    = a1 * GaussFunction.gauss(x,
                                                      HistoryDataSetRendererSample.N_SAMPLES * 1.0 / 3.0,
                                                      HistoryDataSetRendererSample.N_SAMPLES / 20.0)
                      * 1000;
        final double y2    = a2 * GaussFunction.gauss(x,
                                                      HistoryDataSetRendererSample.N_SAMPLES * 2.0 / 3.0,
                                                      HistoryDataSetRendererSample.N_SAMPLES / 20.0)
                      * 1000;
  ...
  public void start(final Stage primaryStage)
  {
    ...
    final DefaultNumericAxis yAxis1 = new DefaultNumericAxis("y axis #1",
                                                             0.0,
                                                             1.3,
                                                             0.2);
    ...
    final DefaultNumericAxis yAxis2 = new DefaultNumericAxis("y axis #2",
                                                             0.0,
                                                             1.3,
                                                             0.2);
    ...
    startTimer.setOnAction(evt ->
    {
      if (timer == null)
      {
        timer = new Timer("sample-update-timer",
                          true);
        timer.scheduleAtFixedRate(getTask(chart),
                                  HistoryDataSetRendererSample.UPDATE_DELAY,
