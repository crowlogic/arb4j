 ...
 * @author rstein
 */
public class CustomColourSchemeSample extends
                                      Application
{
  private static final Logger LOGGER          = LoggerFactory.getLogger(CustomColourSchemeSample.class);
  private static final int    N_SAMPLES       = 2000;                                                   // default
                                                                                                        // number of
                                                                                                        // data
                                                                                                        // points
  ...
  public void start(final Stage primaryStage)
  {
    ...
    strokeStyleCB.getSelectionModel()
                 .select(DefaultRenderColorScheme.Palette.getValue(DefaultRenderColorScheme.strokeColorProperty()
                                                                                           .get()));
    ...
    fillStyleCB.getSelectionModel()
               .select(DefaultRenderColorScheme.Palette.getValue(DefaultRenderColorScheme.fillColorProperty()
                                                                                         .get()));
    ...
    customFill.setOnAction(evt ->
    {
      ...
      for (Color colour : DefaultRenderColorScheme.fillColorProperty())
      {
        Stop[]         stops    = new Stop[]
        { new Stop(0,
                   colour.brighter().interpolate(Color.TRANSPARENT, 0.4)),         // NOPMD
          new Stop(1,
                   colour.brighter().interpolate(Color.TRANSPARENT, 0.95)) };      // NOPMD
