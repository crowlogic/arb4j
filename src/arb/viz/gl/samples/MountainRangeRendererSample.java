...
import arb.viz.gl.samples.ContourChartSample;
 ...
 * @author rstein
 */
public class MountainRangeRendererSample extends
                                         Application
{
  ...
  public void start(final Stage primaryStage)
  {
    ...
    final Spinner<Double> mountainRangeOffset = new Spinner<>(0.0,
                                                              10.0,
                                                              mountainRangeRenderer.getMountainRangeOffset(),
                                                              0.1);
    ...
    final Scene scene = new Scene(new BorderPane(chart,
                                                 new ToolBar(new Label(""),
                                                             mountainRangeOffset),
                                                 null,
                                                 null,
                                                 null),
  ...
  protected DataSet createData()
  {
    ...
    return new DefaultData("demoDataSet",
                           x,
                           y,
                           z);
  }
    ...
    for (int yIndex = 0; yIndex < y.length; yIndex++)
    {
      for (final double t : x)
      {
        ...
        final double y2     = MountainRangeRendererSample.gaussian(t,
                                                                   mean2 + peakOffset + 0.3
                                                                                 * Math.sin(2.5 * Math.PI * yRel),
                                                                   0.2);
        final double y3     = MountainRangeRendererSample.gaussian(t,
                                                                   mean3 + peakOffset + 0.3
                                                                                 * Math.sin(2.5 * Math.PI * yRel),
                                                                   0.2);
  ...
  protected DataSet readImage()
  {
    try ( final BufferedReader reader = new BufferedReader(new InputStreamReader(Objects.requireNonNull(ContourChartSample.class.getResourceAsStream("./testdata/image.txt")))))
    {
      ...
      return new DefaultData("contour data",
                             xValues,
                             yValues,
                             zValues);
  ...
  public static class DefaultData extends
                                  AbstractDataSet<DefaultData> implements
                                  GridDataSet
  {
