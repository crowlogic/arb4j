...
/**
 * Simple Example for the use of the TransposedDataSet Shows how 1D and 2D
 * dataSets can be transposed and how to reduce the dimensionality of data.
 *
 * @author akrimm
 */
public class TransposedDataSetSample extends
                                     Application
{
  private static final Logger   LOGGER                = LoggerFactory.getLogger(TransposedDataSetSample.class);
  protected static final int    FONT_SIZE             = 20;
  protected static final String FONT_SYMBOL_TRANSPOSE = "fas-sync:" + FONT_SIZE;                               // sync
                                                                                                               // symbol
  protected static final String FONT_SYMBOL_CHECK     = "fa-check-circle:" + FONT_SIZE;                        // check
                                                                                                               // symbol
  private static final int      N_SAMPLES             = 1000;                                                  // default
                                                                                                               // number
                                                                                                               // of
                                                                                                               // data
                                                                                                               // points
  private static final int      N_TURNS               = 20;                                                    // default
                                                                                                               // number
                                                                                                               // of
                                                                                                               // data
                                                                                                               // points
  ...
  public void start(final Stage primaryStage)
  {
    ...
    final DataSet           dataSet1           = test8Function(0,
                                                               4,
                                                               -1,
                                                               3,
                                                               (Math.PI / N_SAMPLES) * 2 * N_TURNS,
                                                               0.2,
                                                               N_SAMPLES);
    ...
    // init ToolBar items to illustrate the two different methods to flip the
    // DataSet
    ...
    final Button bApplyPermutation = new Button(null,
                                                new FontIcon(FONT_SYMBOL_CHECK));
    ...
    cbTransposed.setOnAction(evt ->
    {
      ...
      textPermutation.setText(Arrays.stream(transposedDataSet2.getPermutation())
                                    .boxed()
                                    .map(String::valueOf)
                                    .collect(Collectors.joining(",")));
    });

    // flipping method #2: via 'setPermutation(int[])' - flips arbitrary combination
    // of axes
    ...
    final ToolBar toolBar = new ToolBar(new Label("method #1 - transpose: "),
                                        cbTransposed,
                                        new Separator(),
                                        new Label("method #2 - permutation: "),
                                        textPermutation,
                                        bApplyPermutation);
    final HBox    hBox    = new HBox(chart1,
                                     chart2);

    VBox.setVgrow(hBox, Priority.ALWAYS);
    final Scene scene = new Scene(new VBox(toolBar,
                                           hBox),
                                  800,
                                  600);
  ...
  private static DataSet createTestData()
  {
    ...
    for (int yIndex = 0; yIndex < y.length; yIndex++)
    {
      for (int xIndex = 0; xIndex < x.length; xIndex++)
      {
        ...
        // }
        z[yIndex][xIndex] = (x[xIndex]) * (y[yIndex] - 3) * Math.sin(5.0 * Math.PI * f * x[xIndex])
  ...
  public static DataSet test8Function(final double xMin,
                                      final double xMax,
                                      final double yMin,
                                      final double yMax,
                                      final double omega,
                                      ...
                                      final int nSamples)
  {
    ...
    {
      x[i] = xMin + (a * (Math.sin(i * omega) + 1)) + (pert * Math.sin(i * omega * 0.95) * Math.cos(i * omega * 2));
      y[i] = yMin + (b * (Math.sin(i * omega * 2) + 1)) + (pert * Math.sin(i * omega * 0.777) * Math.cos(i * omega));
    }
    final DataSet dataSet = new DataSetBuilder().setName("non-sorted 2D DataSet")
                                                .setValues(DIM_X, x)
                                                .setValues(DIM_Y, y)
                                                .build();
