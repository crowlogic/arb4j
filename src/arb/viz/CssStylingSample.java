 ...
 * @author akrimm
 */
public class CssStylingSample extends
                              Application
{
  private static final Logger                LOGGER    = LoggerFactory.getLogger(CssStylingSample.class);
  private static final int                   N_SAMPLES = 100;                                                // default
                                                                                                             // number
                                                                                                             // of
                                                                                                             // data
                                                                                                             // points
  public static final ObservableList<String> CSS_LIST  = FXCollections.observableArrayList("none",
                                                                                           "CustomCss1.css",
                                                                                           "CustomCss2.css");
  ...
  public void start(final Stage primaryStage)
  {
    ...
    final XYChart chart = new XYChart(new DefaultNumericAxis(),
                                      yAxis);
    chart.getPlugins().addAll(new Zoomer(), new CrosshairIndicator(), new EditAxis()); // standard plugin, useful for
                                                                                       // most cases

    chart.getDatasets()
         .addAll(new GaussFunction("Gauss",
                                   N_SAMPLES),
                 new CosineFunction("Cosine",
                                    N_SAMPLES)); // for two data sets
    ...
    final XYChart chartRight = new XYChart(new DefaultNumericAxis(),
                                           yAxisRight);
    chartRight.getPlugins().addAll(new Zoomer(), new DataPointTooltip(), new EditAxis()); // standard plugin, useful
                                                                                          // for most cases

    chartRight.getDatasets()
              .addAll(new RandomWalkFunction("RandomWalk",
                                             N_SAMPLES));
    ...
    final HBox hBox = new HBox( //
                               new VBox(new HBox(new Label("Stylesheet for left HBox: "),
                                                 cssBox),
                                        chart), //
                               new VBox(new HBox(new Label("Stylesheet for right HBox: "),
                                                 cssBoxRight),
                                        chartRight) //
    );
    VBox.setVgrow(hBox, Priority.ALWAYS);
    hBox.getChildren().forEach(child -> HBox.setHgrow(child, Priority.ALWAYS));
    final Scene scene = new Scene(new VBox(new HBox(new Label("Stylesheet for Scene: "),
                                                    globalCssBox),
                                           hBox),
                                  800,
                                  600);
      ...
      else
      {
        scene.getStylesheets()
             .setAll(Objects.requireNonNull(CssStylingSample.class.getResource(newVal),
                                            "could not load css file: " + newVal)
                            .toExternalForm());
      }
    });
