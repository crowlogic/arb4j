                                           ...
                                           AutoCloseable
{
  ...
  /**
   * Calculate the i-th value of step-length {@link #dt()} where
   * i={@link DiffusionProcessState#index()} by calling
   * {@link DiffusionProcess#μ()} and {@link DiffusionProcess#σ()} and scaling by
   * {@link #dt} accordingly. Does *not change the state*, this is done by
   * this{@link #jump(DiffusionProcessState, int, EvaluationSequence)}
   * 
   * @param state   the {@link State} of the {@link DiffusionProcess}
   * @param prec    bits of accuracy
   * @param evalSeq the {@link EvaluationSequence} which will have its i-th value
   *                populated
