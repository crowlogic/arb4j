 ...
 *
 * <p>
 * The exponent {@code Φ_M} of the (rough-Heston) characteristic function is
 * split, by Euclidean division (§3.1), into a Gaussian part and a proper
 * rational part {@code ρ}:
 *
 * <pre>
 ...
 * the single Cayley pole, all chosen by us, none from the exponent.</li>
 * </ul>
 *
 * <p>
                             ...
                             AutoCloseable
{
  ...
  public Real price(int bits, Real res)
  {
          ...
          Real  target   = new Real())   // 2^{−bits}
    {
      ...
      // Highest term index we will ever need (safety cap; the majorant exits
      // long before this for any admissible exponent).
      final int maxN = (p == 0) ? 0 : 4 * bits + 64;

      // ── erfc/Hermite node atoms (§4) ────────────────────────────────────
      // A_1(0) = E(0), A_1(1) = E(1), and A_m(q) for m = 1..maxN+1.
      try ( Real E0 = new Real();
            Real E1 = new Real();
            Real Aq = (maxN >= 1) ? Real.newVector(maxN + 1) : Real.newVector(1))
      {
        Atoms atoms = new Atoms(σT2, σ, ξ, c, bits, maxN + 1);
        ...
        try ( Complex qn = Complex.newVector(maxN + 1))
        {
          exponentialCoefficients(qn, bits, maxN);

          // ── a-priori majorant constants (§7.2) ─────────────────────────
          try ( Real G01   = new Real();   // G_0 + G_1
                ...
                Real τ      = new Real())
          {
            ...
            else
            {
              ...
              tauC.set(discount);
              tauC.mul(G01, bits, tauC).mul(normPsi, bits, tauC);
            ...
            // ── the single series (eq 6.5) with verified-tail exit ─────────
                  ...
                  Real    Rpow = new Real())
            {
              ...
              // C_M = discount·acc, taken real (the price is real; conjugate
              // pole pairs cancel the imaginary part). Widen by τ.
  ...
  /** Widen the ball {@code res} so its radius includes {@code τ}. */
