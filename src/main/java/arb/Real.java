                  ...
                  IntFunction<Real>
{
  ...
  /**
   * Calls {@link Real#random(RandomState, int)} on each element of this set of
   * Real numbers via a parallel stream
   * 
   * @param pdf
   * @param randomState
   * @param prec
   */
  public void
         randomlyGenerate(ProbabilityDistributionFunction pdf, RandomState randomState, int prec)
  {
  ...
  {
    assert dim == that.dim : "dimensions must match, this.dim="
                             + dim
                             + " != that.dim = "
                             + that.dim;
  ...
  public RealPolynomial linearlyRegress(Real y, int bits, RealPolynomial result)
  {
    ...
    try ( Real sumX = x.sum(bits, new Real()); Real sumY = y.sum(bits, new Real());
          ...
          Real sumXX = x.dotProduct(x, bits, new Real()); Real pivot = new Real();)
    {
