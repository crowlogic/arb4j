                  ...
                  Assignable<Real>
{
  ...
  private static final ThreadLocal<Real[]> autocorrelationScratch = ThreadLocal.withInitial(() -> new Real[]
  { new Real(), new Real() });
  ...
  public void returnVariable(Real object)
  {
    synchronized (this)
    {
      assert object.home == this : String.format("%s is owned by %s not %s", object, object.home, this);
   ...
   */
  public Real logΓ(int bits, Real value)
  {
  ...
  public static Predicate<Real> isNegativeIntegerOrZero = isNegativeInteger.or(α -> α.isInteger() && (α.isNegative() || α.isZero()));
  ...
  public ComplexRationalFunction mul(ComplexRationalFunction that, int prec, ComplexRationalFunction result)
  {
  ...
  {
    assert dim == fraction.dim : String.format("this,dim = %d != fraction.dim = %d", this, fraction);
    if (dim == 1)
    {
  ...
  public Real gammaVariance(int n, int prec, Real result)
  {
    ...
    try ( Real unshiftedSlice = slice(0, dim - n); Real shiftedSlice = shift(n); Real y = unshiftedSlice.sub(shiftedSlice, prec, Real.newVector(dim - n)))
    {
  ...
  /**
   * Empirical autocorrelation ρ(k) = (1/(N−k))·Σᵢ Z[i]·Z[i+k] / ⟨Z²⟩ for lags k =
   * 0..maxLagSteps−1, with ρ(0) defined as 1. Direct arb-typed port of
   * Statistics.autocorr(double[], int).
   *
   * <p>
   * This is the variance-normalized autocorrelation: every value is divided by
   * ⟨Z²⟩ and ρ(0) is therefore unity by construction. The actual second-order
   * statistic — the autocovariance, whose lag-0 value is ⟨Z²⟩ itself and which
   * preserves the amplitude of the underlying signal — is provided by
   * {@link #autocovariance(int, int, Real)}.
   */
  ...
  /**
   * Raw empirical autocovariance C(k) = (1/(N−k))·Σᵢ Z[i]·Z[i+k] for lags k =
   * 0..maxLagSteps−1, computed entirely in arb-typed arithmetic at the requested
   * precision. The mean is <em>not</em> subtracted; the lag-0 value C(0) = ⟨Z²⟩
   * is the uncentered second moment, retaining the amplitude of the underlying
   * signal — no centering, no division by variance, no rescaling to a unit peak.
   * Equivalent to {@link #autocovariance(int, int, boolean, Real)} with
   * {@code subtractMean = false}.
   *
   * <p>
   * If a caller wants the centered statistic (1/(N−k))·Σᵢ (Z[i]−Z̄)·(Z[i+k]−Z̄)
   * they can either subtract the mean from the input vector beforehand, or invoke
   * the four-argument overload with {@code subtractMean = true}.
   *
   ...
   *
   * <p>
   * If {@code subtractMean} is {@code false} the raw uncentered statistic \[ C(k)
   * = \frac{1}{N-k} \sum_{i=0}^{N-k-1} Z_i \, Z_{i+k} \] is returned, so C(0) =
   * ⟨Z²⟩.
   *
   * <p>
   * If {@code subtractMean} is {@code true} the centered statistic \[ C(k) =
   * \frac{1}{N-k} \sum_{i=0}^{N-k-1} (Z_i - \bar Z)(Z_{i+k} - \bar Z) \] is
   * returned, so C(0) is the sample variance.
   *
   * <p>
   * The inner per-lag accumulation reuses the same per-thread scratch pair as
   * {@link #autocorrelation(int, int, Real)}, so an O(N²) lag sweep performs no
   * Real allocation beyond the output vector and the already-cached two-Real
   * scratch per worker. The k = 1..maxLagSteps−1 lags run in a parallel stream;
   * lag 0 is computed serially and written into result.get(0). When
   * {@code subtractMean = true} the mean Z̄ is computed once serially up front
   * and captured by the parallel workers via a final local; the per-lag inner
   * loop then fuses the centering with the product accumulation so still no extra
   * allocation occurs inside the parallel section.
   *
   * @param maxLagSteps  total lag count, including lag 0
   * @param bits         arithmetic precision
   * @param subtractMean if {@code true}, subtract Z̄ from each sample before
   *                     forming products
   * @param result       length-maxLagSteps Real vector that receives C(k)
   * @return result
   */
  ...
  public boolean assertContiguity()
  {
    ...
    if (elements != null)
    {
      ...
      {
        assert false : String.format("swigCPtr = %d != element[0].swigCptr=%s", swigCPtr, get(0).swigCPtr);
        return false;
      }
    }
    for (int i = 1; i < dim; i++)
  ...
  public Real unlock()
  {
    int status = arblibJNI.mprotect(swigCPtr, BYTES * dim, Protections.PROT_READ.bitfield | Protections.PROT_WRITE.bitfield);
    assert status == 0 : "mprotect call failed. errno=" + arblib.errorNumber();
    locked = false;
    return this;
  }
  ...
  {
    return StreamSupport.stream(Spliterators.spliterator(iterator(), dim, Spliterator.SIZED | Spliterator.ORDERED), false);
  }
   ...
   * references rather than setting the value to the Real currently referenced at
   * the specified index
   */
  ...
  /**
   * Cached element-array view of the radii of this {@link Real} vector. For a
   * scalar (dim == 1) {@link #rad()} returns {@link #getRad()} directly and this
   * field is never used.
   */
  Magnitude  radView;

  /**
   * Strided {@link Magnitude} view over the radii of this {@link Real} vector.
   * For dim == 1 returns the cell's own {@link #getRad()}. For dim &gt; 1 returns
   * an element-array {@link Magnitude} whose {@code get(i)} aliases
   * {@code this.get(i).getRad()}; no copy. Lazily constructed and cached.
   *
   * <p>
   * Contiguity in arb storage is irrelevant: no FLINT vector op is called on
   * {@link Magnitude} vectors in arb4j, so the element-array view is sufficient.
   */
  ...
  public String toString(int digits, boolean precise)
  {
    ...
    {
      var str = arblib.arb_get_str(this, digits, (printPrecision || precise) ? 0 : 2).replace("[", "").replace("]", "");
      if (!str.contains("+/-"))
      {
  ...
  public RealPolynomial linearlyRegress(Real y, int bits, RealPolynomial result)
  {
    ...
    try ( Real sumX = x.sum(bits, borrowVariable()); Real sumY = y.sum(bits, borrowVariable()); Real sumXY = x.dotProduct(y, bits, borrowVariable());
          Real sumXX = x.dotProduct(x, bits, borrowVariable()); Real pivot = borrowVariable();)
    {
