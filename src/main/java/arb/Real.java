                  ...
                  Assignable<Real>
{
  ...
  public Real set(RationalFunction val)
  {
    if (val.numerator.getLength() > 1 || val.denominator.getLength() > 1)
    {
      throw new ArbException(String.format("Cannot assign %s to a Real value since its degree is greater than 1\n",
                                           val));
    }
    try ( Fraction tmp = new Fraction())
    {
      tmp.getNumerator().set(val.getNumerator().get(0));
      tmp.getDenominator().set(val.getDenominator().get(0));
      return set(tmp);
    }
  }
