...
public class A implements ComplexPolynomialSequence, Typesettable, AutoCloseable, Initializable, Named {
   ...
   @Override
   public ComplexPolynomial evaluate(Integer in, int order, int bits, ComplexPolynomial result) {
      if (result == null) {
      ...
      if (order <= 1) {
         ComplexPolynomial var11 = (ComplexPolynomial)Function.peek(this.cache, in);
         if (var11 != null) {
            result.set(var11);
            return result;
         } else {
