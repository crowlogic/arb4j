                           ...
                           Becomable<ComplexMatrix>
{
  ...
  private void initRows()
  {
    ...
    for (int i = 0; i < getNumRows(); i++)
    {
      if (rows[i] == null)
      {
        rows[i] = new Complex(rowPointers.get(i),
                              false);
      }
      else
      {
  ...
  {
    assert getNumCols()
                  == that.getNumRows() : String.format("this matrix col count %d does not match right matrix row count %d",
                                                       getNumCols(),
                                                       that.getNumRows());
    assert getNumRows()
                  == result.getNumRows() : String.format("Result matrix row count %d does not match left matrix row count %d",
                                                         result.getNumRows(),
                                                         getNumRows());
    assert that.getNumCols()
                  == result.getNumCols() : String.format("Right matrix col count %d does not match result matrix col count %d",
                                                         that.getNumCols(),
                                                         result.getNumCols());
  ...
  public ComplexMatrix
         computeLowerUpperFactorization(LongBuffer permutation, int bits, ComplexMatrix result)
  {
  ...
  public ComplexMatrix()
  {
    this(arblibJNI.new_ComplexMatrix(),
         true);
  }
