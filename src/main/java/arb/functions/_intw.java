...
public class _intw implements RealFunction, Typesettable, AutoCloseable, Initializable, Named {
   ...
   public Real evaluate(Real x, int order, int bits, Real result) {
      if (order > 1) {
         ...
         return this.cℤ0000
            .sub(x, bits, this.vℝ0076)
            .pow(this.α.add(this.cℤ0000, bits, this.vℝ0077), bits, this.vℝ0078)
            .div(this.vℝ0077, bits, this.vℝ0079)
            .mul(
               this.cℤ0000.add(x, bits, this.vℝ0080).pow(this.β.add(this.cℤ0000, bits, this.vℝ0081), bits, this.vℝ0082).div(this.vℝ0081, bits, this.vℝ0083),
               bits,
               result
            );
      }
   ...
   public RealFunction derivative() {
      return Function.express(Real.class, Real.class, RealFunction.class, "_diff_intw", "diff((((1-x)^(α+1))/(α+1))*(((1+x)^(β+1))/(β+1)),x)", this.context);
   }

   @Override
   public RealFunction integral() {
      return Function.express(Real.class, Real.class, RealFunction.class, "_int_intw", "int((((1-x)^(α+1))/(α+1))*(((1+x)^(β+1))/(β+1)),x)", this.context);
   }
