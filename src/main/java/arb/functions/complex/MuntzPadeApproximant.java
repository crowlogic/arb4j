                                         ...
                                         AutoCloseable
{
  ...
  public Complex evaluate(Complex t, int order, int bits, Complex result)
  {
          ...
          Integer M         = new Integer() )
    {
      ...
      // because the OPS h(j) loses precision at some M*, so the neighbour-diff
      // floors above 2^-bits and then climbs as precision is lost (eventually
      // α=σ/h would divide by an arb-zero h(j)). The diff descends monotonically
      // to that floor then stops shrinking, so one exit below always fires at
      // finite M and the returned ball encloses the limit to the achieved
      // precision.
      ...
      for (int m = 3;; m++)
      {
        ...
        {
          // Diff stopped shrinking: the precision floor is reached. The best
