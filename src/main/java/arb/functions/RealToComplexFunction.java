                                       ...
                                       Function<Real, Complex>
{
          ...
          Magnitude tmpm = new Magnitude(); Magnitude tmpn = new Magnitude(); Magnitude newTol = new Magnitude();)
    {
            ...
            Magnitude ms = Magnitude.newVector(2 * allocation, "ms");)
      {
        ...
        while (depth >= 1)
        {
          ...
          {
            glStatus = calculateGaussLegendreIntegral(as.get(top),
                                                      bs.get(top),
                                                      newTol,
                                                      degLimit,
                                                      verbose,
            ...
            if (glStatus)
            {
              ...
              Integrators.accumulateGaussLegendreQuadrature(relAccuracyGoalBits,
                                                            prec,
                                                            s,
                                                            u2,
                                                            tmpm,
  ...
  public default Complex calculateSimpleQuadrature(Real a, Real b, int prec, Complex res, boolean verbose)
  {
          ...
          Real widePoint = new Real();)
    {
      ...
      evaluate(widePoint, 0, prec, res);
      res.mul(δ, prec, res).mul2e(-1, res);
      if (verbose)
      {
