...
//var fourierF = BandLimitation.cesaroFourier();   // throws: development driver
  ...
  public static final RealFunction                        d2Θ;
  public static final RealFunction                        g;
  ...
  /** C₀ = (γ + 3·log(2) + π/2 + log(π))/2 ≈ 2.6860917… (the threshold). */
  public static RealNullaryFunction C0()
  {
    return RealNullaryFunction.express("(γ+3*log(2)+π/2+log(π))/2");
  }

  // ── Riemann–Siegel data ──────────────────────────────────────────────────

  /** N(t) := ⌊√(t/(2π))⌋. */
  public static RealFunction N()
  {
    return RealFunction.express("N:t➔⌊√(t/(2*π))⌋");
  }

  /** tₙ := 2π·n². */
  public static RealFunction tn()
  {
    return RealFunction.express("tn:n➔2*π*n^2");
  }

  /** uₙ := Θ(2π·n²). */
  public static RealFunction un()
  {
    return RealFunction.express("un:n➔Θ(2*π*n^2)", ctx);
  }

  // ── Warp and pullback ────────────────────────────────────────────────────

  /** Θ(t) = ϑ(t) + C·t (with C the slope of the registered Θ object). */
  public static RealFunction theta()
  {
    return RealFunction.express("Θ:t➔Θ(t)", ctx);
  }

  /** Stationary pullback Z̃(u) := Z(g(u)). */
  public static RealFunction Ztilde()
  {
  ...
  /** Φₙ(u) := ϑ(g(u)) − g(u)·log(n). */
  public static RealFunction Φn()
  {
    return RealFunction.express("Φ:(n,u)➔ϑ(g(u))-g(u)*log(n)", ctx);
  }

  /** ωₙ(u) := (ϑ′(g(u)) − log n) / Θ′(g(u)). */
  public static RealFunction ωn()
  {
    return RealFunction.express("ω:(n,u)➔(diff(ϑ(g(u)),u)-log(n))/dΘ(g(u))", ctx);
  }

  /** ωₙ′(u) := C·ϑ″(g(u)) / Θ′(g(u))³. */
  public static RealFunction dωn()
  {
    return RealFunction.express("dω:(n,u)➔c*d2Θ(g(u))/dΘ(g(u))^3", ctx);
  }

  /** Stationary point uₙ★ = Θ(2π·n²·exp(2ξC/(1−ξ))). */
  public static RealFunction ustar()
  {
    return RealFunction.express("ustar:(n,ξ)➔Θ(2*π*n^2*exp(2*ξ*c/(1-ξ)))", ctx);
  }

  // ── Integrals (development drivers — will throw until support lands) ─────
