               ...
               Function<Integer, RealPolynomial>
{
  ...
  public RealPolynomial evaluate(Integer in, int order, int bits, RealPolynomial result)
  {
    ...
    case 1 -> (C.evaluate(ℝ1.set(c2), order, bits, ℝ2)).mul(result.identity(), bits, r̅1)
                                                       .sub(β, bits, r̅2)
                                                       .add(α, bits, r̅3)
                                                       .div(c3, bits, result);
    default -> (A.evaluate(in, order, bits, r̅4)).mul(P.evaluate(in.sub(c2, bits, ℤ1), order, bits, r̅5), bits, r̅6)
                                                 .sub((B.evaluate(ℝ3.set(in),
                                                                  order,
                                                                  bits,
                                                                  ℝ4)).mul(P.evaluate(in.sub(c3, bits, ℤ2),
                                                                                      order,
                                                                                      bits,
                                                                                      r̅7),
                                                                           bits,
