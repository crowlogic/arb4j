...
public class μfunc implements RealSequence, Typesettable, AutoCloseable, Initializable, Named {
   ...
   public RealSequence derivative() {
      return Function.express(
         Integer.class, Real.class, RealSequence.class, "_diffμfunc", "diff(Σk➔(((ω(j)(k))*Γ(k+1))/Γ((k+1)-γ))*(χ(i)(k-γ)){k=ceil(γ)…j},j)", this.context
      );
   }
   ...
   public RealSequence integral() {
      return Function.express(
         Integer.class, Real.class, RealSequence.class, "_intμfunc", "int(Σk➔(((ω(j)(k))*Γ(k+1))/Γ((k+1)-γ))*(χ(i)(k-γ)){k=ceil(γ)…j},j)", this.context
      );
   }
