...
public class A implements RealPolynomialSequence, Typesettable, AutoCloseable, Initializable, Named {
   ...
   public RealPolynomial evaluate(Integer n, int order, int bits, RealPolynomial result) {
      if (order > 1) {
         ...
         return ((Real)this.F.evaluate(n, order, bits, this.vℝ0009))
            .mul(this.vXℝ0001.identity(), bits, this.vXℝ0002)
            .add((Real)this.G.evaluate(null, order, bits, this.vℝ0010), bits, this.vXℝ0003)
            .mul(((Real)this.C.evaluate(this.vℝ0011.set(n), order, bits, this.vℝ0012)).sub(this.cℤ0000, bits, this.vℝ0013), bits, this.vXℝ0004)
   ...
   public RealPolynomialSequence derivative() {
      return Function.express(Integer.class, RealPolynomial.class, RealPolynomialSequence.class, "_diffA", "diff((((F(n)*x)+G())*(C(n)-1))/2,n)", this.context);
   }

   @Override
   public RealPolynomialSequence integral() {
      return Function.express(Integer.class, RealPolynomial.class, RealPolynomialSequence.class, "_intA", "int((((F(n)*x)+G())*(C(n)-1))/2,n)", this.context);
   }
