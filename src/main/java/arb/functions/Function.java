                         ...
                         Named
{
   ...
   * to (1) skip if {@code alreadyInvalidated} already contains {@code this},
   * (2) add {@code this}, (3) reset their own {@code staticPrecision} sentinel,
   * and (4) recurse into each referenced nested function field passing the same
   * set. The mutually-recursive Müntz {@code a ↔ S} cluster (and any other
   * cycle in the function graph) terminates the traversal correctly because
   * the set membership check is identity-based.
   *
