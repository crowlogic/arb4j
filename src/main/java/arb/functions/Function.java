                         ...
                         Named
{
   ...
   * Replaces the previous {@link ArrayList}-backed cache, which silently
   * returned {@code null} for negative indices and triggered unbounded
   * recompute on recursive sequences whose recurrence reaches negative
   * arguments. {@code HashMap} permits arbitrary integer keys, so every
   * {@code peek} that finds a stored value short-circuits the recursion.
   *
   ...
   */
  static <C> C peek(TreeMap<java.lang.Integer, C> cache, Integer n)
  {
    return cache.get(n.getSignedValue());
  }

  /**
   * Memoization poke over a {@link TreeMap} keyed by signed {@code int}
   * index. See {@link #peek(TreeMap, Integer)} for the rationale.
   *
   * @return the inserted value (for chaining)
   */
  static <C> C poke(TreeMap<java.lang.Integer, C> cache, Integer n, C value)
  {
    cache.put(n.getSignedValue(), value);
    return value;
  }

  /**
   * Memoization peek by primitive {@code int}. Used by generated bytecode
   * which has already extracted the signed value from the input parameter.
   */
  static <C> C peek(TreeMap<java.lang.Integer, C> cache, int n)
  {
    return cache.get(n);
  }
   ...
   */
  static <C> C poke(TreeMap<java.lang.Integer, C> cache, int n, C value)
  {
    cache.put(n, value);
    return value;
  }
