...
public class RationalFunction implements RealFunction,NamedField<RationalFunction>,Verifiable,AutoCloseableAssignable<RationalFunction>,ReifiedFunction {
 ...
 public RationalFunction div(Fraction operand, int prec, RationalFunction result)
  ...
  {
    assertPointerConsistency();
    // Numerator = integer (full fmpz precision via fmpz_poly_set_fmpz),
    // denominator = 1. Previously this routed through set(integer.getSignedValue())
    // which silently truncates any fmpz that doesn't fit in a Java signed
    // long — a lossy conversion for a class that otherwise preserves
    // arbitrary precision.
