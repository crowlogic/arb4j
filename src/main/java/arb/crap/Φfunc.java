...
public class Φfunc implements ComplexFunction, Typesettable, AutoCloseable, Initializable, Named {
   ...
   public Complex evaluate(Complex z, int order, int bits, Complex result) {
      if (this.evaluating) {
         ...
         Object var10000;
         try {
            var10000 = this.evaluate_body(z, order, bits, result);
         } finally {
   ...
   private Object evaluate_body(Complex z, int order, int bits, Complex result) {
      ...
      if (order <= 1) {
         return ((Complex)((ComplexPolynomial)this.Φnum.evaluate(this.M, order, bits, this.vXℂ0032)).evaluate(z, order, bits, this.vℂ0004))
