...
the covariance is representable as (the Lebesgue integral) 
                                    ...
                                    r(s, t) = ∫ℝ exp(ⅈ*(s-t)*λ)*aₛ(λ)*āₜ(λ)dF(λ)
...
is assumed, it is not really different since the positive definiteness of F implies that N(s) = cM(s), for some...  c ≥ 0, 
s ∈ ℝ, so that M = N can be taken as a general form.] This class has some nice structural properties of which 
...
r(s, t) = ∫ℝ ∫ℝ eⁱˢλ⁻ⁱᵗμ M(dλ)M(dμ)... = ∫ℝ eⁱˢλ M(dλ) · ( ∫ℝ eⁱᵗμ M(dμ) ) = f(s)f(t) (= M(s)M(t)⁻¹)
...
(Af)(s) = ∫ℝ ṙ(s, t) f(t) dt... is completely continuous on the standard Lebesgue space L²(ℝ), and has a complete set of (orthonormal) 
eigenfunctions corresponding to its eigenvalues λᵢ ≥ 0 (cf. [12], p. 243) so that Aφᵢ = λᵢφᵢ. We then have the following representation:
...
ṙ(s, t) = ∑₍ᵢ₌₁⁾ ∞ λᵢ φᵢ(s) φᵢ(t) ∀ λᵢ ≥ 0... the series converging in mean (and uniformly also). If we take F(⋅) to be the function which 
increases by jumps of size λᵢ at i ≥ 0 and vanishing on the negative line, and aₛ(v) = φᵢ(s) for v = i, and = 0 elsewhere in (4), without 
the exponential term, then (15) can be expressed as
...
where Z(⋅) is a stochastic measure with orthogonal values in L₀²(P), and E(Z(A)Z(B)) = F(A ∩ B) (cf. [5], p. 59; or [2], p. 55). 
If now we set Z({i}) = ξᵢ √(λᵢ) where ξₙ is given by (13), then (17) becomes

Ẋₜ = ∑ₙ₌₁ ∞ √(λₙ) φₙ(t) ξₙ

the series converging in ℋ(X). Since Xₜ = Ẋₜ (1 + t²)¹/², this gives (14). ■
... Remark: If T is a compact set, then no factor such as [(1 + s²) (1 + t²)]⁻¹/² is needed in the above development, 
since the classical Mercer theorem applies and yields the expansion (15) for r. In the noncompact case, Mercer's theorem is no 
longer available and some such positive factor seems necessary to invoke the Hilbert-Schmidt result.

4. A Moving Average Representation

Another expression used in the stationary case is the moving average representation. Following the classical case (cf. [5], p. 79) 
one terms a Karhunen type process {Xₜ, t ∈ ℝ} ⊆ L₀²(P) to have a moving average representation if it is of the form
Xₜ = ∫ℝ g(t - u) Z(du) ∀ t ∈ ℝ... where Z(⋅) is a stochastic measure and ḡ(t - ⋅) ∈ L²(F) for each t. Here F is the spectral measure 
such that F(A, B) = E(Z(A)Z(B)). If Z(⋅) has orthogonal values in L₀²(P) then this reduces to the classical (stationary) case when F 
is moreover the Lebesgue measure. Here we consider again that Z(⋅) is an arbitrary stochastic measure in L₀²(P), and find conditions 
on g in order that the Xₜ-process is weakly harmonizable.

The following strong conclusion is somewhat unexpected.

Theorem 4. Let {Xₜ, t ∈ ℝ} be a process of the form (18) with g = ḟ, where f ∈ L¹(ℝ) and Z is a stochastic measure with values in
 L₀²(P). Then the process is strongly harmonizable and its spectral measure is absolutely continuous relative to the planar Lebesgue 
 measure.
... Proof: Since g = ḟ in (18), and hence g is bounded, it follows that the integral is well defined as a Dunford-Schwartz integral, 
and some of it properties will be used. Thus, since a form of the Fubini theorem is available here (cf. [11], p. 317 for detailed 
reasons), one has
...
where {Ỹ(λ), λ ∈ ℝ} is a weakly harmonizable process by ([2], Prop. 6.4), and the integral now becomes a Bochner integral.
 Let Z̃ : A ↦ ∫ₐ Ỹ(t) f(t) dt, A ∈ ℬ. Then Z̃ is a stochastic measure on the Borel sets ℬ of ℝ, and one has
 
E(Z̃(A)Z̃(B)) = ∫ₐ ∫₆ E(Ỹ(s)Ỹ(t)) f(s)f(t) dsdt
= ∫ₐ ∫₆ r_Y(s, t) f(s)f(t) dsdt
= ν(A, B) (say)

Here r_Y is the covariance function of the Ỹ-process and is bounded. So ν(⋅, ⋅) has finite Vitali variation, and (19) can be
 written, with the properties of the Bochner integral, as
...
Remarks: 1. It is easily shown that ∥ ∫ℝ Xₜ h(t) dt ∥₂ ≤ K₀ · ∥ h ∥∞, for each h ∈ L¹(ℝ) and this implies the
 (V-boundedness and so equivalently) weak harmonizability of the Xₜ-process (cf. [11], Thm. 4.2). But the above stronger 
 conclusion needs additional work, and the given demonstration seems to be the shortest.

2. If Z(⋅) has orthogonal increments with an absolutely continuous spectral measure (i.e., the stationary case), then one 
can prove a converse of the above result. In any event it is an extension of a classical theorem due to Karhunen (cf. [5], p. 83), 
and is a considerable strengthening of ([1], Thm. 4).

5. Operator Representations
... We have defined the Karhunen class and the oscillatory one in Section 1. Both are of second order. The following elementary 
observation is useful for the ensuing work.

Proposition 5. The class of oscillatory processes {Xₜ, t ∈ T} ⊆ L₀²(P) and the class of Karhunen processes indexed by T where 
T = ℝ or ℤ, coincide.

Setting gₛ(λ) = eⁱˢλ aₛ(λ) in (4), s ∈ T, and λ ∈ T̂ where T̂ = ℝ or (0, 2π] accordingly as T = ℝ or ℤ respectively, in their 
definitions, the result follows immediately.

This simple identification enables an operator theoretical representation of mean continuous Karhunen processes indexed by ℝ or ℤ.
 We present this form and connect it with the (weakly) harmonizable class. This helps in gaining a better insight of these 
 families. We shall deduce it from reformulations of essentially known results. Hereafter "operator" means "linear operator."

Theorem 6. Let {Xₜ, t ∈ T} ⊆ L₀²(P) be a mean continuous process. Then it is representable as
Xₜ = Aₜ Uₜ Y₀, t ∈ T (T = ℝ or ℤ)... for some point Y₀ in ℋ(X) = sp{Xₜ, t ∈ T} ⊆ L₀²(P), where Aₜ is a densely defined closed 
operator in ℋ(X), for each t ∈ T, and {Uₛ, s ∈ T} is a weakly continuous unitary group of operators in ℋ(X) which commutes 
with each Aₜ, t ∈ T. If, moreover, the process is weakly harmonizable, then there is an enlarged Hilbert space 𝒦 ⊇ ℋ(X),
 operators Aₜ : 𝒦 → ℋ(X) and {Uₛ, s ∈ T} a weakly continuous unitary group on 𝒦, Y₀ ∈ 𝒦, such that {Aₜ, t ∈ T} restricted 
 to ℋ(X) is a weakly continuous contractive positive definite family satisfying A₀ = identity on ℋ(X). 
 Here positive definiteness means that for any hᵢ ∈ ℋ(X), tᵢ ∈ T, i = 1, ..., n,
 
 ∑₍ᵢ₌₁⁾ⁿ ∑₍ⱼ₌₁⁾ⁿ (Aₜᵢ₋ₜⱼ hᵢ, hⱼ) ≥ 0 ∀ n ≥ 1.

On the other hand a process defined by (21) is always weakly harmonizable if only {Aₜ, t ∈ T} is a weakly continuous positive
 definite contractive family of operators there, with A₀ = identity, as above.
... Proof: This result is obtained from several known, but deep, facts and we include its essential details for completeness.

Since the process is mean continuous, ℋ(X) is separable and so {Xₜ, t ∈ T} is of Karhunen class relative to a family
 {gₜ, t ∈ T̂ } and a Borel measure μ. By the preceding proposition, replacing gₜ' with eⁱᵗ(⋅) gₜ, we may express this as
  (T̂ denoting the dual group of T):
  
Xₜ = ∫T̂ eⁱᵗλ gₜ(λ) Z(dλ) ∀ t ∈ T

where Z(⋅) is an orthogonally valued σ-additive set function in ℋ(X) from the Borel σ-algebra of T̂. 
Here E(Z(A)Z(B)) = μ(A ∩ B) and ∫T̂ |gₜ(λ)|² μ(dλ) < ∞. But then the Yₜ-process defined by

Yₜ = ∫T̂ eⁱᵗλ Z(dλ) ∀ t ∈ T

is stationary; so, as is well known, there is a weakly continuous group of unitary operators {Uₜ, t ∈ T} on ℋ(X) such 
that Yₜ = Uₜ Y₀. Also by the spectral theorem for this family of operators (cf. [12], p. 281 and p. 383)
Uₜ = ∫T̂ eⁱᵗλ E(dλ) ∀ t ∈ T... where {E(⋅), ℬ} is the resolution of the identity of the {Uₜ, t ∈ T}, with ℬ as the 
Borel σ-algebra of T̂. Thus Z(A) = E(A) Y₀, A ∈ ℬ. Now define

Aₜ = ∫T̂ gₜ(λ) E(dλ)

Since μ(ℬ) = (E(B) Y₀, E(B) Y₀) and ∫T̂ |gₜ(λ)|² μ(dλ) < ∞, it follows from standard results of the functional calculus
 in Hilbert space that Aₜ is a closed densely defined operator in ℋ(X) and its domain contains {Yₛ, s ∈ T} for each t. 
 Since Uₜ and E(B) commute for all t and B, we deduce that Aₜ and E(B), B ∈ ℬ commute. It then follows that Aₜ and 
 {Uₛ, s ∈ T} commute for each t (cf. [12], p. 351) and one has
...
... On the other hand, if Xₜ is given by (21), with Aₜ and {Uₛ, s ∈ T} commuting for each t ∈ T, so that Aₜ and
 {E(B), B ∈ ℬ} also commute, one concludes from a deep theorem of von Neumann and F. Riesz (cf. [12], p. 351 again,
  and the footnote there) that Aₜ is a function φₜ of Uₜ. It follows with the spectral theorem that
...
= ∫T̂ eⁱᵗλ φₜ(λ) E(dλ) Y₀
= ∫T̂ eⁱᵗλ φₜ(λ) Z(dλ).
If we set gₜ'(λ) = eⁱᵗλ φₜ(λ), μ(B) = (E(B) Y₀, E(B) Y₀), then the above stated properties imply that gₜ' ∈ L²(T̂, ℬ, μ),
 so that the process {Xₜ, t ∈ T} is of Karhunen class.
... For the last part since each weakly harmonizable process is of Karhunen class, it admits the representation (21), 
but now with special properties for {Aₜ, t ∈ T}. Let Qₜ = Aₜ Uₜ. Then by a known result (cf. [11], p. 330), {Qₜ, t ∈ T} 
must be positive definite, weakly continuous and contractive. Since {Uₜ, t ∈ T} is a unitary group which is weakly 
continuous, it follows that {Aₜ, t ∈ T} must also be contractive and weakly continuous. To see that it is also positive 
definite, let {h₁, ..., hₙ} ⊆ ℋ(X) be an arbitrary set and t₁, ..., tₙ be any points of the additive group T, n ≥ 1.
 Then letting vᵢ = Uₜᵢ* hᵢ ∈ ℋ(X), we have
...
= ∑₍ᵢ₌₁⁾ⁿ ∑₍ⱼ₌₁⁾ⁿ (Aₜᵢ₋ₜⱼ hᵢ, hⱼ).... This establishes the asserted property of the {Aₜ, t ∈ T}. Note that from the 
fact [11] that Q₀ on ℋ(X) is also identity, we obtain that A₀ U₀ = A₀ is identity as well.

In the opposite direction the result depends on an important theorem of Grothendieck. If Aₜ : 𝒦 → ℋ(X) has the stated 
properties, then Qₜ = Aₜ Uₜ is positive definite and satisfies the hypothesis of the converse part of ([11], p. 330). 
Hence a process defined by (21) is weakly harmonizable. This finishes the essential details of all the assertions. ■
... Remarks: 1. As mentioned before, the first part in an equivalent form is already given in [9], and the last part 
in [11]. Both results obviously depend on some deep facts in abstract analysis. The simple Proposition 5 is used in an 
essential way in obtaining the representation (21) which thus gives an operator representation of Karhunen processes 
indexed by T. The same result extends if T is replaced by a locally compact abelian group when ℋ(X) is assumed 
...
where μₜ(A) = ∫A |aₜ(λ)|² F(dλ) (taking T = ℝ for simplicity). This function, depending on t, is termed an 
"evolving spectrum" in time, by Priestley [10]. Since gₜ(λ) = eⁱᵗλ aₜ(λ) can be oscillatory for each t, the 
process Xₜ which necessarily has the representation (cf. [2], p. 55):
Xₜ = ∫ℝ eⁱᵗλ aₜ(λ) Z(dλ) ∀ t ∈ ℝ... is termed an oscillatory process. In [10], the aₜ-family is normalized so 
that a₀(λ) = 1, all λ, based on some natural reasons motivated by applications. (It was further assumed that aₜ(⋅)
 is the Fourier transform of some Hλ(⋅) of bounded variation such that |Hλ(0)| has a maximum.) The point of (29) is
  that Z(⋅) has orthogonal values and the form suggests a parallel development of the ideas (and interpretations) 
  of the global stationary theory, locally at t if aₜ(⋅) is slowly varying. In any case, for the general 
  representation theory one need not place these restrictions, but the specializations can be useful in
   estimation and inference theory itself, as indicated in [10].

3. As Proposition 5 also implies, the representation (21) of a Karhunen process is not unique. The preceding 
sections show that several other (series type) representations are possible.
 * </pre>
 * 
