...
public record LatexPackage(String name, Map<String, String> options, Map<String, Set<Class<?>>> incompatiblePackages)
{
  ...
  public LatexPackage(String name, String incompatiblePackage, Class<?> classThatRequestsPackage)
  {
    this(name,
         null,
         Map.of(incompatiblePackage, Set.of(classThatRequestsPackage)));
  }
  ...
  public static final List<LatexPackage> cleanup(List<LatexPackage> packages, boolean firstDominates)
  {
    ...
    for (LatexPackage pckg : packages)
    {
      ...
      cleanedPckgs.put(pckg.name(),
                       new LatexPackage(pckg.name(),
                                        o,
                                        ip));
    }
   ...
   *                          specified package
   */
  private static final void fill(Map<String, LatexPackage> cleanedPckgs,
                                 LatexPackage pckg,
                                 BinaryOperator<String> options,
                                 Map<String, Set<Class<?>>> incompatibilities)
  {
    ...
    {
      Set<Class<?>> incompatibleClasses = incompatibilities.computeIfAbsent(ip.getKey(),
                                                                            s -> new HashSet<>(ip.getValue()));
