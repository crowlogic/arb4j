...
public final class PgfPlots implements Texable
{
  ...
  public <T> PgfPlots plot(T input, String legend, Map<String, String> options)
  {
    ...
    {
      // we need to be slightly clever to find out whether the user wants to plot a
      // file or a formula
      // this will not work if the script is just saved for future use without having
      // the files in place
    ...
    if (legend != null)
      add("\\addlegendentry{%s};".formatted(legend));
    return this;
  }
  ...
  public PgfPlots contour(double[] x, double[] y, double[][] z, Map<String, String> options)
  {
    ...
    boolean       hasNumber               = Stream.of(options)
                                                  .anyMatch(s -> s.containsKey("contour filled")
                                                                && s.get("contour filled").contains("{number="));
    ...
    {
      concatenatedUserOptions.append(option.getKey()
                    + (Latex.STRING_IS_NOT_BLANK.test(option.getValue()) ? "=" + option.getValue() : "") + ",");
    }

    add("\\addplot3[surf," + "mesh/rows=" + x.length + "," + "mesh/cols=" + y.length + ","
                  + concatenatedUserOptions.toString() + (!hasNumber ? "contour filled={number=7}," : "")
                  + (!hasSamples ? "samples=150," : "") + (!hasShader ? "shader=interp," : "") + "] table {");
  ...
  public int exec(String filepath)
  {
    ...
    plot.preambleEntries.add(new LatexPreambleEntry("\\pgfplotsset",
                                                    Map.of("cycle list/Dark2-8", "")));
    plot.preambleEntries.add(new LatexPreambleEntry("\\pgfplotsset",
                                                    Map.of("cycle multiindex* list",
                                                           "{mark list*\\nextlist Dark2-8\\nextlist}",
                                                           "colormap/viridis",
                                                           "")));
  ...
  {
    preambleEntries.add(new LatexPreambleEntry("\\usetikzlibrary",
                                               Strings.mapOf(libraries),
                                               false));
    return this;
  }
  ...
  {
    preambleEntries.add(new LatexPreambleEntry("\\usepgflibrary",
                                               Strings.mapOf(libraries),
                                               false));
    return this;
  }
  ...
  {
    preambleEntries.add(new LatexPreambleEntry("\\usepgfplotslibrary",
                                               Strings.mapOf(libraries),
                                               false));
    return this;
  }
  ...
  {
    preambleEntries.add(new LatexPreambleEntry("\\usegdlibrary",
                                               Strings.mapOf(libraries),
                                               false));
    return this;
  }
  ...
  {
    preambleEntries.add(new LatexPreambleEntry("\\pgfplotsset",
                                               Map.of("compat", compat),
                                               false));
    return this;
  }
  ...
  public List<String> latexCode()
  {
    ...
    for (Entry<String, String> option : options.entrySet())
    {
      ret.add(Latex.indent(2) + option.getKey()
                    + (Latex.STRING_IS_NOT_BLANK.test(option.getValue()) ? "=" + option.getValue() : "") + ",");
    }
    ret.add(Latex.indent(1) + "]");
    ret.add("");
