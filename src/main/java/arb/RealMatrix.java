                        ...
                        Becomable<RealMatrix>
{
  ...
  public RealMatrix permute(LongBuffer permutation)
  {
    ...
    for (int i = 0; i < n; i++)
    {
      ...
      assert j >= 0 && j < n : String.format("permutation[%d]=%d out of range, this.numRows=%d",
                                             j,
                                             n);
  ...
  {
    assert getNumRows()
                  == result.getNumRows() : String.format("this.numRows=%d != that.numRows = %d\n",
                                                         getNumRows(),
                                                         result.getNumRows());
    assert getNumCols()
                  == result.getNumCols() : String.format("this.numCols=%d != that.numCols = %d\n",
                                                         getNumCols(),
                                                         result.getNumCols());
  ...
  {
    assert getNumRows()
                  == that.getNumRows() : String.format("this.numRows=%d != that.numRows = %d\n",
                                                       getNumRows(),
                                                       that.getNumRows());
    assert getNumCols()
                  == that.getNumCols() : String.format("this.numCols=%d != that.numCols = %d\n",
                                                       getNumCols(),
                                                       that.getNumCols());
  ...
  public String toString()
  {
    ...
    TextTable             table = new TextTable(
                                                IntStream.rangeClosed(1, getNumCols())
                                                         .mapToObj(func)
  ...
  {
    MemorySegment ms = MemorySegment.ofAddress(getRowPointers())
                                    .reinterpret(Long.BYTES * rows.length);
    rowPointers = ms.asByteBuffer().order(ByteOrder.nativeOrder()).asLongBuffer();
    return this;
  }

  /**
   * Sets the result to the inverse of this (square) matrix, computed by solving
   * the system
   * 
   * X = A^(-1)
   * 
  ...
  {
    assert getNumRows()
                  == transposed.getNumCols() : String.format("this.numRows = %d != transposed.numCols = %d\n",
                                                             this.getNumRows(),
                                                             transposed.getNumCols());
    assert getNumCols()
                  == transposed.getNumRows() : String.format("this.numCols = %d != transposed.numRows = %d\n",
                                                             this.getNumCols(),
  ...
  public RealMatrix swapRows(LongBuffer permutations, int r, int s)
  {
    ...
    assert r >= 0 && r < numRows : "Row index r ("
                                   + r
                                   + ") is out of bounds. Valid range: [0, "
                                   + (numRows - 1)
                                   + "].";
    assert s >= 0 && s < numRows : "Row index s ("
                                   + s
                                   + ") is out of bounds. Valid range: [0, "
                                   + (numRows - 1)
                                   + "].";
    assert permutations == null || permutations.capacity()
                  >= numRows : "Permutations buffer size ("
  ...
  {
    assert getNumCols()
                  == that.getNumRows() : String.format("this matrix col count does %d not match right matrix row count %d",
                                                       getNumCols(),
                                                       that.getNumRows());
    assert getNumRows()
                  == result.getNumRows() : String.format("Result matrix row count does %d not match left matrix row count %d",
                                                         getNumRows(),
                                                         result.getNumRows());
    assert that.getNumCols()
                  == result.getNumCols() : String.format("this matrix col count does %d not match right matrix row count %d",
                                                         that.getNumCols(),
                                                         result.getNumCols());
   ...
   */
  public RealMatrix
         computeLowerUpperFactorization(LongBuffer permutation, int bits, RealMatrix result)
  {
   ...
   */
  public RealMatrix
         computeLowerUpperFactorization(int bits, RealMatrix lowerFactor, RealMatrix upperFactor)
  {
    assert lowerFactor.getNumRows()
                  == upperFactor.getNumRows() : format("lowerFactor.numRows = %d != upperFactor.numRows = %d\n",
                                                       lowerFactor.getNumRows(),
                                                       upperFactor.getNumCols());
    assert lowerFactor.getNumCols()
                  == upperFactor.getNumCols() : format("lowerFactor.numRows = %d != upperFactor.numRows = %d\n",
                                                       lowerFactor.getNumRows(),
                                                       upperFactor.getNumCols());
