                                      ...
                                      AutoCloseable
{
  ...
  /** Edgeworth truncation order. Equals N: Φ_N has degree N in v, so a
   *  cumulant κ_j is only meaningful for j ≤ N. */
  ...
  /** Compiled pricing expression: k (log-moneyness) → call price C. */
  ...
  public RoughHestonOptionPricer(Complex v)
  {
    ...
    // SEED_N retained as a constant but no longer set on φ: the Σ{..∞}
    // form in the new RoughHestonCharacteristicFunction owns its own
    // convergence.  J is the Edgeworth-Hermite truncation, still local.
    // (no-op: SEED_N suppressed)
    this.J              = Integer.named("J").set(SEED_N);
  ...
  public RoughHestonOptionPricer(Context context, Real K, Complex v)
  {
    ...
    // SEED_N suppressed: see no-arg constructor.
    this.J              = Integer.named("J").set(SEED_N);
  ...
  private void compileCumulantAndPriceChain()
  {
    ...
    // Σ{k=0..∞} carries its own Aitken-Δ² convergence in the new arb4j
    // codegen (arb4j commit 70beba6f).  The old explicit-N truncation is gone.
    ComplexPolynomialNullaryFunction Φ  = ComplexPolynomialNullaryFunction.express("Φ", "Σk➔T^(k*μ+1)*d(k){k=0..∞}", pricingContext);
    ComplexPolynomialNullaryFunction dΦ = ComplexPolynomialNullaryFunction.express("dΦ", "Σk➔T^(k*μ+1)*dd(k){k=0..∞}", pricingContext);
  ...
  {
    if (φ.config.context.getVariable("J") == null)
      φ.config.context.registerVariable(J);
  }
  ...
  private Context buildPricingContext()
  {
    ...
    ctx.registerVariable(φ.config.T);
    ctx.registerVariable(φ.config.μ);
    ...
    ctx.registerFunction("dd",
                         ComplexPolynomialSequence.express("dd:k➔v➔when(k=0,w(0)*da(1)(v),else,u(k)*da(k)(v)+w(k)*da(k+1)(v))", φ.config.context));
  ...
  public Real call(Real strike, int bits, Real dst)
  {
    ...
    // CUMULANT_ORDER suppressed on φ: Σ{..∞} owns convergence in the new χ.f.
   ...
   public Real callSensitivityForStrike(Real strike, int bits, Real dst)
  {
    if (kLog == null) kLog = new Real();
    strike.div(S0, bits, kLog).log(bits, kLog);
    // CUMULANT_ORDER suppressed on φ.
    J.set(CUMULANT_ORDER);
   ...
   * φ.riccati / φ.d caches live in φ's own context, which {@code
  ...
  public void seedParameterDerivative(String param, int bits)
  {
    ...
    // re-solve the base fractional Riccati via φ.riccati/φ.d. The
