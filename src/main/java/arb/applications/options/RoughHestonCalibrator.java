 ...
 * arbitrary-precision {@link Real}/{@link RealMatrix} arithmetic throughout —
 ...
 * {@link RoughHestonOptionPricer#callSensitivity} (no finite differences) and
 * is returned. No parameter is ever clamped: a step into an inadmissible region
 * simply fails to price and is rejected, so the damping shrinks the step
 * instead. Iteration is hard-bounded and every step is traced.
 *
                                         ...
                                         AutoCloseable
{
  ...
  private RoughHestonOptionPricer buildPricer(Real T, Real p6)
  {
    ...
    Real strike0 = new Real("1",
                            bits);
    owned.add(strike0);
    return new RoughHestonOptionPricer(ctx,
                                       strike0,
  ...
  /**
   * Price the whole set at full parameters {@code p6}; write model prices into
   * the m×1 {@code model}.
   */
  public void priceAll(Real p6, RealMatrix model)
  {
    try ( Real price = new Real(); Real tmp = new Real())
    {
      for (int i = 0; i < options.size(); i++)
      {
        ...
        pr.φ.config.λ.set(p6.get(LAMBDA));
        pr.φ.config.θ.set(p6.get(THETA));
        pr.φ.config.ν.set(p6.get(NU));
        pr.φ.config.V0.set(p6.get(V0));
        pr.φ.config.ρ.set(p6.get(RHO));
        pr.φ.config.μ.set(p6.get(MU));
        pr.invalidate();
  ...
  /**
   * Price the set; return true iff every price is finite. Exceptions propagate —
  ...
  public Result calibrate()
  {
    ...
    try ( Real F = new Real(); Real Fnew = new Real(); Real μ = new Real(); Real tmp = new Real(); Real gnorm = new Real(); Real dxnorm = new Real();
          Real fac = new Real("3",
                              bits);
          Real gtol = new Real("1e-14",
                               bits);
          Real xtol = new Real("1e-12",
                               bits))
    {
      ...
      for (iter = 0;; iter++)
      {
        jacobian(x, model, J).transpose(Jt);
        Jt.mul(J, bits, JtJ);
        Jt.mul(r, bits, g);
        ...
        if (stepped)
          F.set(Fnew);
        trace.accept("CALIB iter " + iter + ": F=" + sci(F) + "  ‖g‖∞=" + sci(gnorm) + "  μ=" + sci(μ) + "  ‖Δx‖∞=" + sci(dxnorm) + "  x=" + vec(x));
        if (!stepped)
        {
   ...
   * @return
   */
  private RealMatrix jacobian(RealMatrix x, RealMatrix modelAtX, RealMatrix J)
  {
    try ( Real deriv = new Real(); Real p6 = withFree(x);)
    {
      for (int i = 0; i < options.size(); i++)
      {
        RoughHestonOptionPricer pr = pricers[i];
