                                   ...
                                   ClassLoader
{
  ...
  /**
   * Bytecodes waiting to be defined, keyed by the slash-form internal name
   * (e.g. {@code "arb/jacobi/P"}), matching what
   * {@link org.objectweb.asm.ClassWriter#visit ClassWriter.visit} writes
   * into the bytecode. Populated by
   * {@link #registerBytecodes(String, byte[])} and consumed (removed) the
   * first time {@link #findClass(String)} is called for that name — the
   * JVM passes {@link #findClass} the dotted binary form, which is
   * normalized back to slashes here on lookup.
   */
   ...
   * findable via parent delegation. If the class is already on the classpath, the
   * registered bytecodes are never consumed.
   *
   ...
   */
  public void registerBytecodes(String internalName, byte[] bytecodes)
  {
    pendingBytecodes.put(internalName, bytecodes);
  }
  ...
  protected Class<?> findClass(String name) throws ClassNotFoundException
  {
    ...
    // pendingBytecodes is keyed by the slash-form internal name. The JVM
    // passes findClass the dotted binary name, so we normalize on lookup.
