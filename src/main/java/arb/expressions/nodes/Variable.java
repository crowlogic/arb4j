 ...
 */
public class Variable<D, R, F extends Function<? extends D, ? extends R>>
                     extends
                     Node<D, R, F>
{
  ...
  private void generateIndexAccess(MethodVisitor mv)
  {
    ...
    if (Integer.class.equals(indexType))
    {
      mv.visitMethodInsn(INVOKEVIRTUAL,
                         ...
                         "get",
                         Compiler.getMethodDescriptor(reference.type(), indexType),
                         false);
...
//                                  referenceType.descriptorString());
  ...
  public void resolveIndependentVariable(Variable<D, R, F> inputVariable)
  {
    assert (inputVariable == null
                  || inputVariable.equals(expression.independentVariable)) : "inputVariable is already "
                                + inputVariable + " it doesnt make sense to change it to " + this;
  ...
  {
    return !type().isAssignableFrom(Polynomial.class) && !type().isAssignableFrom(QuasiPolynomial.class);
  }
