 ...
 */
public class MultiplicationNode<D, R, F extends Function<? extends D, ? extends R>>
                               extends
                               BinaryOperationNode<D, R, F>
{
  ...
  public Node<D, R, F> simplify()
  {
    ...
    if (left.isNegOne())
    {
      return right.simplify().neg();
    }
    if (right.isNegOne())
    {
      return left.simplify().neg();
    }
    ...
    if (left instanceof LiteralConstantNode<D, R, F> leftConstant && right instanceof LiteralConstantNode<D, R, F> rightConstant)
    {
      ...
      {
        try ( var lint = new Integer(leftConstant.value); var rint = new Integer(rightConstant.value);)
        {
          var product = lint.mul(rint,
                                 0,
                                 rint);
    ...
    if ((left.isVariable() && right.isFunction() && right.asFunction().is("δ")) || (right.isVariable() && left.isFunction() && left.asFunction().is("δ")))
    {
  ...
  {
    return format("\\left(%s \\cdot %s\\right)",
                  left.typeset(),
                  right.typeset());
  }
