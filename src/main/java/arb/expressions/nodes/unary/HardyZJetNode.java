 ...
 */
public class HardyZJetNode<D, C, F extends Function<? extends D, ? extends C>> extends
                          JetNode<D, C, F>
{
  public HardyZJetNode(Expression<D, C, F> expression)
  {
    super("Z",
          expression);
  }

  protected HardyZJetNode(Expression<D, C, F> expression, Node<D, C, F> arg, int coefficientIndex,
  ...
  protected void emitSeriesCall(MethodVisitor mv, Class<?> scalarType, boolean isComplex)
  {
    ...
    invokeStaticMethod(mv,
                       arblib.class,
                       isComplex ? "acb_poly_riemann_siegel_z_series"
  ...
  protected JetNode<D, C, F> newInstance(Expression<D, C, F> expression, Node<D, C, F> arg, int coefficientIndex,
                                         JetState sharedState)
  {
    return new HardyZJetNode<>(expression, arg, coefficientIndex, sharedState);
  }

  @Override
  public <E, S, G extends Function<? extends E, ? extends S>> Node<E, S, G> spliceInto(Expression<E, S, G> newExpression)
  {
    return new HardyZJetNode<>(newExpression, arg.spliceInto(newExpression), coefficientIndex, sharedState);
  }

  @Override
  public String toString()
  {
    return coefficientIndex == 0 ? "Z(" + arg + ")" : "Z^(" + coefficientIndex + ")(" + arg + ')';
  }
