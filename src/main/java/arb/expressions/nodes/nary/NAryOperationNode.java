 ...
 * normal resolution inside that operand expression, not in the nullary parent.
 * </p>
 *
                              ...
                              Node<D, R, F>
{
   ...
   * separate subexpression via spliceInto/compile/registerSubexpression.
   */
                           ...
                           boolean functionForm)
  {
    ...
    // ── Step 2: Clear independent variable so operand parse doesn't bind to it
  ...
  private void compileOperandSubexpression()
  {
    ...
    operandExpression.functionNameSpecified = false;

    // Link back into the tree / share context
    operandExpression.ascendentExpression = expression;
    if (expression.context == null)
    {
      expression.context = new Context();
    }
