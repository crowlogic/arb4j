...
/**
 * Represents a literal constant within an arithmetic expression. This class is used to
 * encapsulate specific constant values, such as mathematical constants (π, ½), within the
 * expression tree of the {@link arb.expressions.Expression} framework. It extends the
 * generic {@link Node} to fit within the structure of an expression tree, allowing
 * arithmetic and symbolic operations to be performed on constants.
 * <p>
 * The {@code LiteralConstant} node dynamically identifies and handles constants defined
 * at runtime and supports the declaration of such constants in generated bytecode. This
 * dynamic nature enables the flexible integration of constants into expressions,
 * supporting operations like addition, multiplication, and notably, integration, in a
 * mathematically sound manner.
 * </p>
 * <p>
 * This class also interacts with the {@link Expression} context to ensure constants are
 * correctly identified, managed, and reused where applicable, enhancing performance and
 * memory efficiency within the evaluation of {@link Expression}s
 * </p>
 *
 * Room for improvement: memoization(caching) of repeated subexpressions within an
 * {@link Expression}
 * 
 * 
 * @param <D> type of {@link Domain} of the function, representing the set of values the
 *            {@link Function} takes as input.
 * @param <R> the type Range type of the function, representing the set of values the
 *            function can output.
 * @param <F> A functional interface extending {@link Function}, indicating the nature of
 *            the operation this node represents within an expression tree.
 * 
                                ...
                                Node<D, R, F>
{
  ...
  public ClassVisitor declareField(ClassVisitor classVisitor)
  {
    ...
    if (Expression.trace)
    {
      logger.debug("declareField: fieldName={} type={} value={}",
                   fieldName,
                   type,
                   toString());
    }
  ...
  public MethodVisitor generateLiteralConstantInitializer(MethodVisitor methodVisitor)
  {
    ...
    duplicateTopOfTheStack(generateNewObjectInstruction(loadThisOntoStack(methodVisitor),
                                                        type));
    ...
    {
      generateConstructor(methodVisitor,
                          type);
    }

    return expression.setThisField(methodVisitor,
                                   fieldName,
                                   type);
  }
