                     ...
                     AutoCloseable
{
  ...
  public void wireNewInstanceIntoPeers(String functionName, Function<?, ?> newInstance)
  {
    ...
    // Alias the instance into its own same-named field when the compiled class
    // declares one (a function reachable from itself through an operand — the
    // indirect-recursion shape). The alias is the same `this.<self> = this`
    // that generateSelfReference emits for the directly-recursive case: a
    // re-entrant evaluate() is served from the (index, order)-keyed cache, so
    // one instance memoises once. Without it, the downward dependency
    // assignment `this.<operand>.<self> = this.<self>` in initialize() would
    // propagate null into the operand.
    java.lang.reflect.Field selfField = findAssignableField(newInstance.getClass(), functionName, newInstance.getClass());
    if (selfField != null)
    {
      try
      {
        if (selfField.get(newInstance) == null)
        {
          selfField.set(newInstance, newInstance);
        }
      }
      catch (IllegalAccessException iae)
      {
        Utensils.wrapOrThrow("failed to alias self field " + functionName + " on " + newInstance.getClass().getName(), iae);
      }
    }
  ...
  /**
   * Static entry point for the wire-all pass, invoked from generated bytecode
   * at the single point where a fresh instance is allocated at runtime — a
   * functional element constructed per evaluate(). It runs the SAME
   * deterministic injection pass that {@link Expression#instantiate} performs
   * for canonical instances ({@link #injectFunctionReferences}), copying every
   * canonical peer instance from the registry into the element's public fields
   * before its initialize() asserts them non-null. This replaces the removed
   * per-name {@code lookupFunctionInstance} fallbacks (issue #1055): wiring is
   * one pass at allocation, never a lazy per-field reach-back.
   */
  public static void wireFunctionReferences(Context context, Function<?, ?> f)
  {
