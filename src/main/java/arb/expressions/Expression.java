 ...
 * <h2>Thesarus</h2>
 * <ul>
 * </li>Input=IndependentVariable</li>
 * </li>InputNode=IndependentVariableNode</li>
 * </ul>
 * 
 * <pre>
 * 
 * 
 * Stackmap frames aren't manually computed because it's not worth the additional complexity. 
 * Using `COMPUTE_FRAMES` do this automatically is the pragmatic choice in most situations, 
 * especially for a tool like this which should never be used inside a tight loop anyway
 * 
 * The additional time taken by `COMPUTE_FRAMES` to automatically compute stack
 * map frames is a negligibly small tradeoff to make for the convenience, reliability, and 
 * maintainability it provides.
 * 
 * The Expression compiler is designed to generate bytecode for
 * individual expressions, which are then evaluated repeatedly. The compilation
 ...
 * 
 * The only caveat to this is IF it could be done easily, reliably and with
 *  minimal amounts of code or additional complexity then it would be worth it
 * </pre>
 * 
 * See Also:
                       ...
                       Consumer<Consumer<Expression<?, ?, ?>>>
{
  ...
  public List<FunctionReference>                             functionsReferences;
  ...
  protected void addChecksForNullAscendentInputFields(MethodVisitor mv)
  {
    ...
    {
      var ascendentIndependentVariableNode = upstreamExpression.independentVariable;
  ...
  public boolean anyAscendentIndependentVariableIsNamed(String name)
  {
    ...
    if (upstreamExpression != null)
    {
      if (upstreamExpression.anyAscendentIndependentVariableIsNamed(name))
      {
        return true;
      }
    }
  ...
  private ClassVisitor declareContextField(ClassVisitor cw)
  {
  ...
  {
    declareIsInitializedField(cw);
    declareContextField(cw);
    if (isRegularScalarFunction())
    {
      declareLiteralConstantFields(cw);
      declareIntermediateVariableFields(cw);
    }
    declareFunctionReferenceFields(cw);
    declareVariableFields(cw);
  }

  protected void declareIsInitializedField(ClassVisitor cw)
  {
    Compiler.declareField(cw, IS_INITIALIZED, boolean.class);
  }

  protected boolean isRegularScalarFunction()
  {
    return !isFunctional();
  }

  protected ClassVisitor declareFunctionReferenceFields(ClassVisitor classVisitor)
  {
    if (context == null)
    {
      return classVisitor;
    ...
    functionsReferences = Utensils.sortFunctionReferences(context.functionReferenceGraph,
                                                          referencedFunctions);

    if (saveGraphs)
    {
      context.saveFunctionReferenceGraph(functionsReferences);
    }
    // Declare functions in dependency order
    for (FunctionReference functionReference : functionsReferences)
    {
      declareFunctionReferenceField(classVisitor, functionReference);
    }

    return classVisitor;
  }

  protected void declareFunctionReferenceField(ClassVisitor classVisitor,
                                               FunctionReference dependency)
  {
  ...
  protected void declareIntermediateVariableFields(ClassVisitor classVisitor)
  {
    sortedIntermediateVariableStream().filter(intermediateVariable ->
    {
      if (isIntermediateVariableAlreadyDeclared(intermediateVariable))
      {
        if (trace)
        {
          logger.warn("Expression#{}: IntermediateVariable {} was already declared. classVisitor={}",
                      System.identityHashCode(this),
    ...
    }).forEach(variable -> declareIntermediateVariableField(classVisitor, variable));
  }

  protected void declareIntermediateVariableField(ClassVisitor classVisitor,
                                                  IntermediateVariable<D, C, F> variable)
  {
  ...
  protected boolean
            isIntermediateVariableYetToBeDeclared(IntermediateVariable<D, C, F> intermediateVariable)
  {
    return !isIntermediateVariableAlreadyDeclared(intermediateVariable);
  }

  protected boolean isIntermediateVariableAlreadyDeclared(IntermediateVariable<D, C, F> variable)
  {
    return declaredIntermediateVariables.contains(variable.name);
  }

  protected ClassVisitor declareLiteralConstantFields(ClassVisitor classVisitor)
  {
  ...
  protected void declareVariableEntry(ClassVisitor classVisitor, Entry<String, Named> variable)
  {
    ...
    if (variable.getValue() != null)
    {
      ...
      Class<?> varType = variable.getValue().getClass();
      Compiler.declareField(classVisitor, varName, varType);
      declaredFields.add(varName);
    }
    else
    {
      if (trace)
   ...
   * own class.
   */
  protected void declareVariableFields(ClassVisitor classVisitor)
  {
    ...
    if (upstreamExpression != null)
    {
      ...
      {
        upstreamIndependentVariableNode.declareField(classVisitor);
      }
    }

    declareReferencedUpstreamInputPropagationFields(classVisitor);

    // Declare fields for all referenced upstream independent variables isitor);

    declareContextVariableFields(classVisitor);

    variablesDeclared = true;

  }

  protected void declareContextVariableFields(ClassVisitor classVisitor)
  {
    // Declare context variables
    if (context != null)
    {
      sortedContextVariableEntryStream().forEachOrdered(variable -> declareVariableEntry(classVisitor,
                                                                                         variable));
    }
  }

  protected Stream<Entry<String, Named>> sortedContextVariableEntryStream()
  {
    return context.variableEntryStream().sorted((a, b) -> a.getKey().compareTo(b.getKey()));
  }

  protected void declareReferencedUpstreamInputPropagationFields(ClassVisitor classVisitor)
  {
    upstreamInputVariableEntryStream().forEach(entry ->
    {
      String                varName = entry.getKey();
  ...
  {
    return referencedVariableEntryStream().filter(this::upstreamInputVariablePredicate);
  }
  ...
  /**
   * FIXME: this needs to use this{@link #acceptUntil(Predicate)}
   * 
   * @param name
   * @return
   */
  ...
  public Expression<D, C, F> generate() throws CompilerException
  {
    ...
    try
    {
      ...
      declareIntermediateVariableFields(classVisitor);
      if (needsCloseMethod())
      {
  ...
  protected void generateFunctionReferences(MethodVisitor mv, FunctionReference functionReference)
  {
    var assignments        = functionReference.getReferencedUpstreamFunctions(className, referencedFunctions);

    var functionName       = functionReference.variableName;
   ...
   * 
   * @param varName
   * @return true if {@link #declaredFields} contains varName
   */
  ...
  protected MethodVisitor generateInitializationCode(MethodVisitor mv)
  {
    ...
    {
      functionsReferences.forEach(functionReference -> generateFunctionReferences(mv,
                                                                                  functionReference));
    }
  ...
  public HashSet<String> declaredFields = new HashSet<>();

  private final boolean upstreamInputVariablePredicate(Entry<String, VariableNode<D, C, F>> entry)
  {
    {
      String                varName = entry.getKey();
      VariableNode<D, C, F> varNode = entry.getValue();
      ...
      if (upstreamExpression != null)
      {
        VariableNode<?, ?, ?> ascendentIndependentVariable = upstreamExpression.independentVariable;
        if (ascendentIndependentVariable != null
  ...
  public boolean needsInitializer()
  {
    ...
    boolean hasDependencies           =
                            functionsReferences != null && !functionsReferences.isEmpty();
