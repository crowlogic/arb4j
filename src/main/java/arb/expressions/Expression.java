                       ...
                       Consumer<Consumer<Expression<?, ?, ?>>>
{
                                                      ...
                                                      List<String> assignments)
  {
    ...
    // CRITICAL FIX (Issue #825): Inject shared context into nested function DURING initialization
    // This ensures the nested function's context field points to the shared context,
    // so when IT creates its own nested functions, they can access α, β, etc.
    // Generated bytecode equivalent: this.context.injectReferences(this.<functionName>);
    if (nestedFunction.expression != null)
    {
      loadThisFieldOntoStack(mv, "context", Context.class);
      loadThisOntoStack(mv).visitFieldInsn(GETFIELD,
                                           className,
                                           nestedFunction.functionName,
                                           nestedFunction.functionFieldDescriptor());
      invokeVirtualMethod(mv, Context.class, "injectReferences", void.class, Function.class);
    }
  ...
  protected ClassVisitor generatePolynomialMethod(ClassVisitor classVisitor, String operation)
  {
    ...
    mv.visitLdcInsn(Type.getType(coDomainType));
    mv.visitLdcInsn(Type.getType(functionClass));
    mv.visitLdcInsn("_" + operation + className);
    mv.visitLdcInsn(String.format("%s(%s)", operation, rootNode.toString()));
  ...
  protected void generateSelfReference(MethodVisitor mv)
  {
    loadThisOntoStack(mv).visitFieldInsn(GETFIELD,
                                         className,
                                         functionName,
    ...
    loadThisOntoStack(mv);
    mv.visitFieldInsn(PUTFIELD, functionName, functionName, functionClassDescriptor);
  }
  ...
  protected ClassVisitor generateTypeMethod(ClassVisitor classVisitor,
                                            String methodName,
                                            Type type,
                                            String signature)
  {
  ...
  protected ClassVisitor generateTypesetMethod(ClassVisitor classVisitor)
  {
    ...
    methodVisitor.visitLdcInsn(rootNode.typeset());
