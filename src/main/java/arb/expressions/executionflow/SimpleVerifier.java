 ...
 * reference" type (as done in {@link BasicVerifier}).
 *
 * @author Eric Bruneton
 * @author Bing Ran
 */
public class SimpleVerifier extends
                            BasicVerifier
{
  ...
  /**
   * The types of the interfaces directly implemented by the class that is
   * verified.
   */
  ...
  /**
   * Constructs a new {@link SimpleVerifier}. <i>Subclasses must not use this
   * constructor</i>. Instead, they must use the
   * {@link #SimpleVerifier(int, Type, Type, List, boolean)} version.
   */
   ...
   * @param currentSuperClass the type of the super class of the class to be
   *                          verified.
   ...
   * @param currentSuperClass      the type of the super class of the class to be
   *                               verified.
   * @param currentClassInterfaces the types of the interfaces directly
   *                               implemented by the class to be verified.
   * @param isInterface            whether the class to be verifier is an
   *                               interface.
   */
   ...
   * class will not be loaded into the JVM since it may be incorrect.
   *
   * @param api                    the ASM API version supported by this verifier.
   *                               Must be one of the {@code
   ...
   *                               interface.
   */
  protected SimpleVerifier(final int api,
  ...
  protected boolean isSubTypeOf(final BasicValue value, final BasicValue expected)
  {
    ...
    case Type.OBJECT:
      ...
      else if (type.getSort() == Type.OBJECT || type.getSort() == Type.ARRAY)
      {
        ...
        {
          // The merge of class or interface types can only yield class types (because it
          // is not
          // possible in general to find an unambiguous common super interface, due to
          // multiple
          // inheritance). Because of this limitation, we need to relax the subtyping
          // check here
  ...
  /**
   * Returns whether the given type corresponds to the type of an interface. The
   * default implementation of this method loads the class and uses the reflection
   * API to return its result (unless the given type corresponds to the class
   * being verified).
   *
   * @param type a type.
  ...
  /**
   * Returns the type corresponding to the super class of the given type. The
   * default implementation of this method loads the class and uses the reflection
   * API to return its result (unless the given type corresponds to the class
   * being verified).
   *
  ...
  /**
   * Returns whether the class corresponding to the first argument is either the
   * same as, or is a superclass or superinterface of the class corresponding to
   * the second argument. The default implementation of this method loads the
   * classes and uses the reflection API to return its result (unless the result
   * can be computed from the class being verified, and the types of its super
   * classes and implemented interfaces).
   *
   * @param type1 a type.
   * @param type2 another type.
   * @return whether the class corresponding to 'type1' is either the same as, or
   *         is a superclass or superinterface of the class corresponding to
   *         'type2'.
   */
  ...
  /**
   * Loads the class corresponding to the given type. The class is loaded with the
   * class loader specified with {@link #setClassLoader}, or with the class loader
   * of this class if no class loader was specified.
   *
   * @param type a type.
  ...
  protected Class<?> getClass(final Type type)
  {
    ...
    {
      throw new TypeNotPresentException(e.toString(),
                                        e);
    }
  }
}