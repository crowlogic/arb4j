 ...
 * represented with <i>two</i> slots in local variables, and with <i>one</i>
 * slot in the operand stack.
 *
...
public class Frame<V extends Value>
{
  ...
  /**
   * The expected return type of the analyzed method, or {@literal null} if the
   * method returns void.
   */
   ...
   * {@link #numLocals} elements correspond to the local variables. The following
   * {@link #numStack} elements correspond to the operand stack. Long and double
   * values are represented with two elements in the local variables section, and
   * with one element in the operand stack section.
   */
  private V[]              values;

  /**
   * The number of local variables of this frame. Long and double values are
   * represented with two elements.
   */
  private int              numLocals;

  /**
   * The number of elements in the operand stack. Long and double values are
   * represented with a single element.
   */
  private int              numStack;

  /**
   * The maximum number of elements in the operand stack. Long and double values
   * are represented with a single element.
   */
   ...
   *
   * @param numLocals the number of local variables of the frame. Long and double
   *                  values are represented with two elements.
   * @param maxStack  the maximum number of elements in the operand stack, or -1
   *                  if there is no maximum value. Long and double values are
   *                  represented with a single element.
   */
  ...
  {
    this(frame.numLocals,
         frame.values.length - frame.numLocals);
    init(frame); // NOPMD(ConstructorCallsOverridableMethod): can't fix for backward
                 // compatibility.
  }
  ...
  /**
   * Initializes a frame corresponding to the target or to the successor of a jump
   * instruction. This method is called by
   * {@link Analyzer#analyze(String, org.objectweb.asm.tree.MethodNode)} while
   * interpreting jump instructions. It is called once for each possible target of
   * the jump instruction, and once for its successor instruction (except for GOTO
   * and JSR), before the frame is merged with the existing frame at this
   * location. The default implementation of this method does nothing.
   *
   * <p>
   * Overriding this method and changing the frame values allows implementing
   * branch-sensitive analyses.
   *
   * @param opcode the opcode of the jump instruction. Can be IFEQ, IFNE, IFLT,
   *               IFGE, IFGT, IFLE, IF_ICMPEQ, IF_ICMPNE, IF_ICMPLT, IF_ICMPGE,
   *               IF_ICMPGT, IF_ICMPLE, IF_ACMPEQ, IF_ACMPNE, GOTO, JSR, IFNULL,
   *               IFNONNULL, TABLESWITCH or LOOKUPSWITCH.
   * @param target a target of the jump instruction this frame corresponds to, or
   *               {@literal null} if this frame corresponds to the successor of
   *               the jump instruction (i.e. the next instruction in the
   ...
   *
   * @param v the expected return type of the analyzed method, or {@literal null}
  ...
  /**
   * Returns the maximum number of local variables of this frame. Long and double
   * values are represented with two variables.
   *
   ...
   * Long and double values are represented with a single element.
   *
  ...
  /**
   * Returns the value of the given local variable. Long and double values are
 ...
 /**
  ...
  /**
   * Returns the number of elements in the operand stack of this frame. Long and
   * double values are represented with a single element.
   *
  ...
  /**
   * Simulates the execution of the given instruction on this execution stack
   * frame.
   *
   * @param insn        the instruction to execute.
   * @param interpreter the interpreter to use to compute values from other
   *                    values.
   * @throws AnalyzerException if the instruction cannot be executed on this
   ...
   * @return {@literal true} if this frame has been changed as a result of the
   *         merge operation, or {@literal false} otherwise.
  ...
  /**
   * Merges the given frame into this frame (case of a subroutine). The operand
   * stacks are not merged, and only the local variables that have not been used
   * by the subroutine are merged.
   *
   * @param frame      a frame. This frame is left unchanged by this method.
   * @param localsUsed the local variables that are read or written by the
   *                   subroutine. The i-th element is true if and only if the
   *                   local variable at index i is read or written by the
   *                   subroutine.
   * @return {@literal true} if this frame has been changed as a result of the
   *         merge operation, or {@literal false} otherwise.
   */
