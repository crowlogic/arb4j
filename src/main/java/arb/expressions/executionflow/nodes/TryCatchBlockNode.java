...
public class TryCatchBlockNode
{
   ...
   *                handler (see
   *                {@link org.objectweb.asm.Type#getInternalName()}), or
   *                {@literal null} to catch any exceptions (for "finally"
   *                blocks).
   */
  ...
  /**
   * Updates the index of this try catch block in the method's list of try catch
   * block nodes. This index maybe stored in the 'target' field of the type
  ...
  {
    methodVisitor.visitTryCatchBlock(start.getLabel(),
                                     end.getLabel(),
                                     handler == null ? null : handler.getLabel(),
                                     type);
    if (visibleTypeAnnotations != null)
    {
      for (int i = 0, n = visibleTypeAnnotations.size(); i < n; ++i)
