 ...
 * @author Bing Ran
 */
public class BasicVerifier extends
                           BasicInterpreter
{
  ...
  public BasicValue copyOperation(final AbstractInsnNode insn, final BasicValue value)
  {
    ...
    case ALOAD:
      ...
      {
        throw new RuntimeException(insn + " an object reference" + value);
      }
      return value;
    case ASTORE:
      if (!value.isReference() && !BasicValue.RETURNADDRESS_VALUE.equals(value))
      {
        throw new RuntimeException(insn + "an object reference or a return address" + value);
      }
      return value;
    default:
      return value;
  ...
  public BasicValue unaryOperation(final AbstractInsnNode insn, final BasicValue value)
  {
    ...
    case ARRAYLENGTH:
      ...
      {
        throw new RuntimeException("array ref");
      }
    ...
    case IFNONNULL:
      if (!value.isReference())
      {
        throw new RuntimeException("object ref");
    ...
    {
      throw new RuntimeException(insn.toString() + expected);
  ...
  public BasicValue binaryOperation(final AbstractInsnNode insn, final BasicValue value1, final BasicValue value2)
  {
    ...
    {
      throw new RuntimeException(insn.toString() + "First argument" + expected1 + value1);
    }
    else if (!isSubTypeOf(value2, expected2))
    {
      throw new RuntimeException(insn.toString() + "2nd argument" + expected2 + value1);
                                     ...
                                     final BasicValue value3)
  {
    ...
    if (!isSubTypeOf(value1, expected1))
    {
      throw new RuntimeException(insn + "First argument" + "a " + expected1 + " array reference" + value1);
    }
    else if (!BasicValue.INT_VALUE.equals(value2))
    {
      throw new RuntimeException(insn + "Second argument" + BasicValue.INT_VALUE + value2);
    }
    else if (!isSubTypeOf(value3, expected3))
    {
      throw new RuntimeException(insn + "Third argument" + expected3 + value3);
    }
    return null;
  }
  ...
  public BasicValue naryOperation(final AbstractInsnNode insn, final List<? extends BasicValue> values)
  {
    ...
    if (opcode == MULTIANEWARRAY)
    {
      for (BasicValue value : values)
      {
        ...
        {
          throw new RuntimeException(insn.toString() + BasicValue.INT_VALUE + value);
        }
      }
    }
    else
    {
      ...
      while (i < values.size())
      {
        ...
        {
          throw new RuntimeException(insn.toString() + "Argument " + j + expected + actual);
        }
      }
    }
  ...
  public void returnOperation(final AbstractInsnNode insn, final BasicValue value, final BasicValue expected)
  {
    if (!isSubTypeOf(value, expected))
    {
      throw new RuntimeException(insn + "Incompatible return type" + expected + value);
    }
  }
   ...
   *
   * @param objectArrayValue a value corresponding to array of object (or array)
   *                         references.
   * @return the value corresponding to the type of the elements of
   *         'objectArrayValue'.
   * @throws AnalyzerException if objectArrayValue does not correspond to an array
   *                           type.
   */
  ...
  /**
   * Returns whether the type corresponding to the first argument is a subtype of
   * the type corresponding to the second argument.
   *
   * @param value    a value.
