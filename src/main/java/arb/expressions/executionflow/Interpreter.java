...
public abstract class Interpreter<V extends Value>
{
  ...
  /**
   * The ASM API version supported by this interpreter. The value of this field
   * must be one of the {@code ASM}<i>x</i> values in {@link Opcodes}.
   */
   ...
   *
   * @param api the ASM API version supported by this interpreter. Must be one of
   *            the {@code
   ...
   *
   * <p>
   * Called for method parameters (including <code>this</code>), exception handler
   * variable and with <code>null</code> type for variables reserved by long and
   * double types.
   *
   * <p>
   * An interpreter may choose to implement one or more of
   * {@link Interpreter#newReturnTypeValue(Type)},
   * {@link Interpreter#newParameterValue(boolean, int, Type)},
   * {@link Interpreter#newEmptyValue(int)},
   * {@link Interpreter#newExceptionValue(TryCatchBlockNode, Frame, Type)} to
   * distinguish different types of new value.
   *
   * @param type a primitive or reference type, or {@literal null} to represent an
   *             uninitialized value.
  ...
  /**
   * Creates a new value that represents the given parameter type. This method is
   * called to initialize the value of a local corresponding to a method parameter
   * in a frame.
   *
   * <p>
  ...
  /**
   * Creates a new value that represents the given return type. This method is
   * called to initialize the return type value of a frame.
   *
   * <p>
   * By default, calls <code>newValue(type)</code>.
   *
   ...
   * to initialize the value of a local that does not correspond to a method
   * parameter, and to reset one half of a size-2 value when the other half is
   * assigned a size-1 value.
   *
   * <p>
   * By default, calls <code>newValue(null)</code>.
   *
   * @param local the local variable index.
   * @return a value representing an uninitialized value. The size of the returned
  ...
  /**
   * Creates a new value that represents the given exception type. This method is
   * called to initialize the exception value on the call stack at the entry of an
   * exception handler.
   *
   * <p>
   ...
   * @return a value that represents the given {@code exceptionType}. The size of
   *         the returned value must be equal to 1.
   */
  public V newExceptionValue(final TryCatchBlockNode tryCatchBlockNode,
   ...
   * D2I, D2L, D2F, I2B, I2C, I2S, IFEQ, IFNE, IFLT, IFGE, IFGT, IFLE,
   ...
   * method is called for the following opcodes:
   *
   * <p>
   ...
   * both values (for instance, if the two values are two types, the merged value
   * must be a common super type of the two types. If the two values are integer
   * intervals, the merged value must be an interval that contains the previous
   * ones. Likewise for other types of values).
   *
   * @param value1 a value.
   * @param value2 another value.
   * @return the merged value. If the merged value is equal to {@code value1},
   *         this method <i>must</i> return {@code value1}.
   */
