...
public class SamplerController implements
                               Initializable
{
  ...
  public void initialize(URL location, ResourceBundle resources)
  {
    ...
    List<SampleEntry> samples = Arrays.asList(new SampleEntry("Lena(ImageView)",
                                                              LenaSample::new),
                                              new SampleEntry("FXML(ImageView)",
                                                              FXMLSample::new),
                                              new SampleEntry("ViewportRect(ImageView)",
                                                              ViewportRectSample::new),
                                              new SampleEntry("Arbitrary Node(SubScene)",
                                                              ArbitraryNodeSample::new),

                                              new SampleEntry("Swing(ImageView)",
                                                              SwingSample::new));

    samples.forEach(s -> tabs.getTabs()
                             .add(new Tab(s.name,
                                          s.sampleFactory.get().mkRoot())));
