 ...
 * @author rstein
 */
public class ComplexFourierSample extends
                                  AbstractDemoApplication
{
  ...
  private void initData()
  {
    ...
    frawRe = new DefaultDataSet("raw data real",
                                xValues,
                                yValuesRe,
                                xValues.length,
                                true);
    frawIm = new DefaultDataSet("raw data imag",
                                xValues,
                                yValuesIm,
                                xValues.length,
                                true);
    ...
    fSpectraRe = new DefaultDataSet("FFT_RE",
                                    frequency1,
                                    fftSpectraRe,
                                    frequency1.length,
                                    true);
    fSpectraIm = new DefaultDataSet("FFT_IM",
                                    frequency1,
                                    fftSpectraIm,
                                    frequency1.length,
                                    true);

    fspectra   = new DefaultDataSet("FFT",
                                    frequency1,
                                    mag,
                                    frequency1.length,
                                    true);
