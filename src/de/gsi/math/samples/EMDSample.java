 ...
 */
public class EMDSample extends
                       AbstractDemoApplication
{
    ...
    for (int nmode = 0; nmode < (emd.getColumnDimension() - 1); nmode++)
    {
      ...
      if (nmode < fmodeDataSets.length)
      {
        ...
        LOGGER.atInfo()
              ...
              .log("{} mean = {}");
        fmodeDataSets[nmode] = new DefaultErrorDataSet(name,
                                                       time,
                                                       fmodeData[nmode],
  ...
  private double[] loadSyntheticData()
  {
    ...
    for (int i = 0; i < yValues.length; i++)
    {
      ...
      offset     = (i > 500) ? -20 : 0;
      yModel[i]  = (i > 100 && i < 700) ? 0.7
                    * Math.sin(Math.TWO_PI * 2e-4 * (double) i * ((double) i + offset)) : 0;
