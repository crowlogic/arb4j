 ...
 * @author rstein
 */
public class GaussianFitSample extends
                               AbstractDemoApplication
{
  ...
  private void initData()
  {
    ...
    final MyGaussianFunction func = new MyGaussianFunction("gauss1",
                                                           new double[]
                                                           { -3.0, 1.0, 10.0 });
    ...
    final NonLinearRegressionFitter fitter = new NonLinearRegressionFitter(xValues,
                                                                           yValues,
                                                                           yErrors);
    ...
    {
      LOGGER.atInfo()
            .addArgument(func.getParameterName(i))
            .addArgument(start[i])
            .addArgument(fittedParameter[i])
            .addArgument(fittedParameterError[i])
            .log("fitted-parameter  '{}' = {} -> {} +- {}");
    }

    fmodel      = new DefaultDataSet("design model",
                                     xValues,
                                     yModel,
                                     xValues.length,
                                     true);
    fdataOrig   = new DefaultErrorDataSet("data seed with errors",
                                          xValues,
                                          yValues,
                                          yErrors,
                                          yErrors,
                                          xValues.length,
                                          true);
    fdataFitted = new DefaultErrorDataSet("fitted model",
                                          xValues,
                                          yPredicted,
                                          yPredictedError,
                                          yPredictedError,
    ...
    // .addArgument(0.5 * (func.getParameterRangeMaximum(i) -
    // func.getParameterRangeMinimum(i))) //
    // .log("fitted parameter '{}': {} +- {}"); //
    // }
  }
   ...
   * 0.5*(x-mu)^2/sigma^2)
   */
  protected static class MyGaussianFunction extends
                                            AbstractFunction1D
  {
