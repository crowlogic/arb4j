 ...
 * @author rstein
 */
public class WaveletDenoising extends
                              AbstractDemoApplication
{
  ...
  private void initData()
  {
    ...
    final RandomWalkFunction func = new RandomWalkFunction("rand1",
                                                           0.1);
    ...
    fspectraModel = new DefaultDataSet("model",
                                       xValues,
                                       ySModel,
                                       xValues.length,
                                       true);
    fspectra      = new DefaultDataSet("raw data",
                                       xValues,
                                       ySmooth,
                                       xValues.length,
                                       true);
    fspectraFit   = new DefaultDataSet("reconstructed",
                                       xValues,
                                       Arrays.copyOf(recon, recon.length),
                                       xValues.length,
                                       true);
    ...
    if (error1 > error2 && error1 != 0.0)
    {
      LOGGER.atInfo()
            .addArgument(error1)
            .addArgument(error2)
            .addArgument((error1 - error2) / error1 * 100) //
    ...
    fdata          = new DefaultDataSet("model ",
                                        xValues,
                                        yModel,
                                        xValues.length,
                                        true);
    fraw           = new DefaultDataSet("raw data",
                                        xValues,
                                        yValues,
                                        xValues.length,
                                        true);
    freconstructed = new DefaultDataSet("reconstructed",
                                        xValues,
                                        recon,
                                        xValues.length,
                                        true);
  ...
  private static double[][] readDemoData()
  {
    ...
    try ( BufferedReader reader = new BufferedReader(new InputStreamReader(Objects.requireNonNull(WaveletScalogram.class.getResourceAsStream(fileName)))))
    {
