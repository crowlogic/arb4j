 ...
 * @author rstein
 */
public class TSpectrumSample extends
                             Application
{
  ...
  private final Spinner<Integer>       nIterations     = new Spinner<>(1,
                                                                       100,
                                                                       10);
  ...
  private final Spinner<Integer>       spAverageMarkov = new Spinner<>(1,
                                                                       100,
                                                                       7);
  private final Spinner<Integer>       spAverageSearch = new Spinner<>(1,
                                                                       100,
                                                                       7);

  private final Spinner<Double>        spSigma         = new Spinner<>(0.1,
                                                                       100.0,
                                                                       3,
                                                                       0.1);
  private final Spinner<Double>        spThreshold     = new Spinner<>(0.001,
                                                                       99.0,
                                                                       10.0,
                                                                       0.1);
  ...
  private VBox getBottomControls()
  {
    ...
    toolBarBackground.getItems()
                     .addAll(new Label("background:"),
                             new Label("nIterations: "),
                             nIterations,
                             cbxDirection,
                             cbxFilterOrder,
                             cbxSmoothWindow,
    ...
    toolBarMarkov.getItems()
                 .addAll(new Label("Markov background:"), new Label("avg-width [bins: "), spAverageMarkov);
    ...
    toolBarSearch.getItems()
                 .addAll(new Label("peak search: "),
                         new Label("sigma [bins]: "),
                         spSigma,
                         new Label("threshold [%]: "),
                         spThreshold,
                         new Label("Markov?:"),
                         cbMarkov,
                         new Label("subtract bg: "),
                         cbBackground,
                         new Label("avg [bins]:"),
                         spAverageSearch);
  ...
  {
    final DefaultNumericAxis xAxis = new DefaultNumericAxis("frequency",
                                                            "frev");
    final DefaultNumericAxis yAxis = new DefaultNumericAxis("magnitude",
                                                            "dB");
    yAxis.setForceZeroInRange(true);
    final XYChart chart = new XYChart(xAxis,
                                      yAxis);
  ...
  private ToolBar getTopToolBar()
  {
    ...
    bbqButton2.selectedProperty().addListener((ch, o, n) ->
    {
      ...
      demoDataSet.set(new MathDataSet(null,
                                      DataSetMath::magnitudeSpectrumDecibel,
                                      readDemoData(SOURCE2)));
    });
  ...
  public void start(final Stage primaryStage)
  {
    ...
    MathDataSet dsMarkov = new MathDataSet("bgMarkov",
                                           (final double[] input, final double[] output, final int length) ->
                                           {
                                             ...
                                             ArrayMath.decibelInPlace(TSpectrum.smoothMarkov(ArrayMath.inverseDecibel(input),
                                                                                             output,
                                                                                             length,
                                                                                             nAverage));
                                           },
    ...
    MathDataSet   foundPeaks = new MathDataSet("peak",
                                               (DataSet dataSet) ->
                                               {
                                                 ...
                                                 final List<DoublePoint> peaks = TSpectrum.search(freq,
                                                                  ArrayMath.inverseDecibel(rawData),
                                                                  destVector,
                                                 ...
                                                 dsBgSearch.set(freq,
                                                                ArrayMath.decibel(destVector),
                                                                dataSet.getDataCount(),
                                                                true);

                                                 DoubleDataSet retVal = new DoubleDataSet("peaks",
                                                                                          10);
                                                 LOGGER.atInfo()
                                                       .addArgument(peaks.size())
                                                       .addArgument(dataSet.getDataCount())
                                                       .log("found {} peaks in spectrum of length {}");
                                                 for (DoublePoint point : peaks)
                                                 {
                                                   retVal.add(point.getX(), 20 * Math.log10(point.getY()));
                                                   LOGGER.atInfo()
                                                         .addArgument(point.getX())
                                                         .addArgument(point.getY())
                                                         .log("found peak at ({},{})");
                                                 }
  ...
  {
    demoDataSet.invokeListener(new UpdatedDataEvent(demoDataSet,
                                                    "GUI related update"));
  }

  protected static DoubleDataSet generateDemoSineWaveData(final int nData)
  {
    DoubleDataSet function = new DoubleDataSet("composite sine",
                                               nData);
  ...
  protected static DoubleDataSet readDemoData(final String fileName)
  {
    try ( BufferedReader reader = new BufferedReader(new InputStreamReader(Objects.requireNonNull(TSpectrumSample.class.getResourceAsStream(fileName)))))
    {
      ...
      DoubleDataSet spectrum = new DoubleDataSet("BBQ spectrum",
                                                 nDim);
