...
public class DimReductionDataSetSample extends
                                       Application
{
  private static final Logger LOGGER = LoggerFactory.getLogger(DimReductionDataSetSample.class);
  private static GridDataSet  tempDataSet;                                                      // only needed as
                                                                                                // temporary storage
  ...
  public void start(final Stage primaryStage)
  {
    ...
    DimReductionDataSet horizontalSlice = new DimReductionDataSet(waveletScalogram,
                                                                  DataSet.DIM_Y,
                                                                  Option.SLICE);
    XValueIndicator     xValueIndicator = new XValueIndicator(waveletChart1.getXAxis(),
                                                              0.0,
                                                              "slice-y");
    ...
    DimReductionDataSet horizontalRange    = new DimReductionDataSet(waveletScalogram,
                                                                     DataSet.DIM_Y,
                                                                     Option.MEAN);
    XValueIndicator     xRangeIndicatorMin = new XValueIndicator(waveletChart1.getXAxis(),
                                                                 0.0);
    XValueIndicator     xRangeIndicatorMax = new XValueIndicator(waveletChart1.getXAxis(),
                                                                 0.0);
    XRangeIndicator     xRangeIndicator    = new XRangeIndicator(waveletChart1.getXAxis(),
                                                                 0.0,
                                                                 0.0,
                                                                 "range-y");
    ...
    // reduce wavelet data set to 2D by using slices
    DimReductionDataSet verticalSlice   = new DimReductionDataSet(waveletScalogram,
                                                                  DataSet.DIM_X,
                                                                  Option.SLICE);
    YValueIndicator     yValueIndicator = new YValueIndicator(waveletChart2.getYAxis(),
                                                              0.0,
                                                              "slice-x");
    ...
    // reduce wavelet data set to 2D by integrating over range
    DimReductionDataSet verticalRange      = new DimReductionDataSet(waveletScalogram,
                                                                     DataSet.DIM_X,
                                                                     Option.MEAN);
    YValueIndicator     yRangeIndicatorMin = new YValueIndicator(waveletChart2.getYAxis(),
                                                                 0.0);
    YValueIndicator     yRangeIndicatorMax = new YValueIndicator(waveletChart2.getYAxis(),
                                                                 0.0);
    YRangeIndicator     yRangeIndicator    = new YRangeIndicator(waveletChart2.getYAxis(),
                                                                 0.0,
                                                                 0.0,
                                                                 "range-x");
    ...
    final Scene scene = new Scene(new StackPane(root),
                                  1200,
                                  600);
  ...
  private static GridDataSet createDataSet()
  {
    ...
    for (int i = 0; i < tempDataSet.getShape(DataSet.DIM_Y); i++)
    {
      ...
      {
        // val += tempDataSet.get(DataSet.DIM_Z, i *
        // tempDataSet.getDataCount(DataSet.DIM_X) + j);
        count += 1.0;
      }
  ...
  private static XYChart getChart(final boolean isWavelet)
  {
    DefaultNumericAxis xAxis = new DefaultNumericAxis("time",
                                                      "turns");
    DefaultNumericAxis yAxis = new DefaultNumericAxis("frequency",
                                                      "fs");
  ...
  private static double[] readDemoData(int index)
  {
    ...
    try
    {
      try ( BufferedReader reader = new BufferedReader(new InputStreamReader(Objects.requireNonNull(EMDSample.class.getResourceAsStream(fileName)))))
      {
