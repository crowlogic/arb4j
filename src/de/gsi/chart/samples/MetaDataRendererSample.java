...
public class MetaDataRendererSample extends
                                    Application
{
  private static final Logger LOGGER        = LoggerFactory.getLogger(MetaDataRendererSample.class);
  private static final int    N_SAMPLES1    = 10000;                                                // default:
                                                                                                    // 1000000
  private static final int    N_SAMPLES2    = 50;                                                   // default:
                                                                                                    // 1000000
      ...
      public void run()
      {
        Platform.runLater(() ->
        {
          ...
          renderer1.getDatasets()
                   .setAll(new MetaInfoRandomWalkFunction("random walk",
                                                          MetaDataRendererSample.N_SAMPLES1));
          renderer2.getDatasets()
                   .setAll(new MetaInfoGausFunction("gaussy",
                                                    MetaDataRendererSample.N_SAMPLES2,
                                                    MetaDataRendererSample.N_SAMPLES1));
  ...
  public void start(final Stage primaryStage)
  {
    ...
    final XRangeIndicator xRange = new XRangeIndicator(xAxis1,
                                                       50,
                                                       60);
    ...
    startTimer.setOnAction(evt ->
    {
      if (timer == null)
      {
        timer = new Timer("sample-update-timer",
                          true);
        timer.scheduleAtFixedRate(getTask(renderer1, renderer2),
                                  MetaDataRendererSample.UPDATE_DELAY,
  ...
  class MetaInfoGausFunction extends
                             GaussFunction implements
                             DataSetMetaData
  {
  ...
  class MetaInfoRandomWalkFunction extends
                                   RandomWalkFunction implements
                                   DataSetMetaData
  {
