 ...
 * @author rstein
 */
public class ZoomerSample extends
                          Application
{
  ...
  private static DataSet generateData()
  {
    ...
    dataSet.lock().writeLockGuard(() ->
    {
      ...
      for (int n = 0; n < N_SAMPLES; n++)
      {
        ...
        if (n == 500000)
        { // NOPMD this point is really special ;-)
    ...
    // N.B. for performance reasons we let only 'dataSet' fire an event, since we
    // modified both
  ...
  private static void registerZoomerChangeListener(final Zoomer zoomer, final String chart)
  {
    zoomer.zoomStackDeque().addListener((ListChangeListener<Map<Axis, Zoomer.ZoomState>>) (change ->
    {
      while (change.next())
      {
        ...
        if (added != null)
        {
          added.forEach(ch -> ch.forEach((a, s) -> LOGGER.atInfo()
                                                         .addArgument(chart)
                                                         .addArgument(a.getSide())
                                                         .addArgument(s)
                                                         .log("chart '{}' - axis {} -> new zoomState = {}")));
        }

        List<? extends Map<Axis, ZoomState>> removed = change.getRemoved();
        if (removed != null)
        {
          removed.forEach(ch -> ch.forEach((a, s) -> LOGGER.atInfo()
                                                           .addArgument(chart)
                                                           .addArgument(a.getSide())
                                                           .addArgument(s)
                                                           .log("chart '{}' - axis {} -> removed zoomState = {}")));
        }
      }
    }));
  }
