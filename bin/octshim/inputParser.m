...
classdef inputParser < handle
  ...
  methods
    ...
    function addOptional (this, name, def, val = @(x) ~ischar(x))
    ...
    function parse (this, varargin)
      ...
      this.OptionalNames = {};
      if (numel (this.Optional))
        this.OptionalNames = cellfun (@(s) s.name, this.Optional, ...
                                      "UniformOutput", false);
      endif
      ...
      while (vidx++ < pnargin)
        ...
        elseif (this.is_argname ("Optional", name))
          if (++vidx > pnargin)
            this.error (sprintf ("no value for optional '%s'", name));
          endif
          this.Results.(this.last_name) = varargin{vidx};
        else
