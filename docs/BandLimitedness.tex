...
It remains to show $K_\infty(\nu)=0$. Apply the substitution $u=\theta(t)$ to write
\begin{equation}
K_\infty(\nu) = \frac{1}{2\pi}\int_{\theta(T_0)}^{\infty} s(u)\,e^{-i\nu u}\,du, \qquad s(u) := \frac{Z(t(u))}{\sqrt{\theta'(t(u))}},
\end{equation}
where $t(u)$ denotes the inverse of $\theta$. The function $s$ belongs to $L^1\cap L^2$ by the second moment estimate $\int_{T_0}^T |Z(t)|^2\,dt \sim T\log T$ and the asymptotic $\theta'(t)\sim \tfrac{1}{2}\log(t/(2\pi))$. The Fourier transform $\hat{s}(\nu) = K_\infty(\nu)$. By the stationary-phase locus analysis of \texttt{StationaryPhaseLocusAndRemainderAtom.tex}, the Fourier support of $s$ is contained in $[-2,0]$: each Riemann--Siegel mode contributes a frequency $-\log n/(2\pi)$ in the $u$-variable lying in $[-2,0]$, and the remainder contributes a spectral atom at $-1\in[-2,0]$. Therefore $\hat{s}(\nu)=0$ for every $\nu\notin[-2,0]$, giving
\begin{equation}
