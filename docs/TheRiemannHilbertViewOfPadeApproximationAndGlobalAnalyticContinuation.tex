...
\begin{proposition}[Meromorphy of the transformed solution]
\label{prop:meromorphy}
For every $\mu\in(0,1]$, the function $g(z)=y(z^{1/\mu})$ extends to a meromorphic function on $\mathbb{C}$. Its pole set $\mathcal{P}_{g}$ is discrete with no finite accumulation point.
\end{proposition}

\begin{proof}[Sketch]
For $\mu=1$, the classical Cole--Hopf transformation $y=-w'/(c_{2}w)$ reduces the Riccati equation to a second-order linear ODE with constant coefficients, whose solutions are entire; thus $y$ is meromorphic with poles at the zeros of $w$. For $\mu\in(0,1)$, the constant-coefficient fractional Riccati equation admits a generalized Cole--Hopf linearization to a sequential fractional differential system of order $\mu$. The fundamental matrix of this linear system consists of two-parameter Mittag--Leffler functions $E_{\mu,\mu}(\lambda z^{\mu})$, which are entire functions of finite order. The solution $y$ is a ratio of entries of this fundamental matrix, hence meromorphic in $z$. The discrete nature of the pole set follows from the isolated zeros of the Mittag--Leffler functions.
\end{proof}
...
\begin{proof}
By Proposition~\ref{prop:meromorphy}, $g$ is meromorphic with discrete pole set $\mathcal{P}_{g}$. Let $\delta_{T}>0$ be as in \eqref{eq:delta-T}. The poles of $R_{M}$ fall into two classes:
...
\subsection{The fractional Fokas--Its--Kitaev theorem}
\label{sec:heston-fik}

The meromorphy of the transformed solution $g(z)=y(z^{1/\mu})$, established in Proposition~\ref{prop:meromorphy} via the matrix Mittag--Leffler function, permits an exact Riemann--Hilbert characterization of the Pad\'e approximant that does not rely on the abstract Stahl compact of Section~\ref{sec:stahl}. The contour, the weight, and the jump matrix are all determined explicitly by the zeros of the Mittag--Leffler entries.

\subsubsection{The discrete spectral measure}
\label{sec:ml-spectral-measure}

Let $W(z)=E_{\mu,1}(Az)$ be the matrix Mittag--Leffler function with
\begin{equation}
A=\begin{pmatrix}0 & 1\\ -c_{0}c_{2} & c_{1}\end{pmatrix}.
\label{eq:ml-companion}
\end{equation}
Write
\begin{equation}
W(z)=\begin{pmatrix}E_{11}(z) & E_{12}(z)\\ E_{21}(z) & E_{22}(z)\end{pmatrix},
\label{eq:ml-entries}
\end{equation}
where each
\begin{equation}
E_{ij}(z)=\sum_{k=0}^{\infty}\frac{(A^{k})_{ij}}{\Gamma(k\mu+1)}z^{k}
\label{eq:ml-entry-series}
\end{equation}
is an entire function.

By Proposition~\ref{prop:meromorphy},
\begin{equation}
g(z)=-\frac{E_{21}(z)}{c_{2}E_{11}(z)}
\label{eq:g-ml-ratio}
\end{equation}
is meromorphic on $\mathbb{C}$ with discrete pole set
\begin{equation}
\mathcal{P}_{g}=\{p\in\mathbb{C}:E_{11}(p)=0\}.
\label{eq:ml-pole-set}
\end{equation}
The poles are simple for generic parameters $(c_{0},c_{1},c_{2})$. At each pole $p\in\mathcal{P}_{g}$,
\begin{equation}
\Res(g,p)=-\frac{E_{21}(p)}{c_{2}E_{11}'(p)}.
\label{eq:ml-residue}
\end{equation}

\begin{definition}[Mittag--Leffler spectral measure]
\label{def:ml-spectral-measure}
The discrete measure on $\mathbb{C}$ defined by
\begin{equation}
\mu_{g}=\sum_{p\in\mathcal{P}_{g}}\Res(g,p)\,\delta_{p}
\label{eq:ml-spectral-measure}
\end{equation}
is the \emph{spectral measure} of the fractional Riccati equation. Its total mass is
\begin{equation}
\mu_{g}(\mathbb{C})=\sum_{p\in\mathcal{P}_{g}}\Res(g,p)=-a_{1}=-\frac{c_{0}}{\Gamma(\mu+1)},
\label{eq:ml-total-mass}
\end{equation}
and its moments are exactly the M\"untz coefficients:
\begin{equation}
a_{k}=\int_{\mathbb{C}}s^{k-1}\,d\mu_{g}(s)=\sum_{p\in\mathcal{P}_{g}}\Res(g,p)\,p^{k-1},
\qquad k\ge 1.
\label{eq:ml-moments}
\end{equation}
\end{definition}

\subsubsection{The Riemann--Hilbert problem with explicit data}
\label{sec:fractional-fik}

\begin{theorem}[Fractional Fokas--Its--Kitaev representation]
\label{thm:fractional-fik}
Let $g(z)=\sum_{k=1}^{\infty}a_{k}z^{k}$ be the meromorphic function of Proposition~\ref{prop:meromorphy}, with spectral measure $\mu_{g}$. Let $n\ge 1$ and assume $\tau_{k}=\det H^{(k)}\ne 0$ for $k=1,\dots,n$, where $H^{(k)}=(a_{i+j})_{0\le i,j\le k-1}$. Then there exists a unique $2\times 2$ matrix-valued function $Y:\mathbb{C}\setminus\mathcal{P}_{g}\to GL_{2}(\mathbb{C})$ satisfying:
\begin{enumerate}[label=(\roman*)]
\item \emph{Analyticity.} $Y$ is analytic on $\mathbb{C}\setminus\mathcal{P}_{g}$;
\item \emph{Jump condition.} At every $p\in\mathcal{P}_{g}$, $Y$ has a simple pole with residue
\begin{equation}
\Res(Y,p)=
\begin{pmatrix}
0 & 2\pi i\,Q_{n}(p)\Res(g,p)\\
0 & 2\pi i\,\widetilde{Q}_{n-1}(p)\Res(g,p)
\end{pmatrix},
\label{eq:fik-residue}
\end{equation}
where $\widetilde{Q}_{n-1}$ is the degree-$(n-1)$ polynomial of the second row of $Y$. Equivalently, for any contour $\Sigma$ passing through a finite subset $\mathcal{P}_{g}^{(N)}\subset\mathcal{P}_{g}$,
\begin{equation}
Y_{+}(s)=Y_{-}(s)
\begin{pmatrix}
1 & 2\pi i\,w_{N}(s)\\
...
where
\begin{equation}
w_{N}(s)=\sum_{p\in\mathcal{P}_{g}^{(N)}}\Res(g,p)\,\delta_{p}(s);
\label{eq:fik-weight}
\end{equation}
\item \emph{Asymptotics at infinity.}
\begin{equation}
Y(z)=\bigl(I+O(z^{-1})\bigr)
\begin{pmatrix}
z^{n} & 0\\
0 & z^{-n}
\end{pmatrix},
\qquad z\to\infty;
\label{eq:fik-asymptotics}
\end{equation}
\item \emph{Entry recovery.}
\begin{equation}
Y_{11}(z)=Q_{n}(z),
\qquad
Y_{12}(z)=\sum_{p\in\mathcal{P}_{g}}\frac{Q_{n}(p)\Res(g,p)}{p-z},
\label{eq:fik-entries}
\end{equation}
where $Q_{n}$ is the monic orthogonal polynomial of degree $n$ for $\mu_{g}$.
\end{enumerate}
\end{theorem}

\begin{proof}
The existence and uniqueness follow from the general Fokas--Its--Kitaev theory (Theorem~\ref{thm:fik}) once the moment functional is specified. The moment functional $\mathcal{L}[p]=\int p(s)\,d\mu_{g}(s)$ generates the Hankel determinants $\tau_{n}$ by construction. The discrete support of $\mu_{g}$ implies that the Cauchy transform reduces to a sum over poles, and the jump across any contour through $\mathcal{P}_{g}$ picks up the residue contributions $2\pi i\,\Res(g,p)$ at each pole. The asymptotic condition follows from the degree-$n$ polynomial growth of $Q_{n}$ and the $O(z^{-1})$ decay of the Cauchy transform.
\end{proof}

This is the Fokas--Its--Kitaev theorem for a \emph{concrete} meromorphic function: the contour is the Stahl compact of the Mittag--Leffler ratio; the weight is a sum of Dirac masses at the zeros of $E_{11}(z)$ with masses equal to the residues of $-E_{21}(z)/(c_{2}E_{11}(z))$.

\subsubsection{The Pad\'e approximant as resolvent and as Riemann--Hilbert ratio}
\label{sec:three-representations}

\begin{corollary}[Three representations of the Pad\'e approximant]
\label{cor:three-representations}
Under the hypotheses of Theorem~\ref{thm:fractional-fik}, the diagonal Pad\'e approximant $[n/n]_{g}$ admits three equivalent representations:
\begin{enumerate}[label=(\alph*)]
\item \emph{Rational form:}
\begin{equation}
[n/n]_{g}(z)=\frac{P_{n}(z)}{Q_{n}(z)};
\label{eq:rep-rational}
\end{equation}
\item \emph{Resolvent form:}
\begin{equation}
[n/n]_{g}(z)=\langle e_{0},(J_{n}-zI)^{-1}e_{0}\rangle
=\sum_{j=1}^{n}\frac{|\langle e_{0},\psi_{j}^{(n)}\rangle|^{2}}{\lambda_{j}^{(n)}-z},
\label{eq:rep-resolvent}
\end{equation}
where $J_{n}$ is the $n\times n$ Jacobi matrix with spectral data $(\lambda_{j}^{(n)},\psi_{j}^{(n)})$;
\item \emph{Riemann--Hilbert form:}
\begin{equation}
[n/n]_{g}(z)=\frac{Y_{12}(z)}{Y_{11}(z)}
=\frac{\displaystyle\sum_{p\in\mathcal{P}_{g}}\frac{Q_{n}(p)\Res(g,p)}{p-z}}{Q_{n}(z)}.
\label{eq:rep-rh}
\end{equation}
\end{enumerate}
\end{corollary}
...
\begin{proof}
(a) is the definition. (b) is Theorem~\ref{thm:resolvent-representation}. (c) follows from Theorem~\ref{thm:fractional-fik}(iv) and the identity
\begin{equation}
\sum_{p\in\mathcal{P}_{g}}\frac{Q_{n}(p)\Res(g,p)}{p-z}=P_{n}(z),
\label{eq:cauchy-numerator}
\end{equation}
which holds because the left side is the Cauchy transform of the discrete measure $Q_{n}\cdot\mu_{g}$, and the orthogonal polynomial property ensures that the first $n$ moments of the remainder vanish.
\end{proof}

\subsubsection{The Stahl compact for the Mittag--Leffler meromorphic function}
\label{sec:ml-stahl}

\begin{theorem}[Stahl compact of the fractional Riccati solution]
\label{thm:ml-stahl}
Let $g(z)=-E_{21}(z)/(c_{2}E_{11}(z))$ be the meromorphic continuation of the transformed fractional Riccati solution. Then:
\begin{enumerate}[label=(\roman*)]
\item $g\in\mathcal{S}$ (the Stahl class);
\item the Stahl compact $\Delta_{g}$ satisfies
\begin{equation}
\Delta_{g}=\overline{\mathcal{P}}_{g}\cup\gamma,
\label{eq:ml-stahl-compact}
\end{equation}
where $\gamma$ is a finite union of arcs connecting the poles of $g$ to infinity, selected by the $S$-property;
\item the equilibrium measure $\mu_{\Delta_{g}}$ is the weak-$\ast$ limit of the normalized counting measures of the Pad\'e poles:
\begin{equation}
\frac{1}{n}\sum_{j=1}^{n}\delta_{\lambda_{j}^{(n)}}\xrightarrow{w^{\ast}}\mu_{\Delta_{g}}
\qquad\text{as }n\to\infty;
\label{eq:ml-pole-condensation}
\end{equation}
\item the Green's function $g_{\Delta_{g}}(z,\infty)$ with pole at infinity satisfies
\begin{equation}
\limsup_{n\to\infty}\bigl|g(z)-[n/n]_{g}(z)\bigr|^{1/n}\le e^{-g_{\Delta_{g}}(z,\infty)},
\qquad z\in\mathbb{C}\setminus\Delta_{g}.
\label{eq:ml-rate}
\end{equation}
\end{enumerate}
\end{theorem}
...
\begin{proof}
(i) Meromorphic functions on $\mathbb{C}$ with discrete pole set belong to $\mathcal{S}$ because the pole set has logarithmic capacity zero. (ii) The Stahl compact for a meromorphic function is the union of the pole set and the arcs connecting poles to infinity that minimize capacity; the $S$-property selects these arcs. (iii) This is Theorem~\ref{thm:zeros-equilibrium} applied to the orthogonal polynomial zeros, which coincide with the eigenvalues of $J_{n}$ (Corollary~\ref{cor:poles-eigenvalues}). (iv) This is Corollary~\ref{cor:exponential-rate}.
\end{proof}

\subsubsection{The self-consistency theorem}
\label{sec:loop-closure}

\begin{theorem}[Closure of the Mittag--Leffler--Pad\'e--Riemann--Hilbert loop]
\label{thm:loop-closure}
The following diagram commutes:
\begin{equation}
\begin{array}{ccccc}
\text{fractional Riccati}
&\xrightarrow{\;\text{Cole--Hopf}\;}
&\text{matrix Mittag--Leffler}
&\xrightarrow{\;z=t^{\mu}\;}
&\text{meromorphic }g(z)\\[0.4em]
&&&&\Big\downarrow{\scriptstyle\text{moments}}\\[0.4em]
\text{Pad\'e approximant }[n/n]_{g}
&\xleftarrow{\;\text{resolvent}\;}
&\text{Jacobi operator }J_{n}
&\xleftarrow{\;\text{Hankel}\;}
&\text{spectral measure }\mu_{g}\\[0.4em]
\Big\downarrow{\scriptstyle Y_{12}/Y_{11}}\\[0.4em]
\text{Riemann--Hilbert }Y
&\xrightarrow{\;\text{rational function}\;}
&\multicolumn{3}{l}{\text{Stieltjes transform of }\mu_{g}|_{\mathcal{P}_{g}^{(n)}}}
\end{array}
\label{eq:loop-diagram}
\end{equation}
Moreover, as $n\to\infty$:
\begin{itemize}
\item the Pad\'e poles $\lambda_{j}^{(n)}$ condense on $\Delta_{g}$ (Theorem~\ref{thm:ml-stahl});
\item the Pad\'e approximant $[n/n]_{g}(z)\to g(z)$ uniformly on compact subsets of $[0,\infty)$ (Theorem~\ref{thm:global});
\item the Riemann--Hilbert matrix $Y$ converges to the exact factorization for the infinite pole set $\mathcal{P}_{g}$.
\end{itemize}
\end{theorem}

\begin{proof}
The commutativity of the upper row follows from Proposition~\ref{prop:meromorphy}: the Mittag--Leffler solution yields the meromorphic function $g$, whose Taylor coefficients are the moments of $\mu_{g}$. The middle row is Theorem~\ref{thm:resolvent-representation} (Jacobi resolvent equals Pad\'e). The lower edge is Corollary~\ref{cor:three-representations}(c) (Riemann--Hilbert ratio equals Pad\'e). The convergence statements are Theorems~\ref{thm:global} and \ref{thm:ml-stahl}.
\end{proof}

\subsubsection{Theoretical significance}
\label{sec:fik-significance}

The fractional Fokas--Its--Kitaev theorem (Theorem~\ref{thm:fractional-fik}) achieves what the abstract theory of Sections~\ref{sec:rhp}--\ref{sec:stahl} could not: it identifies the \emph{explicit} data of the Riemann--Hilbert problem for a concrete meromorphic function arising from fractional calculus. The weight $w(s)$ is not an arbitrary density but a sum of Dirac masses at the zeros of the Mittag--Leffler entry $E_{11}(z)$, with masses given by the residues of the ratio $-E_{21}(z)/(c_{2}E_{11}(z))$. The contour $\Sigma$ is not selected by an abstract variational principle but is the Stahl compact of a globally meromorphic function, which decomposes into the pole set plus connecting arcs.

This explicit identification is possible because the Cole--Hopf linearization reduces the nonlinear fractional Riccati equation to a linear system solvable by special functions. The resulting meromorphic structure makes the fractional Riccati a test case for the general Riemann--Hilbert--Pad\'e theory: every object (contour, weight, orthogonal polynomials, Jacobi operator, equilibrium measure) can in principle be computed from the Mittag--Leffler series, even though the pole locations are transcendental numbers.

The closure theorem (Theorem~\ref{thm:loop-closure}) guarantees that the three paradigms---special-function exact solution, rational Pad\'e approximation, and matrix Riemann--Hilbert factorization---are mathematically equivalent for the fractional Riccati equation, with explicit convergence certificates and computable error bounds.
