...
For \(t > 0\), every term in the sum has \((\tfrac{1}{4}+k) > 0\), \(\tfrac{t}{2} > 0\), and \(|z_k|^4 > 0\), so every summand is strictly positive. Hence \(\theta''(t) > 0\) for all \(t > 0\). The formula in the paper is exactly correct. The substitution \(x = \theta'(t)\) is therefore a global \(C^\infty\) bijection with \(C^\infty\) inverse \(t(x)\), and \(\theta'(T) \to \infty\) as \(T \to \infty\). ✓
