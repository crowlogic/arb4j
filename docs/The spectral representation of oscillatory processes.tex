...
\]
The phase factor \(e^{i\omega\tau}\) can be factored outside the stochastic integral as a multiplicative constant (in \(\omega\)) acting on the integrand. This is valid because the integral is defined pathwise or in \(L^2\), and measurable functions multiply integrands in the usual sense.
\end{proof}

\begin{remark}
The ability to factor \(e^{i\omega\tau}\) relies on the fact that it is a measurable function of \(\omega\) alone. The stochastic integral
\[
...
Since \(S_u\) is real-valued and the integral is over the entire real line, substituting \(w = -v\) gives \(dv = -dw\):
\[
X_t = -\int_{\infty}^{-\infty} S_{-w} h_t(-w) \, (-dw) = -\int_{\infty}^{-\infty} S_{-w} h_t(-w) \, dw = \int_{-\infty}^\infty S_{-w} h_t(-w) \, dw.
\]

For a real-valued stationary process, \(S_{-w} = S_w\). Relabeling \(w \to u\):
\[
X_t = \int_{-\infty}^\infty S_u h_t(u) \, du.
\]
...
The negative sign cannot be eliminated when \(S_u\) is complex-valued because complex conjugation is not involved. Thus, the equation
\[
X_t = \int_{-\infty}^\infty S_u h_t(u) \, du
\]
...
Let \(\{X_t\}_{t \in \mathbb{R}}\) be a Gaussian oscillatory process with spectral representation
\[
X_t = \int_{\mathbb{R}} A_t(\omega) e^{i\omega t} \, d\Phi(\omega),
\]
where \(\Phi\) is a complex orthogonal random measure with control measure \(\mu\), and \(\{A_t(\omega)\}\) is a family of oscillatory functions. Given a sample path \(\{X_t\}_{t \in \mathbb{R}}\), the orthogonal random measure \(\Phi\) is recovered via
\[
...
\begin{proof}
\textit{Forward-then-Inverse:} Substitute the spectral representation of \(X_t\) into the recovery formula:
\[
\Phi(B) = \lim_{T \to \infty} \frac{1}{2\pi T}
\int_{-T}^T
\left[
\int_{\mathbb{R}} A_t(\lambda) e^{i\lambda t} \, d\Phi(\lambda)
\right]
\left[
\int_B \frac{e^{-i\omega t}}{A_t(\omega)} \, d\omega
\right]
dt.
\]
...
\int_{\mathbb{R}} d\Phi(\lambda)
\int_B d\omega
\int_{-T}^T
A_t(\lambda) \frac{e^{i(\lambda - \omega)t}}{A_t(\omega)} \, dt.
\]

Evaluate the integral over \(t\):
\[
\int_{-T}^T e^{i(\lambda - \omega)t} \, dt
= \frac{2\sin((\lambda - \omega)T)}{\lambda - \omega}.
\]
...
\int_{\mathbb{R}} d\Phi(\lambda)
\int_B d\omega \,
\frac{A_t(\lambda)}{A_t(\omega)}
\frac{2\sin((\lambda - \omega)T)}{\lambda - \omega}.
\]

For \(\lambda = \omega\), apply L'Hôpital's rule:
\[
\lim_{\omega \to \lambda}
\frac{\sin((\lambda - \omega)T)}{\lambda - \omega}
= T.
\]
The contribution from \(\lambda = \omega\) is
\[
\lim_{T \to \infty}
\frac{1}{2\pi T}
\int_B d\Phi(\lambda) \cdot 2T
= \Phi(B).
\]

For \(\lambda \neq \omega\), let
\[
C_1 = \sup_{t,\lambda,\omega}
\left|
\frac{A_t(\lambda)}{A_t(\omega)}
\right|.
\]
For any \(\lambda\) and B,
\[
\left|
\int_{B \setminus \{\lambda\}}
\frac{A_t(\lambda)}{A_t(\omega)}
\frac{2\sin((\lambda - \omega)T)}{\lambda - \omega}
\, d\omega
\right|
...
\frac{1}{2\pi T}
\int_{\mathbb{R}} d\Phi(\lambda)
\int_{B \setminus \{\lambda\}} d\omega \,
\frac{A_t(\lambda)}{A_t(\omega)}
\frac{2\sin((\lambda - \omega)T)}{\lambda - \omega}
\right|
\leq
\frac{2 C_1 |B|}{2\pi T} \, \mathbb{E}[|\Phi(\mathbb{R})|]
\to 0
...
\textit{Inverse-then-Forward:} Substitute the recovered measure into the spectral representation:
\[
X_t
= \int_{\mathbb{R}} A_t(\omega) e^{i\omega t}
\left[
\lim_{S \to \infty} \frac{1}{2\pi S}
\int_{-S}^S X_s \frac{e^{-i\omega s}}{A_s(\omega)} \, ds
\right]
d\omega.
\]
...
\int_{-S}^S X_s
\left[
\int_{\mathbb{R}} A_t(\omega) e^{i\omega(t-s)} \frac{1}{A_s(\omega)} \, d\omega
\right]
ds.
\]

For \(s = t\), the inner integral is
\[
\int_{\mathbb{R}} \frac{A_t(\omega)}{A_t(\omega)} \, d\omega
= \int_{\mathbb{R}} 1 \, d\omega
= 2\pi \delta(0).
\]
The contribution from \(s = t\) is
\[
\lim_{S \to \infty} \frac{1}{2\pi S} X_t \cdot 2\pi \int_{-S}^S \delta(0) \, ds
= X_t.
\]
...
\[
I(t,s)
= \int_{\mathbb{R}} \frac{A_t(\omega)}{A_s(\omega)} e^{i\omega(t-s)} \, d\omega.
\]
Let
\[
C_2 = \sup_{t,s : t \neq s} |I(t,s)|.
...
\right|
\leq
\frac{1}{2\pi S} \cdot 2S \cdot
\sup_{|s| \leq S} |X_s| \cdot C_2
\to 0
