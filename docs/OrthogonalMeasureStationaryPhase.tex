...
\]
We prove (i) at finite $T$, a fraction
\[
r(T) \;=\; \frac{E_{\mathrm{main}}(T)}{E_{\mathrm{total}}(T)} \;\ge\; 1 - C\,\frac{(\log T)^2}{\sqrt{T}}
\]
of the $L^2$ energy of $|K[T](\xi)|$ lies within the symmetric band $[-\xi_{\mathrm{edge}}(T),\xi_{\mathrm{edge}}(T)]$ with
\[
\xi_{\mathrm{edge}}(T) \;=\; \frac{\vartheta'(T)}{\vartheta'(T)+c},
\]
and the remaining fraction $1-r(T)$ sits in Gibbs-type side lobes that extend beyond the band and decay only in ratio to $\Theta(T)$. The band edge is the $n=1$ image of the integration endpoint $t=T$ under the frequency map $\sigma\omega_n$, $\omega_n(T)=(\vartheta'(T)-\log n)/(\vartheta'(T)+c)$, not a support bound on $|K[T]|$. (ii) $\xi_{\mathrm{edge}}(T)\to 1$ as $T\to\infty$, so the energy fraction $r(T)\to 1$. (iii) The rescaled coordinate $\eta=\xi/\xi_{\mathrm{edge}}(T)$ is the Clerc--Mallat dilation $\mathcal{D}_{\xi_{\mathrm{edge}}(T)}$; after rescaling, the main-lobe region carrying the fraction $r(T)$ of the energy is $[-1,1]$ at every $T$, and outside-$[-1,1]$ mass is Gibbs truncation artifact that vanishes in ratio as $T\to\infty$. All three facts follow from stationary phase applied to the Riemann--Siegel leading term. Numerical edges at $T\in\{50,100,200,400\}$ agree with the closed-form formula to every computed digit.
\end{abstract}
