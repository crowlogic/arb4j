<TeXmacs|2.1.1>

<style|<tuple|generic|alt-colors|framed-theorems>>

<\body>
  <doc-data|<doc-title|Hamiltonian Dynamics>|<doc-author|<author-data|<author-name|Stephen
  Crowley>>>>

  The Koenig's function <math|\<chi\>> of a holomorphic self-map
  <math|\<varphi\>> can be thought of as an ancillary function derived from
  <math|\<varphi\>> and defined on the initial phase space which encodes all
  solutions of the classical equations of
  motion.<cite|Curtright_2009><cite|Curtright_2010><cite|Curtright_2011><cite|pub>
  Here the self-map <math|\<varphi\>> whose fixed-points are of interest will
  be Newton map <math|\<varphi\>=N<around*|(|f|)>=t-<frac|f<around*|(|t|)>|<wide|f|\<dot\>><around*|(|t|)>>>.\ 

  <\bibliography|bib|tm-alpha|refs>
    <\bib-list|4>
      <bibitem*|Cur11><label|bib-pub>Thomas Curtright. <newblock>Potentials
      unbounded below. <newblock><with|font-shape|italic|Symmetry,
      Integrability and Geometry: Methods and Applications>, apr
      2011.<newblock>

      <bibitem*|CZ09><label|bib-Curtright_2009>Thomas Curtright<localize| and
      >Cosmas Zachos. <newblock>Evolution profiles and functional equations.
      <newblock><with|font-shape|italic|Journal of Physics A: Mathematical
      and Theoretical>, 42(48):485208, nov 2009.<newblock>

      <bibitem*|CZ10><label|bib-Curtright_2010>Thomas<nbsp>L
      Curtright<localize| and >Cosmas<nbsp>K Zachos. <newblock>Chaotic maps,
      hamiltonian flows and holographic methods.
      <newblock><with|font-shape|italic|Journal of Physics A: Mathematical
      and Theoretical>, 43(44):445101, oct 2010.<newblock>

      <bibitem*|CZ11><label|bib-Curtright_2011>Thomas<nbsp>L.<nbsp>Curtright<localize|
      and >Cosmas<nbsp>K.<nbsp>Zachos. <newblock>Renormalization group
      functional equations. <newblock><with|font-shape|italic|Physical Review
      D>, 83(6), mar 2011.<newblock>
    </bib-list>
  </bibliography>
</body>

<\initial>
  <\collection>
    <associate|page-height|auto>
    <associate|page-medium|paper>
    <associate|page-type|letter>
    <associate|page-width|auto>
  </collection>
</initial>

<\references>
  <\collection>
    <associate|auto-1|<tuple|?|1|../../.TeXmacs/texts/scratch/no_name_5.tm>>
    <associate|bib-Curtright_2009|<tuple|CZ09|1|../../.TeXmacs/texts/scratch/no_name_5.tm>>
    <associate|bib-Curtright_2010|<tuple|CZ10|1|../../.TeXmacs/texts/scratch/no_name_5.tm>>
    <associate|bib-Curtright_2011|<tuple|CZ11|1|../../.TeXmacs/texts/scratch/no_name_5.tm>>
    <associate|bib-pub|<tuple|Cur11|1|../../.TeXmacs/texts/scratch/no_name_5.tm>>
  </collection>
</references>

<\auxiliary>
  <\collection>
    <\associate|bib>
      Curtright_2009

      Curtright_2010

      Curtright_2011

      pub
    </associate>
    <\associate|toc>
      <vspace*|1fn><with|font-series|<quote|bold>|math-font-series|<quote|bold>|Bibliography>
      <datoms|<macro|x|<repeat|<arg|x>|<with|font-series|medium|<with|font-size|1|<space|0.2fn>.<space|0.2fn>>>>>|<htab|5mm>>
      <no-break><pageref|auto-1><vspace|0.5fn>
    </associate>
  </collection>
</auxiliary>