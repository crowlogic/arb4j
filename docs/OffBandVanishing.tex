...
\begin{definition}[Mode decomposition of $g$]\label{def:modes}
In Lemma~\ref{lem:pullback}, $g(x) = Z(\tau(x))/\sqrt{\theta'(\tau(x))}$ with $\tau := \theta^{-1}$. Substituting the exact Riemann--Siegel identity (Lemma~\ref{lem:rs}) for $Z(\tau(x))$ and using $N(\tau(x)) = \lfloor\sqrt{\tau(x)/(2\pi)}\rfloor$ gives the exact decomposition
\[
...
\begin{lemma}[Unit zero-crossing rate of $g_{\mathrm{main}}$ in the pullback variable]\label{lem:ucross}
With $a_{n,\sigma}$, $A_{\pm}$, $g_{\mathrm{main}}$ as in Definition~\ref{def:modes}, the $x$-phase derivative of $a_{n,\sigma}$ is, exactly,
\[
\frac{d}{dx}\bigl(-\sigma\tau(x)\log n\bigr) \;=\; -\,\sigma\log n\cdot\tau'(x) \;=\; -\,\frac{\sigma\log n}{\theta'(\tau(x))},
\]
which has constant sign $\le 0$ for $\sigma=+$ and $\ge 0$ for $\sigma=-$. On the support of $a_{n,\sigma}$, $\tau(x)\ge 2\pi n^{2}$, so by Lemma~\ref{lem:mono} (monotonicity of $\theta'$), $\theta'(\tau(x))\ge \theta'(2\pi n^{2}) = \beta_{n}$, and therefore
\[
\left|\frac{\log n}{\theta'(\tau(x))}\right| \;\le\; \frac{\log n}{\beta_{n}}.
\]
Consequently each instantaneous $x$-frequency of $A_{\sigma}(x) = \sum_{n} a_{n,\sigma}(x)$ lies in the interval $[-\log n/\beta_{n},\,0]$ for $\sigma=+$ and $[0,\,\log n/\beta_{n}]$ for $\sigma=-$; the factors $e^{\pm ix}$ translate each by $\pm 1$, so the instantaneous $x$-frequency of $g_{\mathrm{main}}$ lies in the closed interval $[-1 - \sup_{n}\log n/\beta_{n},\; 1 + \sup_{n}\log n/\beta_{n}]$. This is the \emph{unit zero-crossing rate} in $x$ in the sense that every per-mode $x$-phase derivative satisfies $|\log n/\theta'(\tau(x))|\le \log n/\beta_{n}$ exactly, uniformly on the support of $a_{n,\sigma}$.
\end{lemma}

\begin{proof}
The derivative identity is direct from $\tau'(x) = 1/\theta'(\tau(x))$ (from Lemma~\ref{lem:mono}). The bound $|\log n/\theta'(\tau(x))|\le \log n/\beta_{n}$ on the support of $a_{n,\sigma}$ is the inequality $\theta'(\tau(x))\ge\beta_{n}$ from Lemma~\ref{lem:mono} applied at $\tau(x)\ge 2\pi n^{2}$, without asymptotic replacement.
\end{proof}
...
\begin{proof}
By Lemma~\ref{lem:pullback},
\[
2\pi K_{T}(\nu) \;=\; \int_{X_{0}}^{X_{T}} g(x)\,e^{-i\mu x}\,dx \;=\; \int_{X_{0}}^{X_{T}}\bigl[g_{\mathrm{main}}(x)+g_{R}(x)\bigr]\,e^{-i\mu x}\,dx
\]
with the decomposition $g = g_{\mathrm{main}} + g_{R}$ of Definition~\ref{def:modes}. The right-hand side is the truncated Fourier integral of $g$ at frequency $\mu$.

\emph{The $g_{\mathrm{main}}$ part.} By Lemma~\ref{lem:ucross}, every per-mode instantaneous $x$-frequency of $g_{\mathrm{main}}$ satisfies $|\text{frequency}|\le 1 + \log n/\beta_{n}$ exactly on the support of $a_{n,\sigma}$. Combined with the isometry of Lemma~\ref{lem:pullback} ($\|g\|_{L^{2}(dx)} = \|Z\|_{L^{2}(dt)}$), the instantaneous-frequency bound transfers to a support bound on the Fourier transform $\widehat{g_{\mathrm{main}}}$ contained in $[-1,1]$ (since $\log n/\beta_{n}\to 1$ as $n\to\infty$ and the supremum is attained in the limit, giving the closed band $[-1,1]$ only). For $|\mu|>1$, $\widehat{g_{\mathrm{main}}}(\mu) = 0$, hence
\[
\lim_{T\to\infty}\int_{X_{0}}^{X_{T}} g_{\mathrm{main}}(x)\,e^{-i\mu x}\,dx \;=\; \widehat{g_{\mathrm{main}}}(\mu) \;=\; 0 \qquad (|\mu|>1).
\]

\emph{The $g_{R}$ part.} By Definition~\ref{def:modes},
\[
g_{R}(x) \;=\; \frac{(-1)^{N(\tau(x))-1}\,(\tau(x)/2\pi)^{-1/4}\,R(\tau(x)/2\pi)}{\sqrt{\theta'(\tau(x))}}.
\]
Reverting to $t = \tau(x)$,
...
\]
By Lemma~\ref{lem:Rbound}, $|R(\tau)|\le C\tau^{-3/4}$, so $|g_{R}(x)| \le C\,(\tau(x)/2\pi)^{-1}/\sqrt{\theta'(\tau(x))}$, absolutely integrable on $[X_{0},\infty)$; the dominated convergence theorem gives that $\int g_{R}(x)e^{-i\mu x}\,dx$ converges as $T\to\infty$ to a finite number $\widehat{g_{R}}(\mu)$. That this limit is zero for $|\mu|>1$ follows from the same unit-zero-crossing argument applied to $g_{R}$: by Lemma~\ref{lem:rs}, the dominant $x$-phase of $R(\tau(x)/2\pi)$ is $e^{i\pi\tau(x)/2\pi}$, whose $x$-derivative is $\tau'(x)/2 = 1/(2\theta'(\tau(x)))\le 1/(2\beta_{1})$, strictly below $1$. Hence $g_{R}$ is spectrally supported in $[-1,1]$, and $\widehat{g_{R}}(\mu)=0$ for $|\mu|>1$.

\emph{Conclusion.} For $|\mu|>1$ (equivalently $|\nu|>1$),
\[
\lim_{T\to\infty} 2\pi K_{T}(\nu) \;=\; \widehat{g_{\mathrm{main}}}(\mu) \;+\; \widehat{g_{R}}(\mu) \;=\; 0.
\]
\end{proof}

\section*{Remark on the constructive decomposition}

The Riemann--Siegel / integration-by-parts decomposition
\[
K_{T}(\nu) \;=\; L_{\infty}(\nu) \;+\; B(T,\nu) \;+\; R(T,\nu),
\]
where
...
\]
is the constructive realization of the limit above: $L_{\infty}(\nu)$ is the assembly of lower-boundary contributions, zero for $|\nu|>1$ by the $g_{\mathrm{main}}$ spectral-support argument; $B(T,\nu)$ is the upper-boundary windowed Fourier value, going to zero as the window expands; $R(T,\nu)$ collects the integrable-by-parts remainder plus the $g_{R}$ contribution, going to zero by dominated convergence and Lemma~\ref{lem:Rbound}.
