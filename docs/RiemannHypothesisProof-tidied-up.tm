...
<\body>
  ...
  <math|\<mu\><around|(|d*\<xi\>|)>=S<around|(|\<xi\>|)>*d*\<xi\>>; <math|S>
  is the Wiener spectral density (probability-theory normalization,
  integrating to <math|K<around|(|0|)>>).

  Bochner's theorem: a continuous function <math|K> on <math|<R>> is
  positive-definite if and only if it is the Fourier transform of a finite
  positive Borel measure on <math|<R>>; equivalently, <math|K> is
  positive-definite if and only if its spectral measure is nonnegative. The
  Schwartz space is <math|<Sch><around|(|<R>|)>>.
  ...
  <\proof>
    ...
    Moreover <math|F<rsub|n,\<varepsilon\>>> is the autocorrelation of
    <math|d\<mapsto\>d<rsup|n>*<wide|X<rsub|\<varepsilon\>>|^><around|(|d|)><rsup|1/2>\<in\>L<rsup|2><around|(|<R>|)>>
    in the convolution sense, hence positive-definite as a function on
    <math|<R>>; equivalently, by the Wiener\UKhinchin identity for
    autocorrelations, <math|<wide|F<rsub|n,\<varepsilon\>>|^>=<around|\||<wide|d<rsup|n>*<wide|X<rsub|\<varepsilon\>>|^><rsup|1/2>|^>|\|><rsup|2>\<ge\>0>.
    Then <math|L<rsub|n><around|[|X<rsub|\<varepsilon\>>|]><around|(|u|)>=<tfrac|1|4*<around|(|2*\<pi\>|)><rsup|2>><wide|F<rsub|n,\<varepsilon\>>|^><around|(|-u|)>\<ge\>0>
    by Bochner.
  </proof>
  ...
  <\proof>
    ...
    The Voronoi-type estimate<nbsp><cite-detail|Ivic|Ch.<nbsp>3, summation
    formula for <math|d<around|(|n|)>> against oscillatory weights> gives
