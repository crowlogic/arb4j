...
\label{def:goncar}
A function $f\in\mathcal{S}$ satisfies the \emph{Gon\v{c}ar condition} if the number of poles of $[n/n]_{f}$ in any compact $K\subset D_{f}$ is uniformly bounded in $n$ by a constant depending only on $K$ and $f$.
\end{definition}

\begin{theorem}[Gon\v{c}ar's uniform convergence theorem]
\label{thm:goncar}
If $f\in\mathcal{S}$ satisfies the Gon\v{c}ar condition, then $[n/n]_{f}\to f$ uniformly on compact subsets of $D_{f}\setminus\mathcal{P}_{f}$. The exceptional set vanishes for sufficiently large $n$.
\end{theorem}

\begin{remark}
\label{rem:goncar-restrictive}
The Gon\v{c}ar condition is restrictive. It is satisfied by hyperelliptic functions and by certain explicitly constructed classes, but it fails generically. For most functions of practical interest, including the rough Heston characteristic function discussed in Section~\ref{sec:heston}, only convergence in capacity is available, and the implementation must accommodate the possibility of spurious poles in the working domain through an a-posteriori certificate of the type developed in Section~\ref{sec:heston-error}.
\end{remark}
...
\end{equation}
where $D^{\mu}$ is the Caputo derivative of order $\mu=H+\tfrac{1}{2}\in(\tfrac{1}{2},1)$, $H$ is the Hurst parameter, and the coefficients $c_{0},c_{1},c_{2}\in\mathbb{C}$ depend on the model parameters and the Fourier variable. We develop the M\"untz spectral solution, the Pad\'e resummation in $z=t^{\mu}$, and the global convergence theorem with computable error bound.
...
Let
\begin{equation}
T_{0}^{\mu}=\min\!\left(\frac{\Gamma(\mu+1)}{|c_{0}|+|c_{1}|+|c_{2}|+1},\;\frac{\Gamma(\mu+1)}{2(|c_{1}|+2|c_{2}|+1)}\right)>0.
\label{eq:T0}
\end{equation}
\begin{enumerate}[label=(\roman*)]
...
\label{eq:phi-op}
\end{equation}
For $y\in B_{1}$, the integrand is bounded by $|c_{0}|+|c_{1}|+|c_{2}|$, and \eqref{eq:Ir-power} gives
\begin{equation}
|\Phi(y)(t)|\le(|c_{0}|+|c_{1}|+|c_{2}|)\cdot\frac{T_{0}^{\mu}}{\Gamma(\mu+1)}<1
\label{eq:phi-bound}
\end{equation}
by the first term in \eqref{eq:T0}. For $y_{1},y_{2}\in B_{1}$,
\begin{equation}
|c_{1}(y_{1}-y_{2})+c_{2}(y_{1}^{2}-y_{2}^{2})|\le(|c_{1}|+2|c_{2}|)\|y_{1}-y_{2}\|_{\infty},
\label{eq:lipschitz}
\end{equation}
so
\begin{equation}
\|\Phi(y_{1})-\Phi(y_{2})\|_{\infty}\le \frac{T_{0}^{\mu}(|c_{1}|+2|c_{2}|)}{\Gamma(\mu+1)}\|y_{1}-y_{2}\|_{\infty}<\tfrac{1}{2}\|y_{1}-y_{2}\|_{\infty}
...
\begin{remark}[Invertibility of $H_{M}$]
\label{rem:invertibility}
$H_{M}$ is invertible if and only if $g(z)$ is not a rational function of degree less than $M$. For $\mu=1$ the Riccati solution is rational, and $H_{M}$ becomes singular for $M$ exceeding the exact pole order. For $\mu\in(0,1)$, $g(z)$ admits no finite-order rational representation, and $\det H_{M}\ne 0$ generically for all $M\ge 1$. If a particular order yields $\det H_{M}=0$, the index $M\pm 1$ may be used.
\end{remark}
...
\label{eq:rho-star}
\end{equation}
and assuming Assumption~\ref{ass:geometric-tail}(iii) holds with this $\rho_{\ast}$,
\begin{equation}
...
\begin{theorem}[de~Montessus de~Ballore]
\label{thm:dMdB}
Let $g$ be meromorphic in $\{|z|<\rho\}$ with exactly $n$ poles (counted with multiplicity) in that disc and no poles on the boundary. The diagonal Pad\'e approximants $R_{M}$ converge to $g$ uniformly on compact subsets of $\{|z|<\rho\}\setminus\{\text{poles}\}$ as $M\to\infty$.
\end{theorem}

\begin{theorem}[Nuttall--Pommerenke]
\label{thm:NP}
Let $g$ be meromorphic in $\mathbb{C}$ with at most countably many poles having no finite accumulation point. The diagonal sequence $R_{M}$ converges to $g$ in capacity on $\mathbb{C}\setminus E$, where $\capa(E)=0$. Convergence is quasi-everywhere but not necessarily pointwise.
\end{theorem}

\begin{remark}[Choice of theorem]
\label{rem:choice}
Theorem~\ref{thm:dMdB} gives uniform convergence on compacta but requires knowing the exact pole count. Theorem~\ref{thm:NP} requires no such count but yields only quasi-everywhere convergence and presupposes meromorphy on $\mathbb{C}$. When $g(z)=y(z^{1/\mu})$ is meromorphic in $\mathbb{C}$ with no poles on $(0,\infty)$, the canonical case for globally bounded Riccati solutions on the real time axis, both theorems apply, and $R_{M}(t^{\mu})\to y(t)$ for every $t>0$.
\end{remark}

\begin{hypothesis}[Stahl class membership]
\label{hyp:stahl}
The function $g(z)=y(z^{1/\mu})$ belongs to Stahl's class $\mathcal{S}$: its global singular set has logarithmic capacity zero.
\end{hypothesis}

\begin{hypothesis}[Square-root branch points]
\label{hyp:sqrt}
The branch points of $g$ are of square-root type. This is suggested by the formal Cole--Hopf transformation $h=-w'/((c_{2})w/2)$, which would reduce the quadratic Riccati equation to a second-order linear equation with discriminant yielding square-root branch points; the fractional derivative introduces non-local effects that may alter the local monodromy, so this is a structural assumption rather than a theorem.
\end{hypothesis}

\begin{hypothesis}[No positive real poles]
\label{hyp:no-pos-poles}
The solution $y(t)$ is bounded on $[0,\infty)$, equivalently, $g(z)$ has no poles on the positive real axis.
\end{hypothesis}

\begin{theorem}[Global Pad\'e--M\"untz representation]
\label{thm:global}
Let $y(t)$ solve the fractional Riccati equation \eqref{eq:fractional-riccati}. Under Hypotheses~\ref{hyp:stahl}, \ref{hyp:sqrt}, and \ref{hyp:no-pos-poles}, with $g$ meromorphic in $\mathbb{C}$ with at most countably many poles having no finite accumulation point,
\begin{equation}
\boxed{\;y(t)=\lim_{M\to\infty}R_{M}(t^{\mu})=\lim_{M\to\infty}\frac{P_{M}(t^{\mu})}{Q_{M}(t^{\mu})}\qquad\forall t\ge 0.\;}
\label{eq:global-pade}
\end{equation}
\end{theorem}

\begin{proof}
Under the meromorphy hypothesis, Theorem~\ref{thm:NP} yields convergence in capacity on $\mathbb{C}\setminus E$ with $\capa(E)=0$. By Hypothesis~\ref{hyp:no-pos-poles}, $g$ is holomorphic in a neighborhood of every point of $(0,\infty)$ and meromorphic in every disc $\{|z|<\rho\}$ with finitely many poles, so Theorem~\ref{thm:dMdB} gives uniform convergence on compact subsets of $\mathbb{C}\setminus\{\text{poles}\}$. In particular, $R_{M}(z)\to g(z)$ pointwise at every $z=t^{\mu}\in(0,\infty)$. At $t=0$, $y(0)=0$ and $R_{M}(0)=p_{0}/Q_{M}(0)=0$, so the limit holds trivially.
\end{proof}

\begin{remark}[Status of the hypotheses]
\label{rem:hypothesis-status}
Hypothesis~\ref{hyp:stahl} is established for $\mu=1$, where the linearization $y=-w'/(c_{2}w)$ reduces the Riccati equation to a linear ODE with entire solutions, hence $g$ is meromorphic with at most countable poles. For $\mu\in(0,1)$ it is conjectured on the basis of the Painlev\'e property of the underlying flow and the empirical pole distribution of the diagonal Pad\'e sequence, and a complete proof for arbitrary $\mu$ remains open. Hypothesis~\ref{hyp:sqrt} is a structural assumption motivated by the classical case. Hypothesis~\ref{hyp:no-pos-poles} is verifiable a-posteriori, in practical computation, by the convergence behavior of $R_{M}(t^{\mu})$ at probe points $t$ of interest, controlled by the bound \eqref{eq:error-final}.
\end{remark}
...
The incremental Hankel solver that builds $H_{M+1}^{-1}$ from $H_{M}^{-1}$ via a bordered rank-one update in $O(M^{2})$ operations is the algebraic realization of the Jacobi truncation: it adds one row and column to $J_{M}$ to produce $J_{M+1}$, and the resulting Pad\'e iterate $R_{M+1}$ is the resolvent of the enlarged operator at the basis vector $e_{0}$.
\end{theorem}
...
\label{eq:cgf}
\end{equation}
where $h$ solves the fractional Riccati equation \eqref{eq:fractional-riccati} with parameters $(c_{0},c_{1},c_{2})$ depending on $u$. Then $\Phi$ admits the explicit representation
\begin{equation}
\Phi(u,T)=\sum_{k=1}^{\infty}a_{k}(u)G_{k}(T),
\label{eq:cgf-series}
\end{equation}
where the coefficients $a_{k}(u)$ are the Puiseux coefficients of Theorem~\ref{thm:riccati-series} and the time integrals are
\begin{equation}
G_{k}(T)=\theta\lambda\frac{T^{k\mu+1}}{k\mu+1}+V_{0}\frac{\Gamma(k\mu+1)}{\Gamma((k-1)\mu+2)}T^{(k-1)\mu+1}.
\label{eq:Gk}
\end{equation}
Truncated at order $N$ and resummed via Pad\'e in $\xi=T^{\mu}$, the compiled expression
\begin{equation}
\Phi:v\mapsto \sum_{k=1}^{N}a_{k}(v)\cdot G_{k}
\label{eq:compiled}
\end{equation}
is the spectral reconstruction of the characteristic function from its moment sequence. Convergence is governed by Theorem~\ref{thm:global} under Hypotheses~\ref{hyp:stahl}--\ref{hyp:no-pos-poles}, and is certified a-posteriori by Corollary~\ref{cor:practical-cert}.
\end{theorem}
...
\item Solve $H_{M}\bm{q}=-\bm{b}$ for the denominator coefficients, by direct factorization or by an incremental bordered update. Cost: $O(M^{3})$ direct, $O(M^{2})$ amortized.
