...
Theorem~\ref{thm:global} requires no \emph{ad hoc} hypotheses. Proposition~\ref{prop:meromorphy} is a structural property of the constant-coefficient fractional Riccati equation, proved via linearization. Lemma~\ref{lem:no-real-poles} follows from the global existence theory of the real Volterra equation. Lemma~\ref{lem:goncar-interval} uses only the generic boundedness of Froissart doublets (Proposition~\ref{prop:doublet-count}) and the de~Montessus separation of true-pole approximants. The result is valid for all $\mu\in(0,1]$, including the classical case $\mu=1$.
\end{remark}
