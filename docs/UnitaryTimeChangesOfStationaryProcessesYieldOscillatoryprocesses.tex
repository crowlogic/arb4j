...
\begin{abstract}
  ...
  \lambda \theta (t)}$ and evolutionary spectrum $dF_t (\lambda) =
  \dot{\theta} (t) dF (\lambda)$. It is proved that sample paths of any
  non-degenerate second-order stationary process lie in $L^2_{\tmop{loc}}
  (\mathbb{R})$, making the operator applicable to typical realizations. A
  zero-localization measure $d \mu (t) = \delta (Z (t)) | \dot{Z} (t) | dt$
  induces a Hilbert space $L^2 (\mu)$ on the zero set of an oscillatory
  process $Z$, and the multiplication operator $(Lf) (t) = tf (t)$ has simple
  pure point spectrum equal to the zero crossing set of $Z$.
\end{abstract}
...
\begin{definition}
  \label{def:cramer}\tmtextbf{[Cram{\'e}r representation]} A zero-mean
...
\begin{theorem}
  \label{thm:existence_osc}\tmtextbf{[Existence]} Let $F$ be an absolutely
...
\begin{theorem}
  ...
  with $\dot{\theta} (t) > 0$ almost everywhere and $\dot{\theta} (t) = 0$
  only on sets of Lebesgue measure zero. For $f$ measurable, define
  \begin{equation}
...
\begin{theorem}
  ...
  and covariance
  \begin{equation}
    \label{eq:covariance_Z} R_Z (t, s) = \sqrt{\dot{\theta} (t)  \dot{\theta}
...
\begin{proof}
  \begin{enumerate}
    ...
    \begin{equation}
      \label{eq:spectrum_closure} \sigma (L) = \overline{\{t \in \mathbb{R}: Z
      (t) = 0\}}
    \end{equation}
  \end{enumerate}
\end{proof}
