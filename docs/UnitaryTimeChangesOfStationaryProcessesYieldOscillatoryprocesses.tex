...
\begin{definition}
  \label{def:gaussian_process}\textbf{(Gaussian process)} Let $(\Omega,
...
\begin{definition}
  \label{def:cramer}\textbf{[Cram{\'e}r spectral representation]}
...
\begin{definition}
  \label{def:fourier_conventions}\textbf{[Fourier transform conventions]}
...
\begin{definition}
  \label{def:L2loc}\textbf{[Locally square-integrable functions]} Define
  \begin{equation}
...
\begin{definition}
  ...
  \begin{equation}
    A_t \in L^2 (F) \quad \forall t \in \mathbb{R} \label{eq:gain_L2}
  \end{equation}
...
\begin{theorem}
  [Triad of Gain Representations]\label{thm:duality_complete}Let $A_t
  (\omega)$ be a gain function as in \eqref{eq:gain_L2} such that for each
  fixed $t \in \mathbb{R}$, the map $\omega \mapsto A_t (\omega)$ belongs to
  $L^2 (\mathbb{R})$.  Then the following relationships hold
  simultaneously:
  \begin{equation}
    A_t (\omega) = \int_{- \infty}^{\infty} e^{i \lambda t}  \hspace{0.17em}
    dH_{\omega} (\lambda) = \int_{- \infty}^{\infty} h_t (u) e^{i \omega u} 
    \hspace{0.17em} du \label{gain}
  \end{equation}
  \begin{equation}
    h_t (u) = \frac{1}{2 \pi}  \int_{- \infty}^{\infty} A_t (\omega) e^{- i
    \omega u}  \hspace{0.17em} d \omega \label{filterGain}
  \end{equation}
  \begin{equation}
    H_{\omega} (\lambda) = \frac{1}{2 \pi}  \int_{- \infty}^{\infty} A_t
    (\omega) e^{- i \lambda t}  \hspace{0.17em} dt \label{gainDistribution}
  \end{equation}
\end{theorem}
...
\begin{proof}
  \begin{enumerate}
    ...
    \item Define $h (t, u)$ by \eqref{filterGain}:
    \begin{equation}
      h_t (u) \assign \frac{1}{2 \pi}  \int_{- \infty}^{\infty} A_t (\omega)
      e^{- i \omega u}  \hspace{0.17em} d \omega \label{eq:def_ht}
    \end{equation}
    which is well-defined in $L^2(\mathbb{R})$ by Plancherel's theorem since
    $A_t \in L^2(\mathbb{R})$ by hypothesis. Substituting \eqref{eq:def_ht}
    into the right-hand side of \eqref{gain} yields
    \begin{equation}
      \begin{array}{ll}
        \int_{- \infty}^{\infty} h_t (u) e^{i \omega u}  \hspace{0.17em} du &
        = \int_{- \infty}^{\infty} \left( \frac{1}{2 \pi}  \int_{-
        \infty}^{\infty} A_t (\nu) e^{- i \nu u}  \hspace{0.17em} d \nu
        \right) e^{i \omega u}  \hspace{0.17em} du\\
        & = \frac{1}{2 \pi}  \int_{- \infty}^{\infty} A_t (\nu) \left(
        \int_{- \infty}^{\infty} e^{i (\omega - \nu) u}  \hspace{0.17em} du
        \right) d \nu
      \end{array}
    \end{equation}
    The inner integral is the delta function
    \begin{equation}
      \int_{- \infty}^{\infty} e^{i (\omega - \nu) u}  \hspace{0.17em} du = 2
      \pi \delta (\omega - \nu)
    \end{equation}
    therefore,
    \begin{equation}
    ...
    \item To show that the gain $A_t (\lambda)$ and gain distributions
    $H_{\omega} (\lambda)$ are Fourier transform pairs fix $\omega \in
    \mathbb{R}$. By \eqref{gain}, the function $t \mapsto A_t (\omega)$ is the
    Fourier transform (in $t$) of the tempered distribution $H_{\omega}$:
    \begin{equation}
      A_t (\omega) = \int_{- \infty}^{\infty} e^{i \lambda t}  \hspace{0.17em}
      dH_{\omega} (\lambda) =\mathcal{F}_{\lambda} [H_{\omega}] (t)
    \end{equation}
    where $\mathcal{F}_{\lambda}$ denotes the Fourier transform with respect
    to $\lambda$. Consider the Fourier transform of $A_t (\omega)$ with
    respect to $t$:
    \begin{equation}
      \mathcal{F}_t [A_t (\omega)] (\lambda) = \int_{- \infty}^{\infty} A_t
      (\omega) e^{- i \lambda t}  \hspace{0.17em} dt
    \end{equation}
    Substituting the representation of $A_t (\omega)$,
    \begin{equation}
      \begin{array}{ll}
        \int_{- \infty}^{\infty} A_t (\omega) e^{- i \lambda t} 
        \hspace{0.17em} dt & = \int_{- \infty}^{\infty} \left( \int_{-
        \infty}^{\infty} e^{i \lambda' t}  \hspace{0.17em} dH_{\omega}
        (\lambda') \right) e^{- i \lambda t}  \hspace{0.17em} dt\\
        & = \int_{- \infty}^{\infty} \left( \int_{- \infty}^{\infty} e^{i
        (\lambda' - \lambda) t}  \hspace{0.17em} dt \right) dH_{\omega}
        (\lambda')\\
        & = \int_{- \infty}^{\infty} 2 \pi \delta (\lambda' - \lambda) 
        \hspace{0.17em} dH_{\omega} (\lambda')\\
        & = 2 \pi H_{\omega} (\lambda)
      \end{array}
    \end{equation}
    Thus,
    \begin{equation}
      \int_{- \infty}^{\infty} A_t (\omega) e^{- i \lambda t}  \hspace{0.17em}
      dt = 2 \pi H_{\omega} (\lambda)
    \end{equation}
    ...
    \begin{equation}
      ...
      (\omega) e^{- i \lambda t}  \hspace{0.17em} dt
    \end{equation}
    as an identity in $\mathcal{S}' (\mathbb{R})$ with respect to the
    $\lambda$-variable. This is precisely \eqref{gain}.
  \end{enumerate}
  
\end{proof}

\begin{theorem}[Duality of Gain Representations]
  \label{thm:duality}Let $A_t (\omega)$ be a complex-valued function on
  $\mathbb{R} \times \mathbb{R}$ satisfying:
  \begin{enumerate}
    \item \label{item:hypothesis_t} For each fixed $\omega \in \mathbb{R}$,
    ...
    \item \label{item:hypothesis_omega} For each fixed $t \in \mathbb{R}$, the
  ...
  simultaneously:
  \begin{enumerate}
    \item \label{eq:gain_from_measure} $A_t (\omega) = \langle H_{\omega},
    e^{i \lambda t} \rangle$ for all $t \in \mathbb{R}$;
    
    \item \label{eq:gain_from_filter} $A_t (\omega) = \int_{- \infty}^{\infty}
    h_t (u)  \hspace{0.17em} e^{i \omega u}  \hspace{0.17em} du$ for all
    $\omega \in \mathbb{R}$, where the integral is taken in the sense of
    $L^2$-limits;
    
    \item \label{eq:filter_from_gain} $h_t (u) = \frac{1}{2 \pi}  \int_{-
    ...
    \item \label{eq:measure_from_gain} as an identity in $\mathcal{S}'
...
\begin{proof}
  \begin{enumerate}
    \item \tmtextbf{Proof of \eqref{eq:gain_from_measure}.} By hypothesis
    \ref{item:hypothesis_t}, for each fixed $\omega$ there exists a tempered
    distribution $H_{\omega} \in \mathcal{S}' (\mathbb{R})$ such that
    \[ A_t (\omega) = \langle H_{\omega}, e^{i \lambda t} \rangle  \quad
       \text{for all } t \in \mathbb{R}. \]
    This is precisely the content of \eqref{eq:gain_from_measure}.
    
    \item \tmtextbf{Proof of \eqref{eq:filter_from_gain}.} Fix $t \in
    \mathbb{R}$. Hypothesis \ref{item:hypothesis_omega} gives $A_t (\cdummy)
    ...
    \item \tmtextbf{Proof of \eqref{eq:gain_from_filter}.} For each fixed $t$,
    ...
    \eqref{eq:gain_from_filter}.
    
    \item \tmtextbf{Proof of \eqref{eq:measure_from_gain}.} For each fixed
    $\omega$, hypothesis \ref{item:hypothesis_t} and
    \eqref{eq:gain_from_measure} say that $A_{\cdot} (\omega)$ is the Fourier
