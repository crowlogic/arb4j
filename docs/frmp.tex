...
$O(M\log^2 M)$ operations per strike-maturity pair, with the
front-end Chebyshev construction performed once symbolically in
$\C[v]$ at $O(M^3)$ cost per model calibration and amortised
across the entire $(K,T)$-surface. The price formula is derived
by two analytically independent routes (convolution chain and
Mellin--Barnes inversion) converging on identical parameter data.
The Pad\'{e} characteristic function has Gaussian real-axis decay,
sharper than the stretched-exponential decay of the exact rough
Heston cf. Third (Theorem~\ref{thm:price}), the same prices may
...
and no grid in $v$ is required. The front-end is executed
symbolically in $\C[v]$, producing $\Phi_M(v,T)$ as a rational
function of $(v,T)$ once per model parameter set.

\subsection{Three-tier amortisation}

\paragraph{Tier 0 (per model calibration).}
M\"untz coefficients $\{a_k(v)\}_{k=1}^{2M}$ are polynomials in
$\C[v]$ of degree at most $2M+1$. The Chebyshev algorithm
executes $O(M^2)$ ring operations on polynomials of degree
$O(M)$, giving $O(M^3)$ scalar operations to produce $P_M(z;v)$
and $Q_M(z;v)$. Run once per model parameter set.

\paragraph{Tier 1 (per maturity $T$).}
Specialise $z=T^\mu$, reduce to univariate rational in $v$,
$O(M)$. PFE by root-finding and cover-up, $O(M^2)$ per $T$.

\paragraph{Tier 2 (per strike $K$ at fixed $T$).}
Fox $H$ evaluation via Newton--Girard consolidation and residue
series, $O(M\log^2M)$ per $(K,T)$, no quadrature.

\subsection{Total cost across an $(N_T,N_K)$-surface}

\begin{equation}\label{eq:total_cost}
\underbrace{O(M^3)}_{\text{Tier 0, once}}+\underbrace{N_T\cdot O(M^2)}_{\text{Tier 1, per }T}+\underbrace{N_TN_K\cdot O(M\log^2M)}_{\text{Tier 2, per }(K,T)}.
\end{equation}

\subsection{COS endpoint}
The COS endpoint requires $\phi_M(v_n,T)$ on a Fourier grid;
with the symbolic $\Phi_M(v,T)\in\C(v)$ from Tier 0, evaluation
at each $v_n$ is $O(M)$, giving $O(NM)$ per $(K,T)$.

\begin{remark}[Cost decomposition]\label{rem:cost_decomposition}
The pipeline scales as $O(M^3)$ once per model calibration,
$O(M^2)$ per maturity, and $O(M\log^2M)$ per $(K,T)$ via Fox $H$.
Tier 0 is independent of both $T$ (the Chebyshev recurrence acts
on $\{M_k(v)\}$ before $z=T^\mu$) and $K$ (execution is symbolic
in $\C[v]$, with $K$ entering only at Tier 2 through $\zeta_S(K,T)$).
\end{remark}
...
with no quadrature; the front-end is $O(M^3)$ once per model
calibration in $\C[v]$ symbolically and $O(M^2)$ per maturity,
amortised across the entire $(K,T)$-surface
