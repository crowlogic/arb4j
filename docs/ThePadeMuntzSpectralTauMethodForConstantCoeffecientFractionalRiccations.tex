...
\begin{abstract}
  ...
  derived from a uniform geometric tail estimate.
\end{abstract}
...
\begin{definition}
  ...
  \begin{equation}
    \label{eq:caputo_RL} I^{\mu} D^{\mu} f (t) = f (t) - f (0)
  \end{equation}
  \begin{equation}
    D^{\mu} I^{\mu} f (t) = f (t)
  \end{equation}
...
\begin{proof}
  ...
  \begin{equation}
    I^r t^s = \frac{1}{\Gamma (r)}  \int_0^t (t - u)^{r - 1} u^s
    \hspace{0.17em} \dd u
  \end{equation}
  The substitution $u = tx$ gives
  \begin{equation}
    \begin{array}{ll}
      I^r t^s & = \frac{t^{s + r}}{\Gamma (r)}  \int_0^1 (1 - x)^{r - 1} x^s
      \hspace{0.17em} \dd x\\
      & = \frac{t^{s + r}}{\Gamma (r)} \cdot \frac{\Gamma (s + 1) 
      \hspace{0.17em} \Gamma (r)}{\Gamma (s + r + 1)} = \frac{\Gamma (s +
      1)}{\Gamma (s + r + 1)}  \hspace{0.17em} t^{s + r}
    \end{array}
  \end{equation}
...
\begin{equation}
  \label{eq:frac_riccati_V} y (\lambda, t) = \frac{1}{\Gamma (\mu)}  \int_0^t
  (t - s)^{\mu - 1}  \hspace{-0.17em} \left( c_0 (\lambda) + c_1 (\lambda)
  \hspace{0.17em} y (s) + c_2 (\lambda) y (s)^2 \right) \dd s
\end{equation}
\begin{remark}
...
\begin{theorem}
  ...
  \begin{enumerate}
    ...
    \end{equation}
    \item {\tmem{Generic finiteness, $R < |c_0 + c_1 y (t) + c_2 y (t)^2 | \le
    |c_0 | + |c_1 | + |c_2 | \infty$.}} For generic parameters with $c_2 \ne
    0$, $R < \infty$: the complexified Riccati flow develops movable
    singularities at finite distance from the origin in the complex $z$-plane.
  \end{enumerate}
\end{theorem}
...
\begin{proof}
  \begin{enumeratenumeric}
    ...
    \begin{equation}
      \begin{array}{l}
        \assign \hspace{0.27em} \frac{1}{\Gamma (\mu)}  \int_0^t (t - s)^{\mu
        - 1}  \left( c_0 + c_1  \hspace{0.17em} y (s) + c_2  \hspace{0.17em} y
        (s)^2 \right) \dd s\\
        = \hspace{0.27em} I^{\mu}  (c_0 + c_1 y + c_2 y^2) (t)\\
        
      \end{array}
    \end{equation}
    For $y \in B_1$ and $t \in [0, T_0]$,
    \begin{equation}
    ...
    \begin{equation}
      \begin{array}{ll}
        | c_1 (y_1 - y_2) + c_2 (y_1^2 - y_2^2) | & = \hspace{0.27em} | c_1 +
        c_2 (y_1 + y_2) |  \hspace{0.17em} |y_1 - y_2 | \hspace{0.27em}\\
        & \le \hspace{0.27em} (|c_1 | + 2| c_2 |)  \hspace{0.17em} \|y_1 -
        y_2 \|_{\infty}
      \end{array}
    \end{equation}
    so
    ...
    \begin{equation}
      \frac{T_0^{\mu} (|c_1 | + 2| c_2 |)}{\Gamma (\mu + 1)} \le \frac{|c_1 |
      + 2| c_2 |}{[2 (|c_1 | + 2| c_2 | + 1)]} < \tfrac{1}{2}
    \end{equation}
    hence $\Phi$ is a strict contraction on $B_1$. The Banach fixed-point
    theorem provides a unique $y^{\ast} \in B_1$ with $\Phi (y^{\ast}) =
    y^{\ast}$, which solves \eqref{eq:frac_riccati_V}. The formal Puiseux
    series defined by \eqref{eq:a1_const}--\eqref{eq:ak_const} solves the same
    Volterra equation order-by-order; by uniqueness, the series equals
    $y^{\ast}$ on $[0, T_0]$, and hence converges for $|t^{\mu} | <
...
\begin{lemma}
  ...
  \begin{align}
    ...
    \quad \forall 1 \le n \le M  \label{eq:Pade1}\\
    0 & = a_n + \sum_{j = 1}^M q_j  \hspace{0.17em} a_{n - j} \tmxspace
    \forall M + 1 \le n \le 2 M  \label{eq:Pade2}
  \end{align}
...
\begin{enumerate}
  ...
  \item Evaluate $y_M (t) = \frac{P_M (t^{\mu})}{Q_M (t^{\mu})}$ at any $t \ge
  0$. Cost: $\mathcal{O} (M)$ per query
\end{enumerate}
