...
\usepackage{geometry,amsmath,amssymb,hyperref,latexsym,theorem}
...
\begin{align*}
  ...
  \hspace{-0.17em} (t - s)^{\mu - 1} \dd s + \frac{c_1}{\Gamma (\mu)}  \sum_{k
  \ge 1} a_k  \hspace{-0.17em} \int_0^t \hspace{-0.17em} (t - s)^{\mu - 1}
  s^{k \mu} \dd s\\
...
\begin{proof}
  \tmtextit{Item~(i).} Let $B_1 \assign \{y \in C [0, T_0] : \|y\|_{\infty}
  \le 1\}$ with the supremum norm, and define
  ...
  For $y \in B_1$ and $t \in [0, T_0]$, $|c_0 + c_1 y (t) + c_2 y (t)^2 | \le
  |c_0 | + |c_1 | + |c_2 |$, so by \eqref{eq:Ir_power},
  \[ | \Phi (y) (t) | \hspace{0.27em} \le \hspace{0.27em} (|c_0 | + |c_1 | +
     ...
     + 1)} . \]
  By the first term in \eqref{eq:T0_choice}, $T_0^{\mu}  (|c_0 | + |c_1 | +
  |c_2 |) / \Gamma (\mu + 1) \le (|c_0 | + |c_1 | + |c_2 |) / (|c_0 | + |c_1 |
  + |c_2 | + 1) < 1$; hence $\Phi (B_1) \subset B_1$. For $y_1, y_2 \in B_1$,
  \[ | c_1 (y_1 - y_2) + c_2 (y_1^2 - y_2^2) | \hspace{0.27em} =
     ...
     \|y_1 - y_2 \|_{\infty}, \]
  so
  \[ \| \Phi (y_1) - \Phi (y_2)\|_{\infty} \hspace{0.27em} \le \hspace{0.27em}
     \frac{T_0^{\mu}}{\Gamma (\mu + 1)} (|c_1 | + 2| c_2 |)  \hspace{0.17em}
     \|y_1 - y_2 \|_{\infty} . \]
  ...
  the same Volterra equation order-by-order; by uniqueness, the series equals
  $y^{\ast}$ on $[0, T_0]$, and hence converges for $|t^{\mu} | < T_0^{\mu}$,
  proving $R \ge T_0^{\mu} > 0$.
  
  \tmtextit{Item~(ii).} By item~(i), $g (z) = y (z^{1 / \mu}) = \sum_{k \ge 1}
  a_k z^k$ is analytic on $\{|z| < R\}$. For any $0 < r < R$, Cauchy's
  integral formula gives
  \[ a_k \hspace{0.27em} = \hspace{0.27em} \frac{1}{2 \pi \ii}  \oint_{|z| =
     r} \frac{g (z)}{z^{k + 1}} \hspace{0.17em} \dd z, \qquad |a_k |
     \hspace{0.27em} \le \hspace{0.27em} \sup_{|z| = r} |g (z) | \cdot r^{- k}
     \hspace{0.27em} = \hspace{0.27em} M (r)  \hspace{0.17em} r^{- k} . \]
  \tmtextit{Item~(iii).} For the classical case $\mu = 1$ with $c_2 \ne 0$,
  the linear substitution $y = - w' / (c_2 w)$ reduces $y' = c_0 + c_1 y + c_2
  y^2$ to a second-order linear ODE with constant coefficients for $w$; the
  zeros of $w$ generically lie at finite $t$, and at each such zero $y$ has a
  simple pole. The same algebraic structure of the
  recurrence~\eqref{eq:ak_const} persists for $\mu \in (0, 1)$, and the
  resulting $g (z)$ generically inherits movable singularities at finite
  distance from the origin in the $z$-plane (the exact location of the nearest
  singularity is encoded by the limit of the diagonal Pad{\'e} poles, see 
  Section~\ref{sec:pade}). Hence $R < \infty$ for generic parameters with $c_2
  \ne 0$.
\end{proof}
...
\section{Resurgence, Borel--Pad{\'e} Resummation, and
Riemann--Hilbert}\label{sec:resurgence}

The Pad{\'e}--M{\"u}ntz construction of the previous sections solves the
constant-coefficient fractional Riccati equation in closed algebraic form by
exploiting two independent structural facts: the M{\"u}ntz lattice closes the
differential algebra of $D^{\mu}, \hspace{0.17em} I^r, \hspace{0.17em} \cdot$;
and the diagonal Pad{\'e} in $z = t^{\mu}$ re-sums the resulting Puiseux
series past its finite radius of analyticity. These two facts are not
coincidences. Both are special cases of a deeper and far more general
principle drawn from the theory of {\tmem{resurgent functions}} and the
{\tmem{Riemann--Hilbert correspondence}}.

\subsection{The substitution $z = t^{\mu}$ is a power-series algebraization}

A constant-coefficient fractional ODE in $t$ is {\tmem{not}} algebraic in $t$
--- the operator $D^{\mu}$ is non-local, and the solution is generically only
H{\"o}lder continuous of exponent $\mu$ at the origin
(Remark~\ref{rem:regularity}). After the substitution $z = t^{\mu}$, the
solution $y (t)$ is represented by the convergent power series $g (z) =
\sum_{k \ge 1} a_k  \hspace{0.17em} z^k$ (Theorem~\ref{thm:riccati_series}),
whose coefficients satisfy the closed Gamma-ratio recurrence
\eqref{eq:a1_const}--\eqref{eq:ak_const}. In this sense, $z = t^{\mu}$ is the
canonical {\tmem{analytic}} variable for the problem: it converts the
H{\"o}lder-continuous solution $t \mapsto y (t)$ into a power-series-analytic
function $z \mapsto g (z)$, lifting the problem from the H{\"o}lder category
into the analytic category and making the full machinery of complex analysis
--- in particular Pad{\'e} approximation and analytic continuation ---
available.

A point of precision deserves emphasis: the substitution $z = t^{\mu}$ does
{\tmem{not}} reduce the Volterra equation \eqref{eq:frac_riccati_V} to a local
ordinary differential equation in $z$. A direct change of variables converts
\eqref{eq:frac_riccati_V} into the functional integral equation
\begin{equation}
  \label{eq:g_functional} g (z) \hspace{0.27em} = \hspace{0.27em} \frac{z}{\mu
  \hspace{0.17em} \Gamma (\mu)}  \hspace{-0.17em} \int_0^1 (1 - u^{1 /
  \mu})^{\mu - 1}  \hspace{0.17em} u^{1 / \mu - 1}  \hspace{0.17em} \left( c_0
  + c_1  \hspace{0.17em} g (zu) + c_2  \hspace{0.17em} g (zu)^2 \right) \dd u
\end{equation}
which involves the values $g (zu)$ for $u \in [0, 1]$ and is therefore
non-local in $z$. The ``algebraization'' performed by $z = t^{\mu}$ is not the
algebraization of the equation itself, but of the {\tmem{representation}} of
the solution: the move from a non-power-series object (the H{\"o}lder function
$y (t)$) to a convergent power series $g (z)$, whose coefficients are
extracted by a closed Gamma-ratio recurrence and whose analytic continuation
is then accessible through Pad{\'e} theory.

\subsection{Solutions are conjecturally resurgent transseries}

When $g (z)$ is meromorphic on $\C$ (the hypothesis of 
Theorem~\ref{thm:pade_riccati}) and the underlying flow possesses the
Painlev{\'e} property, the formal expansion at any singular point can be
promoted to a {\tmem{resurgent transseries}} in the sense of {\'E}calle
{\cite{ecalle1981,costin2008}}:
\begin{equation}
  \label{eq:transseries} \tilde{y} (z) \hspace{0.27em} = \hspace{0.27em}
  \sum_{n \ge 0} e^{- n \hspace{0.17em} A / z}  \hspace{0.17em} z^{n \beta_n} 
  \sum_{k \ge 0} a_{n, k}  \hspace{0.17em} z^k,
\end{equation}
where the leading sector $n = 0$ is the Puiseux series of 
Theorem~\ref{thm:riccati_series} and the higher sectors $n \ge 1$ encode
{\tmem{non-perturbative}} contributions controlled by the action $A$ (the
Riccati pole structure). The {\tmem{alien derivatives}} $\Delta_A  \tilde{y}$
of {\'E}calle measure the obstruction to analytic continuation across each
singularity, and the {\tmem{Stokes constants}} $S_A$ record the
discontinuities of the lateral Borel sums when crossing Stokes lines. For the
constant-coefficient fractional Riccati, the leading-sector coefficients are
exactly the M{\"u}ntz $a_k$; the higher sectors capture the multivaluedness of
$y$ around the movable poles. The resurgent picture is established rigorously
for integer-order ODEs with the Painlev{\'e} property and is conjectural in
the fractional regime; it is treated here as a structural perspective unifying
the M{\"u}ntz Tau, Pad{\'e}, and Riemann--Hilbert ingredients.

\subsection{Pad{\'e} is computational Borel--Pad{\'e} resummation}

The diagonal Pad{\'e} approximant in $z$ is the canonical computational
instantiation of {\tmem{Borel--Pad{\'e} resummation}}. Recall that the Borel
transform sends a divergent or finite-radius series $\sum a_k z^k$ to its
Borel transform $\hat{g} (\zeta) = \sum a_k \zeta^{k - 1} / \Gamma (k)$, then
resums by Laplace-transforming $\hat{g}$ along a ray. The Pad{\'e} approximant
$R_M$ computes essentially the same object: the rational function $P_M / Q_M$
is the unique meromorphic continuation that interpolates the formal series and
respects the underlying singularity structure. Concretely, 
Theorems~\ref{thm:dMdB} and~\ref{thm:NP} say that, under the meromorphy
hypothesis of  Theorem~\ref{thm:pade_riccati}, $R_M$ recovers exactly the
lateral Borel--Laplace sum on the real $z$-axis whenever no Stokes ray crosses
$(0, \infty)$ {\cite{costin2008,pommerenke1973}}; this is precisely the
boundedness hypothesis in  Theorem~\ref{thm:pade_riccati}.

\subsection{The Riemann--Hilbert structure}

The deeper geometric content is this. The poles of $R_M$ for finite $M$ are
not arbitrary --- as $M \to \infty$, they cluster along {\tmem{Stokes lines}}
in the $z$-plane, which are the branch cuts of the analytic continuation of
$y$. Nuttall--Pommerenke (Theorem~\ref{thm:NP}) is a weak version of this
fact: convergence in capacity means the spurious poles distribute themselves
along the natural boundary of analyticity. The strong version, due to Stahl,
Gonchar, and others {\cite{stahl1997}}, identifies the {\tmem{equilibrium
measure}} on those Stokes lines as the limiting pole distribution of the
diagonal Pad{\'e} sequence. This equilibrium problem is precisely a
{\tmem{scalar Riemann--Hilbert problem}} for the resolvent of a logarithmic
potential, and the diagonal Pad{\'e} asymptotics are obtained by the
Deift--Zhou steepest descent / Fokas--Its--Kitaev machinery applied to the
associated jump matrix {\cite{deift1999}}.

The unifying perspective of this paper, in maximum generality, is therefore:

\begin{quoteenv}
  {\tmem{For a constant-coefficient fractional ODE $\hspace{0.17em} D^{\mu} y
  = F (y) \hspace{0.17em}$ with polynomial $F$ whose substituted solution $g
  (z) \assign y (z^{1 / \mu})$ is meromorphic on $\C$, the substitution $z =
  t^{\mu}$ promotes the solution to a resurgent transseries in $z$. The
  diagonal Pad{\'e} in $z$ is a Borel--Pad{\'e} resummation of that
  transseries, and its global analytic continuation along the positive real
  axis is governed by a Riemann--Hilbert problem whose jump contours are the
  Stokes lines of the transseries.}}
\end{quoteenv}

\subsection{Generalizations}

The framework of this paper extends in three natural directions, each obtained
by relaxing one structural hypothesis.

\paragraph{Higher-degree polynomial nonlinearities (Bernoulli, Abel, and
beyond).} For $D^{\mu} y = F (y)$ with $F$ any polynomial of degree $d$, the
M{\"u}ntz lattice remains closed under the nonlinearity by
\eqref{eq:prod_Muntz}, so the Tau substitution yields a closed Gamma-ratio
recurrence of the same form as \eqref{eq:ak_const} but with a $d$-fold
convolution sum. The Pad{\'e} step is unchanged, the Hankel system
\eqref{eq:hankel_system} is unchanged, and the de~Montessus /
Nuttall--Pommerenke theorems apply identically (under the same meromorphy
hypothesis). Cubic Riccati (Abel of the second kind), quartic, and arbitrary
polynomial $F$ are all immediate.

\paragraph{Fractional ODE systems via Hermite--Pad{\'e}.} For a system
$D^{\mu} \tmmathbf{y}=\tmmathbf{F} (\tmmathbf{y})$ with polynomial
$\tmmathbf{F}$, each component admits a M{\"u}ntz Puiseux expansion, and the
scalar Hankel system is replaced by a {\tmem{matrix Pad{\'e}}} (or
simultaneous {\tmem{Hermite--Pad{\'e}}}) system involving multiple
denominators. The convergence theory generalizes: the scalar Riemann--Hilbert
problem becomes {\tmem{matrix-valued}}, with jump contours determined by the
eigenvalue structure of the linearization at the movable singularities
{\cite{deift1999}}. Multi-factor rough Heston, multi-asset rough volatility,
and multi-component fractional reaction--diffusion all fall in this class.

\paragraph{Non-constant coefficients: isomonodromy and Painlev{\'e}.} When
$D^{\mu} y = c_0 (t) + c_1 (t) y + c_2 (t) y^2$ with non-constant $c_i$ (the
natural setting for the rough Heston model when leverage and feedback are
time-varying {\cite{eleuch2019}}), the M{\"u}ntz lattice is generally no
longer preserved (the coefficients can introduce new exponents), the explicit
Gamma-ratio recurrence breaks down, and the Puiseux structure may need to be
enlarged to include logarithmic and Fuchsian terms. The associated
Riemann--Hilbert problem is then {\tmem{isomonodromic}} rather than
equilibrium-type, and the nonlinear special functions that arise --- notably
the Painlev{\'e} transcendents and their $q$- and fractional analogs ---
replace the elementary rational Pad{\'e} approximants. This is also where the
rough volatility literature stalls, and where new mathematics is genuinely
required.
