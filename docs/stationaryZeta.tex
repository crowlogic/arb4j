...
is \(N(\vartheta^{-1}(T))\). From \eqref{eq:counting},
\[
N(\vartheta^{-1}(T))
=
...
\to 1.
\]
By Hardy--Littlewood, if this limit holds then \(e(\vartheta(t))^2\sim c\,\vartheta'(t)\)
for some constant \(c>0$. Inserting back into the limit and using \eqref{eq:HL} and
\eqref{eq:theta-stirling} forces \(c=2\).
\end{proof}
...
Multiplying by \(e^{i\vartheta(t)}\) shows \(\overline{Z(t)}=Z(t)\).
\end{proof}
...
\]
For \(m,n\le M(t)\sim\sqrt{t/(2\pi)}\), the term \(\log(mn)\le 2\log M(t)\sim\log(t/(2\pi))\sim 2\vartheta'(t)-O(1)\).
Hence the derivative is bounded away from \(0\) or grows with \(t\), and integration
by parts bounds the time integral for each pair \((m,n)\) by \(O(1)\). Summing over
all pairs with \(m,n\le M(S)\), which number \(O(M(S)^2)=O(S)\), and dividing by
...
\[
2\int_0^1\cos(\Delta(1-u))\,du.
\]
Substituting \(v=1-u\),
\[
2\int_0^1\cos(\Delta v)\,dv=\frac{2\sin\Delta}{\Delta}.
\]
Multiplying by the prefactor \(\frac12\) from the product-to-sum splitting gives
\(\sin\Delta/\Delta\).
\end{proof}
...
\begin{proof}
Assembling the preceding lemmas: the oscillatory \((+)\)-part and the off-diagonal
\((-)\)-part both contribute \(0\) in the limit. The diagonal \((-)\)-part contributes
\(\sin\Delta/\Delta\). Multiplying by \(e^{-i\Delta}\) from \eqref{eq:R-leading} gives
