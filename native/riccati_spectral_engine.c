...
void spectral_engine_init(spectral_engine_t eng, const arb_t a, const arb_t b, const arb_t c, const arb_t alpha) {
    arb_init(eng->a); arb_set(eng->a, a);
    arb_init(eng->b); arb_set(eng->b, b);
    arb_init(eng->c); arb_set(eng->c, c);
    arb_init(eng->alpha); arb_set(eng->alpha, alpha);
    
    arb_poly_init(eng->u);
    arb_poly_init(eng->alpha_arr);
    arb_poly_init(eng->beta_arr);
    
    /* Seed the dynamic geometric 2D array */
    eng->sigma_alloc = 16;
...
void spectral_engine_clear(spectral_engine_t eng) {
    arb_clear(eng->a); arb_clear(eng->b); arb_clear(eng->c); arb_clear(eng->alpha);
    arb_poly_clear(eng->u); arb_poly_clear(eng->alpha_arr); arb_poly_clear(eng->beta_arr);
    for (slong i = 0; i < eng->sigma_alloc; i++) {
...
static void generate_next_puiseux_moment(spectral_engine_t eng, slong prec) {
    ...
    /* SPECTRAL TAU ALGEBRAIC GENERATOR 
     * Generating exact fractional series coefficients algebraically. Never arbitrarily truncated.
     */
    if (k == 1) {
        arb_t arg, gamma_val; 
        arb_init(arg); arb_init(gamma_val);
        arb_add_ui(arg, eng->alpha, 1, prec);
        arb_gamma(gamma_val, arg, prec);
        arb_div(u_k, eng->a, gamma_val, prec);
        arb_clear(arg); arb_clear(gamma_val);
    } else {
        arb_t sum, temp, num, den, arg;
        arb_init(sum); arb_init(temp); arb_init(num); arb_init(den); arb_init(arg);
        arb_zero(sum);

        if (!arb_is_zero(eng->c) && k > 2) {
            arb_t u_j, u_km1mj; 
            arb_init(u_j); arb_init(u_km1mj);
            ...
            arb_clear(u_j); arb_clear(u_km1mj);
        }
        ...
        arb_clear(sum); arb_clear(temp); arb_clear(num); arb_clear(den); arb_clear(arg);
    }
    
    /* arb_poly natively scales geometrically based on the accessed index */
...
void spectral_engine_next_jacobi(arb_t alpha_out, arb_t beta_out, spectral_engine_t eng, slong prec) {
    ...
    /* AMORTIZED GEOMETRIC SCALING FOR THE INFINITE HORIZON */
    ...
    /* THE MODIFIED CHEBYSHEV MAP 
     * Condenses the continuous infinite Stieltjes measure into the discrete Jacobi matrix natively,
     * entirely bypassing the fatal O(N^3) instability of Hankel determinant inversions.
     */
    arb_t temp_val; 
    arb_init(temp_val);
    ...
    if (M == 0) {
        ...
        arb_init(sig_0_1); arb_init(sig_0_0);
        ...
        arb_clear(sig_0_1); arb_clear(sig_0_0);
    } else {
        arb_t t1, t2, a_prev, b_prev, sig_curr, sig_prev, sig_prev2;
        arb_init(t1); arb_init(t2); arb_init(a_prev); arb_init(b_prev); 
        arb_init(sig_curr); arb_init(sig_prev); arb_init(sig_prev2);
        ...
        arb_clear(t1); arb_clear(t2); arb_clear(a_prev); arb_clear(b_prev); 
        arb_clear(sig_curr); arb_clear(sig_prev); arb_clear(sig_prev2);
    }
